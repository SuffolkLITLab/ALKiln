...
const setup = async () => {
  ...
  // If server is not responding, try again for some number of times
  while ( !succeeded && num_tries <= max_tries ) {  
    try {
      log.info({ type: `setup`, pre: `Trying to create a docassemble project.` });
      await create_project();
      log.info({ type: `setup`, pre: `Trying to pull code from the "${ session.get_branch_name() }" branch.` });
      let task_id = await da.pull();
...
const wait_for_server_to_restart = async ( task_id ) => {
  ...
  while ( !done_restarting && num_tries <= max_tries ) {
    ...
    try {
      done_restarting = await da.has_server_restarted( task_id );
    } catch ( error ) {
      log.debug({ pre: `How did nothing get logged?` });
      console.log( error );
    }
