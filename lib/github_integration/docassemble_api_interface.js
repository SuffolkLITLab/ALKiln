...
// node -e 'require("./lib/github_integration/docassemble_api_interface.js").is_server_responding()'
dai.is_server_responding = async function ( timeout=5000 ) {
  /* Make an arbitrary request to the server to see if it's responding. */
  // What is response when server isn't ready? Undefined?
  try {
    let id = await dai.get_dev_id( timeout );
...
dai.get_dev_id = async function ( timeout=3000 ) {
