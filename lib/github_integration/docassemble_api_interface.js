    ...
    try {
      ...
      if ( error.response && error.response.data ) {
        ...
        if ( is_timeout_error( error ) && name_incrementor < max_tries ) {
          log.info({ pre: `Server is busy when trying to create project.` });
          await time.waitForTimeout( wait_seconds * 1000 );
          continue;
...
da.loop_to_pull = async function ( pull_options, loop_options ) {
  ...
  *    might have very different wait times for module restarts. */
  ...
  return await dai.loop_func_on_server_timeout(() => {
    dai.pull( pull_options );
...
dai.has_server_restarted = async function ( task_id, options ) {
