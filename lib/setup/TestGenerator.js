 ...
 *     problem we haven't solved yet.) 2. Saying
 *     "ALKILN_MAX_RANDOM_TESTS_PER_SCENARIO" is confusing when "Scenario" means
 *     different things - a generator Scenario and a regular Scenario. Also,
 *     "ALKILN_MAX_RANDOM**IZED**_TESTS_PER_SCENARIO" might be more accurate. Is
 *     "ALKILN_MAX_TESTS_PER_GENERATOR" better?
...
module.exports = class TestGenerator {
  ...
  create({ sources_path, logger }) {
    ...
    log.debug({ code: `ALK0267`, context: `generating files`, }, `Constrained random answers tests:\n`, new_files_strings );
    ...
    log.debug({ code: `ALK0271`, context: `generating files`, }, `Tried to save generated files to ${ artifacts_path }` );
  ...
  get_new_file_contents({ doc_AST={}, file_text=``, generator_path }) {
    ...
    if ( !scenario_objs[0] ) {
      let syntax_error = log.error({ code: `ALK0235`, context: `generating files`, before: `\n\n`, },
        `It looks like the author's generator .feature file at "${ generator_path }" has 0 Scenarios.`
      );
    ...
    // One string for each generator Scenario
    let { scenarios_strings, total_Scenarios_made, errors: scenarios_errors } = this.get_new_Scenarios_strs({
      scenario_objs,
      generator_lines
    });
    generation_errors.push( ...scenarios_errors );

    let count_msg = `ALKiln generated ${ total_Scenarios_made } Scenario or Scenarios`;
  ...
  get_new_Scenarios_strs({ scenario_objs, generator_lines }) {
     ...
     *     into. Each generator Scenario gets one string of all its Scenarios.
     * 
     * @param {object} obj - Named arguments
    ...
    for ( let scenario_obj_i = 0; scenario_obj_i < scenario_objs.length; scenario_obj_i++ ) {
      ...
      let { new_Scenarios_text, num_made } = errs_n_warns.add(
    ...
    return { scenarios_strings: strs_for_all_new_Scenarios, total_Scenarios_made, ...errs_n_warns };
  }
  ...
  get_1_text_block_of_all_new_Scenarios_from_1_generator_Scenario({
    ...
    if ( got_fewer_tests_than_desired ) {
      let fewer_test_msg = `ALKiln tried a bunch of times, but it could only`
      + ` make ${ new_test_bodies.length } *unique* tests out of the`
      + ` ${ num_to_make } the author wanted for this Scenario. This probably`
      + ` happened because ALKiln makes sure every test is unique^1. Maybe`
      + ` there were too few combinations for ALKiln to use. The Scenario:`
      + `\n      ${ scenario_description_for_logs({ scenario }) }`
      + `\n\n[^1]: "Unique tests" are only unique from the other tests ALKiln`
      + ` is generating *right now*. ALKiln doesn't check your regular tests.`;

      let warn_too_few = log.warn({ code: `ALK0260`, context: `generating files generator-scenario-level`, },
        fewer_test_msg,
      )
    ...
    let num_made = new_test_bodies.length;
    new_Scenarios_text = add_total_tests({
      scenario_group_str: new_Scenarios_text, num_tests: num_made
    });

    log.debug({ code: `ALK0261`, context: `generating files`, }, `new_Scenarios_text before adding warnings:\n`, new_Scenarios_text );

    return { new_Scenarios_text, num_made, ...misfits };
  }
...
function save_files({ tests_path, artifacts_path, file_strings }) {
  ...
  log.debug({ code: `ALK0269`, context: `generating files`, },
    `Saving ${ file_strings.length } newly generated files in "${ tests_path }" and "${ artifacts_path }"`
  );
  ...
  log.info({ code: `ALK0270`, context: `generating files`, },
    `Saved new generated files in "${ artifacts_path }"`
  );
}
