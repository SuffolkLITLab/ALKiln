  ...
  if ( num_to_make !== user_num ) {
    warnings.push(
      log.warn({ code: `ALK0241`, context: `generating files`, },
        `The author asked to make ${ user_num } tests, but maximum allowed is ${ num_to_make }. ALKiln will now make ${ num_to_make } tests. If you want to increase the maximum, use the ALKILN_MAX_RANDOM_TESTS_PER_SCENARIO input. Read more about inputs at https://assemblyline.suffolklitlab.org/docs/alkiln/writing/#workflow-inputs.`
      )
    );
  }
