...
/**
 * TODO: Update ALKilnInThePlayground to let author download the generated
 *     tests. Also, store them in a unique location. 
 ...
 *     file incorrectly.
 * Discuss: Require author to use "Template Scenario:" as they keyword (Also
 *     "Template Scenario Outline:") (or "Scenario Template", maybe harder to
 *     scan) for 2 reasons. 1. Gets the vocab of "template" in early (though
 *     that doesn't match "constraints" Step, which is a whole other language
 *     problem we haven't solved yet.) 2. Saying "ALKILN_MAX_RANDOM_TESTS_PER_SCENARIO"
 *     is confusing when "Scenario" means different things - a template
 ...
 * Discuss: We might give various levels of categories for warnings. For example,
...
module.exports = class TestGenerator {
  ...
  create({ sources_path, logger }) {
    ...
    log.debug({ code: `ALK0267`, context: `generating files`, }, `Constrained random answers tests:`, new_files_strings );
    ...
    log.info({ code: `ALK0271`, context: `generating files`, }, `Tried to save generated files to ${ artifacts_path }` );
  ...
  parse_file({ file_text, generator_path, logger }) {
    ...
    if ( generated_parsing_errors.length > 0 ) {
      let syntax_error = log.error({ code: `ALK0266`, context: `generating files`, before: `\n\n`, },
  ...
  get_new_file_contents({ doc_AST={}, file_text=``, generator_path }) {
    ...
    if ( !scenario_objs[0] ) {
      let syntax_error = log.error({ code: `ALK0235`, context: `generating files`, before: `\n\n`, },
        `It looks like the author's template .feature file at "${ generator_path }" has 0 Scenarios.`
      );
    ...
    if ( scenario_objs.length > 1 ) {
      let too_many_scenarios = log.error({ code: `ALK0236`, context: `generating files`, },
        `A template file can only have 1 template Scenario. This one has ${ scenario_objs.length }.`
      );
    ...
    if ( scenarios_strings.length === 0 ) {
      let error = log.error({ code: `ALK0263`, context: `generating files`, }, count_msg );
      generation_errors.push( new Error( error ));
      return { new_contents: null, errors: generation_errors };
    }

    log.info({ code: `ALK0264`, context: `generating files`, }, count_msg );
    ...
    if ( !new_file_contents ) {
     ...
     let error = log.error({ code: `ALK0265`, context: `generating files`, }, msg );
  ...
  get_new_Scenarios_strs({ scenario_objs, generator_lines }) {
    ...
    // Soon we want to be able to handle multiple template Scenarios per file
    for ( let scenario_obj_i = 0; scenario_obj_i < scenario_objs.length; scenario_obj_i++ ) {
      ...
      log.info({ code: `ALK0237`, context: `generating files`, },
      ...
      log.debug({ code: `ALK0262`, context: `generating files`, }, `new_Scenarios_text with ${ errs_n_warns.warnings.length } warnings:\n`, new_Scenarios_text );
  ...
  get_1_text_block_of_all_new_Scenarios_from_1_generator_Scenario({
    ...
    if ( got_fewer_tests_than_desired ) {
      let warn_too_few = log.warn({ code: `ALK0260`, context: `generating files template-scenario-level`, },
        `ALKiln could only make ${ new_test_bodies.length } *unique* tests out of the ${ num_to_make } the author wanted for this Scenario. Note that ALKiln compares all generated Scenarios with each other to make sure all are unique. This Scenario may be a duplicate of a Scenario generated from a previous template. The Scenario:\n      ${ scenario_description_for_logs({ scenario }) }`,
      )
      // Discuss: Should we add more structure to Misbehaviors to give them
      //     context levels (Feature, template Scenario, generated Scenario)
    ...
    log.debug({ code: `ALK0261`, context: `generating files`, }, `new_Scenarios_text before adding warnings:\n`, new_Scenarios_text );
  ...
  try_to_get_one_unique_middle({
    ...
    log.debug({ code: `ALK0249`, context: `generating files`, }, `Trying to get unique middle` );
    ...
    while ( !is_unique && attempts.count < attempts.max ) {
      ...
      log.debug({ code: `ALK0257`, context: `generating files` }, `Generated test:\n${ one_test_body }` );

      let is_duplicate = this.all_Scenarios_bodies_map[ one_test_body ];
      if ( is_duplicate ) {
        log.debug({ code: `ALK0258`, context: `generating files` }, `Test is a duplicate on attempt ${ attempts.count }.` );
        if ( attempts.count >= attempts.max ) { clear_inner_state(); }
        continue;
      }

      // Otherwise, add to the global map and finish
      log.debug({ code: `ALK0259`, context: `generating files` }, `This test is unique on attempt number ${ attempts.count }.` );
      is_unique = true;
...
function get_generator_AST({ file_text, generator_path }) {
  ...
  if ( generator_errors.length > 0 ) {
    let syntax_error = log.error({ code: `ALK0232`, context: `generating files`, before: `\n\n`, },
      `The author's template .feature file at "${ generator_path }" has Gherkin syntax error(s). You might see more warnings above or below this one.\n`,
  ...
  if ( !doc_AST.feature ) {
    let feature_error = log.error({ code: `ALK0233`, context: `generating files`, before: `\n\n`, },
      `The author's template .feature file at "${ generator_path }" needs to include the \`Feature\` keyword.`
    );
  ...
  let unknown_error = log.error({ code: `ALK0234`, context: `generating files`, before: `\n\n`, },
    `The author's template .feature file at "${ generator_path }" ran into an unanticipated error when ALKiln tried to parse it. If you have a moment, we would love for you to send us your debug.txt.`
  );
...
function get_fore_Steps({ scenario }) {
  ...
  if ( there_were_no_fore_Steps ) {
    misfits.add({ errors: [
      new Error( log.error({ code: `ALK0242`, context: `generating files`, },
        `A template Scenario's first Step must say how many constrained random answers tests to make. The template Scenario on line ${ scenario.location.line } is missing that Step. The Scenario's description is:\n${ description }`
      ) )
    ] });
  } else {
...
function get_number_of_Scenarios_to_generate2({ step, prev_num_to_make }) {
  ...
  if ( prev_num_to_make ) {
    ...
    warnings.push( 
      log.warn({ code: `ALK0238`, context: `generating files`, },
        `A template Scenario has too many Steps that set the number of tests to make. ALKiln will use the final number Step it finds or calculates. The current Step:\n      ${ text }`
      )
    );
  }
  ...
  if ( number_matcher === null ) {
    warnings.push( 
      log.warn({ code: `ALK0239`, context: `generating files`, },
        `A template Scenario is missing the number of tests to make. ALKiln will now make 1 constrained random answers test. The current Step:\n      ${ text }`
      )
    );
  ...
  if ( number_matcher && number_matcher.length > 1 ) {
    ...
    //    than the first. Also, matches Story Table behavior.
  ...
  let num_to_make = Math.min(
    session_vars.get_max_tests_per_template_scenario(),
    user_num
  );

  if ( num_to_make !== user_num ) {
    warnings.push(
      log.warn({ code: `ALK0241`, context: `generating files`, },
        `The author asked to make ${ user_num } tests, but maximum allowed is ${ num_to_make }. ALKiln will now make ${ num_to_make } tests. If you want to increase the maximum, use the ALKILN_MAX_RANDOM_TESTS_PER_SCENARIO input. Read more about inputs at https://assemblyline.suffolklitlab.org/docs/alkiln/writing/#workflow-inputs. The Step:\n      ${ text }`
      )
    );
  }
...
function get_definitely_unqiue_Scenario_lines({
  ...
  log.debug({ code: `ALK0248`, context: `generating files`, },
...
function get_possibly_unique_lines_data({
  ...
  if ( !found_at_least_1_significant_step ) {
    let error = log.error({ code: `ALK0256`, context: `generating files`, },
...
function get_random_Story_Table({
  ...
  if ( !step.dataTable ) {
    ...
    let error = log.error({ code: `ALK0252`, context: `generating files`, },
  ...
  log.debug({ code: `ALK0255`, context: `generating files`, }, `Table:`, table );
...
function get_final_ids({ step_text }) {
  ...
  if ( ids === null ) {
    let missing_ids_msg = `The constrained random answers Step is missing its question ids. ALKiln is unable to create random tests for this template Scenario. The Step:\n${ step_text }`;
    let error = log.error({ code: `ALK0250`, context: `generating files`, }, missing_ids_msg );
...
function pick_random_answer({ answers_str=`` }) {
  ...
  if ( choices.includes(``) ) {
    ...
    log.debug({ code: `ALK0254`, context: `generating files` },
...
function add_total_tests({ scenario_group_str, num_tests }) {
  /** Paste in the total number of tests generated for this template Scenario.
   * 
   * @param {object} obj - Named arguments
...
function add_warnings_Scenario({ scenarios_str, warnings }) {
   ...
   *     identifying the template Scenario of the subsequent new Scenarios.
   *     That is, even when there is no warning to give.
   * 
   * TODO: If/when we allow multiple template Scenarios, use the ID here too
   * 
   * @param {object} obj - Named arguments
  ...
  let keyword_line = `Scenario: ALKiln begs the author's forgiveness. The next tests might be flawed.`;
...
function save_files({ tests_path, artifacts_path, file_strings }) {
  ...
  log.debug({ code: `ALK0269`, context: `generating files`, },
    `Saving ${ file_strings.length } newly generated files in "${ tests_path }"`
  );

  let count = 1;
  for ( let file_str of file_strings ) {
    let filename = `_alkiln_generated_${ count }_${ Date.now() }.feature`;
    count++;
  ...
  log.debug({ code: `ALK0270`, context: `generating files`, },
...
function get_tweens({
  ...
  try {
    ...
    log.debug({ code: `ALK0246`, context: `generating files`, },
    ...
    errors.push( new Error( log.error({ code: `ALK0247`, context: `generating files`, },
...
function get_authors_indent({ located }) {
  ...
  if ( typeof located !== `object` || !located.location ) {
    log.debug({ code: `ALK0253`, context: `generating files`, }
