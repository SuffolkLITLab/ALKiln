...
dai.server_is_busy = async function ( options ) {
  ...
  try {
    ...
    log.info({ pre: `Tried to check if the server was busy, but ran into an unexpected error.` });
...
dai.get_dev_id = async function ( options ) {
  /* Mirror da.get_dev_id for consistency. See https://docassemble.org/docs/api.html#user. */
  let { timeout } = options ||  { timeout: 3000 };
  return await da.get_dev_id( timeout );
};  // Ends dai.get_dev_id();


dai.loop_to_get_dev_id = async function ( dev_id_options, loop_options ) {
  /* For consistency and future-proofing */
  dev_id_options = dev_id_options ||  { timeout: 3000 };
  return await dai.loop_func_on_server_timeout(async () => {
    await dai.get_dev_id( dev_id_options );
  }, loop_options );
};  // Ends dai.loop_to_get_dev_id();


// node -e 'require("./lib/github_integration/docassemble_api_interface.js").try_api_key()'
dai.try_api_key = async function ( dev_id_options, loop_options ) {
  /* Make sure the api key is the right one for this server.
  *   See https://docassemble.org/docs/api.html#user. */
  dev_id_options = dev_id_options ||  { timeout: 3000 };

  log.info({ pre: `Checking that the docassemble server's testing account's API key is valid.` });
  try {

    // There's only one possible success result
    // As long as this succeeds, we're good
    await dai.loop_to_get_dev_id( dev_id_options, loop_options );
...
dai.has_task_finished = async function ( task_id, options ) {
  ...
  *    See https://docassemble.org/docs/api.html#restart_status. */
  ...
  try {
    ...
    log.info({ pre: `Tried to check for server restart, but ran into an unexpected error.` });
    throw error;
