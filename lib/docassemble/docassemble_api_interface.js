...
da_i.loop_to_create_project = async function( creation_options ) {
  ...
  *    isn't already taken and then use that name to create the Project. */
  ...
  // Unlikely, but 2 devs could run locally and create the same Project name
  let num_tries = 0;
  ...
  while ( num_tries <= max_tries ) {
    ...
    try {
      ...
      project_name = session_vars.get_project_base_name().replace(/[^A-Za-z0-9]/g, '') + `${Date.now()}`;
      ...
      if ( response && response.data ) {
        ...
        if ( is_timeout_error( error ) && num_tries < max_tries ) {
          log.info({ pre: `The docassemble server was busy when we tried to `
            + `create the Project. Attempt ${ num_tries } of `
            + `${ max_tries }. Will try again in ${ wait_seconds } seconds.` });
          num_tries--;  // Don't count this try
...
da_i.install_on_server = async function ( install_options ) {
  let { timeout } = install_options || { timeout: DEFAULT_MAX_REQUEST_MS };

  try {
    let response = await _da_REST.install_on_server(
      session_vars.get_repo_url(),
      session_vars.get_branch_name(),
      timeout
    );
    if ( response && response.status === 200 ) {
      log.debug({ pre: `install task id: ${ response.data.task_id }` });
      return response.data.task_id;
    }
    return null;
  } catch (error ) {
    log.error({ pre: `Installing on the docassemble serve ran into an unexpected error.`, data: error.toJSON()});
  }

};

// node -e 'require("./lib/docassemble/docassemble_api_interface.js").has_task_finished()'
da_i.has_task_finished = async function ( api_endpoint, task_id, options ) {
  ...
  *    See https://docassemble.org/docs/api.html#restart_status. */
  ...
  try {
    ...
    let response = await _da_REST.has_task_finished( api_endpoint, task_id, timeout );
    ...
    if ( response.data.status === 'completed' ) {
      ...
      // Server is working, but status 'unknown'? Bad design, but this
      // does get returned when install packages, so return true
      log.info({
...
da_i.wait_for_playground_to_restart = async ( task_id, options ) => {
  return await da_i.wait_for_server_to_restart( "/api/restart_status", task_id, options );
};

da_i.wait_for_package_to_restart = async ( task_id, options ) => {
  return await da_i.wait_for_server_to_restart( "/api/package_update_status", task_id, options );
};

da_i.wait_for_server_to_restart = async ( api_endpoint, task_id, options ) => {
  ...
  *     that aren't equivalent to the server being done with a task. */
  ...
  while ( !done_restarting && num_tries <= max_loops ) {
    ...
    done_restarting = await da_i.has_task_finished( api_endpoint, task_id, { timeout: wait_for_response } );
    if ( !done_restarting ) {
