  ...
  *    isn't already taken and then use that name to create the Project. */
  ...
  while ( name_incrementor <= max_tries ) {
    ...
    try {
      ...
      project_name = session_vars.get_project_base_name().replace(/[^A-Za-z0-9]+/, '') + name_incrementor;
      log.info({ pre: `Trying to create a Project with the name ${ project_name }` });
      response = await _da.create_project( project_name, timeout );
  ...
  if ( response.data && response.data.includes( `Invalid project` ) && name_incrementor > max_tries) {
