...
// node -e 'require("./lib/docassemble/docassemble_api_interface.js").delete_project( "create1" )'
da_i.delete_interview = async function ({ interview_vars = null, options = {} }) {
  /** Delete a user's interview. Do we need to watch for a timeout? */

  // Defaults
  let { timeout } = options ||  { timeout: DEFAULT_MAX_REQUEST_MS };

  if ( interview_vars && interview_vars.i ) {
    try {
      log.debug({ type: `da api info`, code: `ALK0188`, pre: `Trying to delete the current interview.` });
      // Actual work:
      let response = await _da_REST.delete_interview( timeout, interview_vars );
      log.debug({ type: `da api info`, code: `ALK0189`,
        pre: `Deleted interview "${ interview_vars.uid }" at ${ interview_vars.i }.`,
        data: response
      });
    } catch ( error ) {
      error.interview_vars = interview_vars;
      let msg = `Ran into an error when deleting the interview. See https://docassemble.org/docs/api.html#session_delete.`
