  ...
  *    isn't already taken and then use that name to create the Project. */
  ...
  while ( num_tries <= max_tries ) {
    ...
    try {
      ...
      // Try adding a new timestamp to the name. Must remove all non-valid characters
      ...
      // ends if error.response
        ...
        if ( is_timeout_error( error ) && num_tries < max_tries ) {
          ...
          num_tries--;  // Try this name again next time
    ...
    // TODO: Add a message that we're done trying to create a project, successful or not
  }  // ends while
