...
da.delete_interview = async function ( timeout, { i = null, uid = null }) {
  /* Delete a user's interview. */
  let options = {
    method: 'DELETE',
    headers: { 'X-API-Key': session_vars.get_dev_api_key() },
    // Can't use `api/session`. Author may have signed in with an
    // account that's different than the ALKiln dev account.
    url: `${ session_vars.get_da_server_url() }/api/interviews`,
    params: { i: i, session: uid },
    timeout: timeout,
  };
