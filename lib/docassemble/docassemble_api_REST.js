...
da.install_on_server = async function ( github_repo, branch_name, timeout ) {
  let options = {
    method: 'POST',
    url: `${ session_vars.get_da_server_url() }/api/package`,
    data: qs.stringify( {
...
da.has_playground_task_finished = async function ( task_id, timeout ) {
  return await da.has_task_finished( "/api/restart_status", task_id, timeout);
}

da.has_package_task_finished = async function ( task_id, timeout ) {
  return await da.has_task_finished( "/api/package_update_status", task_id, timeout);
}

// node -e 'require("./lib/docassemble/docassemble_api_REST.js").has_task_finished()'
da.has_task_finished = async function ( api_endpoint, task_id, timeout ) {
  /* Check task status based on `task_id`. */
  let options = {
    method: 'GET',
    headers: {'X-API-Key': session_vars.get_dev_api_key()},
    url: `${ session_vars.get_da_server_url() }${ api_endpoint }`,
    params: {
      task_id: task_id,
    },
    timeout: timeout,
