...
const setup = async () => {
  ...
  */
  ...
  // internet. Some command here or in dainstall don't work on windows.
  ...
  // the response to find out

  if ( result.status !== 0 ) {
    log.error({
      type: `setup`,
      pre: `Error: Status code ${ result.status }. Could not dainstall this package on your server. See above for more details.`
    });
    throw(result.stderr);
  }
