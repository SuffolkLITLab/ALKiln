...
const setup = async () => {
  /** This is for a package being installed in an author's account (Playground)
   *  on their docassemble server. Using the author's
   *   DOCASSEMBLE_DEVELOPER_API_KEY secret, create a new Project, pull code
   *   into that project, and, if needed, wait for the server to reload.
   */
  ...
  if ( result.status !== 0 ) {
    ...
    log.throw({ code: `ALK0047`, context,
      ...
      error: new Error(`Error while installing project on your docassemble server`) },
      install_err_msg
    );
  }
...
// 2. Prepare logs
// Local devs: This will be separate from your test paths unless you pass a
//    `path` argument into the cucumber run command
