  ...
  try {
    ...
    if ( response.data && response.data.task_id ) {
      if ( session.get_debug() ) {
        console.log( `Pull task id: ${ response.data.task_id }` );
        let done = await is_package_done_updating( response.data.task_id, timeout );
        console.log( `Is package done being pulled? ${ done }` );
      }
      return response.data.task_id;
    } else {
      if ( session.get_debug() ) { console.log( `When pulling code, there either no data or no id. Response:`, response ); }
      return response;
    }
  } catch (error) {
    // Should we throw in here?
