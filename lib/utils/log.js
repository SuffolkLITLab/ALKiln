...
class Log {
  ...
  // Avoid infinite loops. Feels fragile. Discuss: Accumulate debug log failures
  // instead of logging them to the debug log every time so we can avoid using
  // this.
  loop_count = 0;
  max_loops = 5;
   ...
   *
   * Summary of flow: .debug() -> write to the console -> return or throw
   *
  ...
  debug( debug_opts = {}, ...logs ) {
    ...
    try {
      ...
      // TODO: exclude ": " from metadata. Inserted it when we need it.
      let metadata_str = this._format_metadata({ level, icon, code, context });
      ...
      try {
        ...
        // TODO: remove check for instance of Error because non warnings/errors
  ...
  _stringify_logs(...logs) {
    ...
    try {
      ...
      try {
        // Discuss: Should `stringify_error` get recorded even when
        // `this._inspect()` works?
  ...
  _inspect({ value }) {
    try {
      ...
      return `🔎 ALK0229 inspect log WARNING [${ Date.now() }]: Skipped util.inspect. ${ direct_inspect_err.stack }`;
    }
  }
