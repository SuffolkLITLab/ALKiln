...
log.error = function ( error='', msg='' ) {
  if ( typeof error === 'object' && error.response && error.response.status ) {
    console.trace( `Docassemble API error log ${ error.response.status }: ${ error.response.data } See ${ msg } for more details.` );
  } else {
    console.log( `ALKiln experienced an error. ${ msg }` );
    console.trace( error );
  }
};  // Ends log.error()
...
log.debug = function ({ type='', pre='', data='', post='' }) {
  ...
  if ( session_vars.get_debug() ) { console.log( `ALKiln${ type } debug: ${ pre }`, data, post ); }
}
