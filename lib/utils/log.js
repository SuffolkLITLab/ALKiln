 ...
 * - [x] Write user errors and warnings (sent to .console()) to unexpected results file
 ...
 * - [ ] Discuss: Insert periods between inline logs?
 *
 * Scratchpad
 *
   *
   ...
   *
   * Using Log:
   * foo = new Error('something');
   * log.throw({code: 'ALK000d', context: 'setup', error: foo}, 'some log');
   *
   * In more detail:
   * - Console logs: Logs setup and takedown messages to the console
   *   after writing them to the debug files.
   * - Errors: Allows setup and takedown processes to throw more meaningful
   *   error messages and metadata when they throw
   *   errors. Log first writes these messages to the debug files.
   * - Saves messages with additional metadata to debug files throughout the tests, including before doing the above.
   *
   * It lets processes use `.throw()` to throw errors with more
   *  meaningful messages.
   *
   * It also lets processes throw errors with more
   *  meaningful messages. ("lets" = allows, provides, etc.)
   *
   * It also gives processes a way to throw errors with more
   *  meaningful messages.
 * */

class Log {
  /** This class includes methods to save meaningful messages with
   *  additional metadata to the debug files. Some methods show
   *  these messages to the user. It includes `.throw()`, for throwing
   *  errors with meaningful information.
   *
   * In preparation for integrating with the test reports, it
   *   also logs to the console without new lines (process.write).
   *
   * The different log files:
   * - debug_log.txt contains detailed info for internal debugging.
   * - unexpected_results.txt contains detailed messages only about
   *   warnings and failures. It will only exist if an unexpected
   *   result happens.
   * - To integrate:
   *    - report.txt is a pretty file that is only complete at the end of all tests.
   *    - report_log.txt is not formatted nicely. It saves the same info as the report
   *      while tests are running in case tests end early.
   *    - TODO: Consider a separate report file for just report error messages.
   *
   * TODO: integrate reports
   *
   * TODO: Document that we're not using console.Console or write streams
   * because we can't
   ...
   * .info/success -> .console -> .debug saves to debug log -> .console logs to the console. Returns string of logs.
   * .warn -> .console -> .debug saves to debug and unexpected results logs -> .console logs to the console. Returns string of logs.
   * .throw -> .console -> .debug saves to debug and unexpected results logs -> .console throws the error, no return value
   ...
   * 1. Log (to the console) info during proccesses that wrap test
   *  runs - setup takedown, etc., for the authors and our team. Also,
   *  throw relevant errors. These can be .success, .info, .warn, .throw.
   * 2. All of them: Save debugging information from start to finish for us to
   *  help authors and discover our own bugs. We can give them levels
   *  too, like warning. We assume authors will look at this too, so
   *  we try to use levels that will be useful to them as well. For
   *  example, we only use the `error` level in debug when the failure
   *  is something that the authors can affect. For example, we will
   *  avoid `error` for failures to save to log files or failures to
   *  delete interview sessions. Instead, we will use `warn`.
   * 3. In future, integrated with report and report log, that shows
   *  information to the user about their test run.
   * 4. Avoid throwing Log's own errors. Those should be stored in
   *  debug files.
   *
  ...
  constructor({ path, context } = {}) {
     ...
     * @params
     * options { object }
     * options.path { str } - Optional. The directory where it should
  ...
  console( console_opts = {}, ...logs ) {
     ...
     *   code: 'ALK00d1',
     *   context: 'demo',
     * }, 'demo info log', 'demo info log 2');
     * // & ALK00d1 demo INFO [2024-08-16 00:30:39UTC]: demo info log demo info log 2
     *
     * try {
     ...
     * //   demo: 'obj'
     * // }
     * // <the actual error with its stack>
     *
    ...
    if ( do_throw ) {
      ...
      // logs that get logged here before catching will be confusing.
      try {
  ...
  debug( debug_opts = {}, ...logs ) {
    ...
    try {
      ...
      try {
        ...
        // Answer: an actual error will be thrown, but with `logs` logged
        // to the console. `formatted` won't be logged there, just returned.
        // We can't test this without a spy
        formatted = `${ before }${ metadata_str }${ stringified }`;
      } catch ( formatted_log_error ) {
        // Consider using `util.inspect()` for the error
        logging_errors.push(`🔎 ALK0212 internal WARNING [${ Date.now() }]: Skipped creating full debug log`);
        logging_errors.push( formatted_log_error );
      }
      ...
      try {
        ...
        // Consider using `util.inspect()` for the error
        logging_errors.push(`🔎 ALK0213 internal WARNING [${ Date.now() }]: Could not append to debug log file`);
        logging_errors.push( fs_append_error );
      }
      ...
      if ( logging_errors.length > 0 ) {
        for ( let logging_error of logging_errors ) {
          if ( fs_append_failed ) {
            console.warn( logging_error );  // TODO: Need a spy to test this
          } else {
            fs.appendFileSync( `${ this.path }/${ this.debug_log_filename }`, `\n` + logging_error );
  ...
  _combine_logs_and_possible_error({ logs=[], maybe_error, do_throw }) {
    /** Return a list combining values to be logged if appropriate */
    try {

      // If we're not supposed to throw, we already gave a warning
      // that there should be no value for error. Here, we make
      // sure it gets no value.
      ...
      if ( maybe_error instanceof Error ) {
        ...
        // The stack trace will be less helpful, but it's all we can do
        // if the caller hasn't already created the error.
  ...
  _get_safe_val( risky_value ) {
    /** Return a value that is not a circular reference, regardless
     *  of what came in. */
    let safe_val;

    try {

      // Possibly trigger an error (e.g. circular reference error)
      // (new Error() doesn't get caught)
      let will_cause_error_if_its_a_circular_reference = JSON.stringify( risky_value );
      // If no error got triggered, this value will be safe
      // to use to create an error and we can return it as is
      ...
      try {
        // If stringifying did create an error, try modifying it
        // to make it safe. Not as good as the original, but
        // better than nothing.
  ...
  stdout(stdout_opts = {}, ...logs) {
     ...
     *  and for reports. Logs are joined with no separator.
     *
     * @params
     * options { obj }
