 ...
 * - [ ] Discuss: Should `.error()` return an Error? See discussion:
 *   https://github.com/SuffolkLITLab/ALKiln/discussions/988
 * */
...
class Log {
  ...
  constructor({ path, context=`` } = {}) {
    ...
    console.log(`🐛🐛🐛🐛🐛 ALKTemp debugging Log path. Calculated path:`, this.path);
    if ( is_new_folder ) {
      this.info({ code: `ALK0214`, context, }, files_msg );
    }  else {
      this.debug({ code: `ALK0277`, context, }, files_msg );
    }
  ...
  _make_or_get_folder({ dir }) {
    ...
    try {
      ...
      console.log(`🐛🐛🐛🐛🐛 ALKTemp debugging Log path. Given dir:`, dir);

      // GitHub actions pass in a dir, but other processes, like
      //    ALKilnInThePlayground, don't have a path
      if ( !dir ) {
        console.log(`🐛🐛🐛🐛🐛 ALKTemp debugging Log path. No dir given`);
        if ( session_vars.get_artifacts_path_name() ) {
          console.log(`🐛🐛🐛🐛🐛 ALKTemp debugging Log path. Artifacts path already has a name`);
          dir = session_vars.get_artifacts_path_name();
        } else {
          console.log(`🐛🐛🐛🐛🐛 ALKTemp debugging Log path. Making brand new artifacts path`);
          dir = files.make_artifacts_folder();
        }
      }

      if ( !fs.existsSync( dir )) {
        console.log(`🐛🐛🐛🐛🐛 ALKTemp debugging Log path. Artifacts path dir doesn't yet exist`);
      ...
      // Logs should fail silently. With no dir, though, we can't save the error
   ...
   * */
    ...
    if ( do_throw ) {
      // WARNING: If someone catches the error we throw, these strings will get
      // logged. The final resulting logs may become confusing.
      try {
  ...
  debug( debug_opts = {}, ...logs ) {
    ...
    this.internal_log_records.push({ opts: debug_opts, logs });
