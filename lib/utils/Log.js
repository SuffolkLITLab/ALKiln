...
class Log {
  ...
  constructor({ path, context=`` } = {}) {
    ...
    if ( is_new_folder ) {
      // Add `log` to the given context and tell the author what the path is
      if ( context ) { context = context + ` log`; }
      else { context = `log`; }
      this.info({ code: `ALK0214`, context, },
        `Saving test results files at "${ this.path }"`
      );
    }
  ...
  _make_or_get_folder({ dir }) {
    ...
    try {
      ...
      // GitHub actions will pass in a dir (it may already exist, but that's ok)
      if ( dir ) {
        files.make_artifacts_folder( dir );
      // Other processes, like ALKilnInThePlayground, don't have or give a path
      } else {
   ...
   * */
    ...
    if ( do_throw ) {
      ...
      let alkiln_ran_into_error = error;
      if ( error instanceof Error ) {
        throw alkiln_ran_into_error;
      } else {
        ...
        throw new Error( alkiln_ran_into_error );
      }
    }
