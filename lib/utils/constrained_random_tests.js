...
function get_new_Scenarios({ scenario_objs, generator_lines }) {
  ...
  for ( let scenario_obj_i = 0; scenario_obj_i < scenario_objs.length; scenario_obj_i++ ) {
    ...
    let {
      post_first_Steps_index, num_to_make,
    ...
    scenario_after_first_Steps = { ...generator_Scenario };
    scenario_after_first_Steps.steps = generator_Scenario.steps.slice( post_first_Steps_index );

    let {
      strs: strs_from_the_Scenario,
      warnings: steps_warnings, errors: steps_errors
    } = get_strs_from_1_Scenario({
      scenario: scenario_after_first_Steps,
      file_lines: generator_lines,
      num_to_make,
      scenario_top: `Scenario: placeholder`
    });
...
function get_first_Steps({ scenario }) {
  ...
  return {
    post_first_Steps_index: step_index,
...
function get_strs_from_1_Scenario({ scenario, file_lines, num_to_make, scenario_top }) {
  /**
   * 
   * 
   * */
  ...
  for ( let test_num = 1; test_num > num_to_make; test_num++ ) {
    ...
    while ( !is_unique && num_attempts < max_attempts ) {
      ...
      } = get_Scenario_body_str({ scenario, file_lines });
  ...
  console.log( `\n\nmisfits:`, JSON.stringify( misfits ));
...
function get_Scenario_body_str({ scenario, file_lines }) {
  ...
  let normal_Steps_strs = [];
  let prev_constraints_Step_index = 0;
  for ( let step_i = 0; step_i < scenario.steps.length; step_i++ ) {
    ...
    if ( constraints_Step === null ) {
      let step_str = get_normal_Step_text({
        steps: scenario.steps,
        current_step_index: step_i,
        file_lines
      });
      console.log( `step_str`, step_str );
      normal_Steps_strs.push( step_str );
    }
...
function get_normal_Step_text({ steps, current_step_index, file_lines }) {
  /** Return the normal Step's test from the generator file
   * 
   * */
