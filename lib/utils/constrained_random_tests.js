...
function get_generator_AST({ file_text, generator_path }) {
  ...
  if ( !doc_AST.feature ) {
    let feature_error = log.warn({ code: `ALK0254`, context: `generating files`, before: `\n\n`, },
...
function get_new_file_contents({ doc_AST={}, file_text=``, generator_path }) {
  ...
  if ( !scenario_objs[0] ) {
    let syntax_error = log.warn({ code: `ALK0256`, context: `generating files`, before: `\n\n`, },
  ...
  let feature_text = get_Feature_text({
    feature: doc_AST.feature,
    generator_lines
  });

  let { scenarios_strings, errors: scenarios_errors } = get_Scenarios({
    scenario_objs,
    generator_lines
  });
...
function get_Scenarios({ scenario_objs, generator_lines }) {
  /** Return all the strings of the generated Scenarios and any Errors we ran
   *    into.
   * 
   * @param {object} obj - Named arguments
   * @param {object} obj.scenario_objs - Gherkin Scenario objects
  ...
  for ( let scenario_obj_i = 0; scenario_obj_i < scenario_objs.length; scenario_obj_i++ ) {
    ...
    let description = generator_Scenario.name;
    if ( generator_Scenario.tags.length > 0 ) {
      description += `, ${ generator_Scenario.tags.join(` `) }`;
    }
    log.info({ code: `ALK0260`, context: `generating files`, },
      `Generating files for Scenario "${ description }"`
    );

    // Get the values for the various Steps that have to come before any other
    //     Steps. Right now, that's only num_to_generate. Premature? Ask Will.
    let {
      num_to_generate,
      warnings: first_Steps_warnings,
      errors: first_Steps_errors
    } = get_first_Steps({ scenario: generator_Scenario });
    errs_n_warns.add({
      warnings: first_Steps_warnings, errors: first_Steps_errors
    });
    ...
    // all_Scenarios_new_strings.push( generated_Scenarios_str );
  }  // ends for every scenario_container

  // return { scenarios_strings: all_Scenarios_new_strings, errors: errs_n_warns.errors }
  return { scenarios_strings: ['Feature: deving it'], ...errs_n_warns };
}  // Ends get_Scenarios()
...
function get_first_Steps({ scenario }) {
  /**
   * 
  ...
  // let index_of_first_non_first_Step = null;
  let step_index = -1;
  ...
  log.debug({ code: `ALK0262`, context: `generating files` },
    `The final "first Step" index was ${ step_index }`
  );

  return {
    final_index: step_index,
