...
function get_possibly_unique_lines_data({
  ...
  if ( !found_at_least_1_significant_step ) {
    ...
    misfits.add({ errors: [new Error(
      log.error({ code: `ALK0241`, context: `generating files`, },
        `Found 0 constraints Steps for the Scenario at line ${ scenario.location.line }: "${ scenario_description_for_logs({ scenario }) }". ALKiln will make a regular test instead.`
      )
    )] });
  }
...
function get_random_Story_Table({
  ...
  if ( !step.dataTable ) {
    let error = log.error({ code: `ALK0239`, context: `generating files`, },
      `The **table** of the constrained random answers Step at line ${ step.location.line } is missing. The Step: "${ step.text }"`,
    );
    ...
    let error = log.error({ code: `ALK0242`, context: `generating files`, },
      `There is only a header row in the **table** of the constrained random answers Step at line ${ step.location.line }. The body of the table is missing. The Step: "${ step.text }"`,
    );
    misfits.add({ errors: [ new Error( error ) ] });
