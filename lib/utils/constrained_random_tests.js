...
function get_Scenarios({ scenario_objs, generator_lines }) {
  ...
  for ( let scenario_obj_i = 0; scenario_obj_i < scenario_objs.length; scenario_obj_i++ ) {
    ...
    let description = scenario_description({ scenario: generator_Scenario });
    log.info({ code: `ALK0260`, context: `generating files`, },
      `Generating files for Scenario. Description: ${ description }`
    );
    ...
    // // And ALKiln makes 34 tests that get to any of ["end"] when I pick from:
    // // And ALKiln makes 1 test that gets to "end1", "end 2", or "end-3" with:
    // let {
    //   constraints_Step,
...
function get_first_Steps({ scenario }) {
  ...
  let there_were_no_first_steps = step_index === 0;
  if ( there_were_no_first_steps ) {
    let description = scenario_description({ scenario: generator_Scenario });
    misfits.add({ errors: [ log.error({ code: `ALK0263`, context: `generating files` },
  ...
  return {
    post_first_step_index: step_index,
...
function scenario_description({ scenario }) {
   ...
   * @returns {string} - Description of the Scenario
   * */
  let description = `line ${ scenario.line }: "${ scenario.name }"`;
