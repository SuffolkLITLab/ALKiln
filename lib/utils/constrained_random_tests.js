...
function get_new_Scenarios({ scenario_objs, generator_lines }) {
  ...
  for ( let scenario_obj_i = 0; scenario_obj_i < scenario_objs.length; scenario_obj_i++ ) {
    ...
    let {
      post_fore_Steps_index, num_to_make,
    ...
    scenario_after_fore_Steps.steps = generator_Scenario.steps.slice( post_fore_Steps_index );

    let {
      strs: strs_from_the_Scenario,
      warnings: steps_warnings, errors: steps_errors
    } = get_strs_from_1_Scenario({
      scenario: scenario_after_fore_Steps,
      file_lines: generator_lines,
      num_to_make,
      scenario_top: `Scenario: placeholder`,
      body_start_index: post_fore_Steps_index,
    });
...
function get_fore_Steps({ scenario }) {
  ...
  let there_were_no_fore_Steps = step_index === 0;
  ...
  return {
    post_fore_Steps_index: step_index,
...
function get_Scenario_body_str({ scenario, file_lines, start_index }) {
  ...
  let first_normal_lines = file_lines.slice( start_index, scenario.steps[0].location.line );
  let steps_strs = [ ...first_normal_lines ];
  let final_file_line_index_of_previous_Step = 0;  // includes rows
