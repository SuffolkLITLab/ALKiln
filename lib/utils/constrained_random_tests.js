...
module.exports = class TestGenerator {
  constructor() {
    ...
    this.all_Scenario_bodies_map = {};  // map for O^1 adding operation
  }
  ...
  get_strs_from_1_Scenario({
    ...
    for ( let test_num = 1; test_num <= num_to_make; test_num++ ) {
      console.log(`for test`, test_num);
    // console.log(10)

      let new_test_text = ``;
      let definitely_unqiue_scenario_lines = [];

      let { tags: modified_tag_line, tags_file_index } = get_tag_line({
        scenario, generator_lines, test_num
      });
      definitely_unqiue_scenario_lines.push( modified_tag_line );
      // console.log(11)

      let { keyword_line, keyword_file_index } = get_keyword_line({
        scenario, generator_lines, test_num
      });
      // console.log(12)
      ...
      if ( tags_file_index ) {
        // console.log(12.01);
        let { tweens, errors: tags_tween_errors } = misfits.steal_misbehaviors(
          get_tweens({
            end_index_of_prev: tags_file_index,
            start_index_of_next: keyword_file_index,
            generator_lines: generator_lines,
            indexes_to_delete
          })
        );
        // console.log(12.1, tags_tween_errors)
        if ( tags_tween_errors.length > 0 ) { break; }
        // console.log(12.2)
        tags_tweens = tweens;
        // console.log(12.3)
      }
      ...
      definitely_unqiue_scenario_lines.push( ...tags_tweens, keyword_line );
      if ( scenario.steps.length === 0 ) { break; }  // TODO: Can this happen?
      // console.log(14)

      // // Description, comments, and blank lines
      // let first_step_index = as_index( scenario.steps[0].location.line );
      // let { tweens, error: tween_error } = get_tweens({
      //   end_index_of_prev: tags_file_index,
      //   start_index_of_next: keyword_file_index,
      //   generator_lines: generator_lines
      // });
      // misfits.add({ error: tween_error });
      // if ( tween_error ) { break; }
      console.log( `\n\n\n` );

      let prev_saved_file_line_index = keyword_file_index;
      let one_test_body = null;
      ...
      while ( !is_unique && num_attempts < max_attempts ) {
        possibly_not_unique_lines = [];  // reset to build new unique lines
        console.log( `all bodies map:`, this.all_Scenario_bodies_map );

        // console.log( `while is unique`, is_unique );
        // console.log( `while, prev_saved_file_line_index:`, prev_saved_file_line_index );
        ...
        for ( let step of scenario.steps ) {
          ...
          // TODO: Look for end of Scenario/start of next scenario
          // TODO: Warn (error?) when no constraints tables found
          // TODO: Error when no first Steps found
          ...
          let { tweens } = misfits.steal_misbehaviors(
            get_tweens({
              end_index_of_prev: prev_saved_file_line_index,
          ...
          // Modify constraints step and copy it over along with comment lines
          // in a similar way
          let { table: story_table_str } = misfits.add(
            get_random_Story_Table({ ids, step, generator_lines })
          );
          if ( misfits.errors.length > 0 ) { break; }
          attempt_lines.push( story_table_str );

          prev_saved_file_line_index = get_final_file_index_of_constraints_or_fore_Step({ step });
        }  // Ends loop for every step

        /** Get the rest of the lines till the end of the Scenario */
        if ( misfits.errors.length > 0 ) { break; }
        possibly_not_unique_lines.push( ...attempt_lines );
        // Get all the remaining file lines. Alternatively, get all the remaining
        // lines until the next Scenario that are before comments and blank lines
        // though maybe that's not possible given descriptions.
        let next_line = prev_saved_file_line_index + 1;
        let rest_of_lines = generator_lines.slice( next_line );
        possibly_not_unique_lines.push( ...rest_of_lines );

        one_test_body = possibly_not_unique_lines.join(`\n`);
        log.debug({ code: `ALK0000`, context: `generating files` },
          `Generated test:\n${ one_test_body }`
        );
        // If not unique, try again
        if ( this.all_Scenario_bodies_map[ one_test_body ] ) {
          log.debug({ code: `ALK0000`, context: `generating files` },
            `Generated non-unique test on attempt ${ num_attempts }.`
          );
          continue;
        }

        log.debug({ code: `ALK0000`, context: `generating files` },
          `Unique test successful on attempt ${ num_attempts }.`
        );
        // Otherwise, add to the global map
        is_unique = true;
        this.all_Scenario_bodies_map[ one_test_body ] = true;
      }  // ends while generated scenario is not unique

      if ( misfits.errors.length > 0 ) { break; }

      // If text is valid
      if ( possibly_not_unique_lines.length > 0 ) {
        // // Get lines before the next Scenario until all comments/blank lines
        // let final_lines = [];
        // // ...

        new_test_text = [
          ...definitely_unqiue_scenario_lines,
          ...possibly_not_unique_lines,
          // ...final_lines
        ].join(`\n`);

        // TODO: Add a warning tag if necessary (add to the end of the 0th item in
        // the definitely_unique... string)
        new_test_text = handle_warning_tag({
          new_text: new_test_text,
          has_warnings: misfits.warnings.length > 0
        });

        console.log( `new_test_text:`, new_test_text )
        strs_from_this_Scenario.push( new_test_text );
      }
    ...
    // TODO: Replace the number of total tests created placeholder in the
    // Scenario(s) with the actual number (in tags and keyword lines)
    strs_from_this_Scenario = add_total_tests({
      one_generators_strings: strs_from_this_Scenario
    });

    let got_fewer_tests_than_desired = strs_from_this_Scenario.length < num_to_make;
    if ( got_fewer_tests_than_desired ) {
      misfits.add({ warnings:
        [ log.warn({ code: `ALK0243`, context: `generating files`, },
            `ALKiln only managed to make ${ strs_from_this_Scenario.length } unique tests out of the ${ num_to_make } desired for this Scenario:`,
            scenario_description_for_logs({ scenario })
        )]
      });
    }

    console.log(`strs_from_this_Scenario:`, strs_from_this_Scenario);

    return {
      generated_Scenarios_text: strs_from_this_Scenario.join(`\n\n`),
      ...misfits
    };
    ...
    //     if ( this.all_Scenario_bodies_map[ one_test_body ] ) { continue; }
    //     is_unique = true;
    //     this.all_Scenario_bodies_map[ one_test_body ] = true;
...
// console.log(100)
  try {
    ...
    // console.log(105)
    let error = log.error({ code: `ALK0000`, context: `generating files internal`, error: tween_error },
      `ALKiln skipped trying to get generator lines between significant lines`
    );
    // console.log(106)
    errors.push( error );
    // console.log(107)
...
function get_random_Story_Table({
  ids, step, generator_lines, indexes_to_delete
}) {
   ...
   * @param {object} obj.generator_lines - List of lines from the generator file
   * 
   * TODO: Get tweens
   *
   * @returns {object} rtn - Named return values
...
function get_tag_line({ scenario, generator_lines=[], test_num=1 }) { // generator_num }) {
  ...
  return { tags: tags.join(` `), tags_file_index: generator_file_index };
}


function handle_warning_tag({ new_text, has_warnings }) {
  if ( has_warnings ) {
...
function add_warning_tag({ new_text }) {
  return new_text.replace( WARNING_TAG_PLACEHOLDER, ` @alkiln_warning` );
}
...
function add_total_tests({ one_generators_strings }) {
  let with_num_total_tests = [];
  for ( let new_scenario of one_generators_strings ) {
    scenario_with_total = new_scenario.replace(
      TOTAL_TESTS_REGEX,
      one_generators_strings.length
    );
    with_num_total_tests.push( scenario_with_total );
  }
  return with_num_total_tests;
}


function get_keyword_line({ scenario, generator_lines=[], test_num=1 }) {
   ...
   *     we need to generate for this scenario
   * 
   * @returns { keyword_line: str, keyword_file_index: int }
   * */
  ...
  return { keyword_line, keyword_file_index: generator_file_index };
}
