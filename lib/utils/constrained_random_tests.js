...
/**
 * Log codes in this file
 * 
 * "*": tested
 * "^": awkward to test and untested
 * "x": untestable
 * 
 * - [^] ALK 0231 (generator info - starting to generate)
 * - [*] ALK 0236 (generator error - synax error in original)
 * - [*] ALK 0237 (generator warning - no number)
 * - [*] ALK 0238 (generator warning - multiple numbers)
 * - [*] ALK 0239 (generator error - missing table)
 * - [*] ALK 0240 (generator error - multiple constraints Step)
 * - [*] ALK 0241 (generator error - missing constraints Step)
 * - [*] ALK 0242 (generator error - only table header)
 * - [*] ALK 0243 (generator warning - non-unique-ifyable)
 * - [*] ALK 0244 (generator debug - blank answer)
 * - [*] ALK 0245 (generator error - missing ids)
 * - [*] ALK 0246 (generator error - num generated)
 * - [^] ALK 0247 (generator info - num generated )
 * - [x] ALK 0248 (generator error - blank content (unreachable? hopefully))
 * - [x] ALK 0249 (generator error - syntax error in generated (unreachable? hopefully))
 * - [^] ALK 0250 (generator debug - starting to save files)
 * - [^] ALK 0251 (generator debug - deleting previous files)
 * - [^] ALK 0252 (generator debug - finished saving files)
 * */
...
constrained_random_tests.parse_file = function({ file_text, generator_path, logger }) {
   ...
   * @returns {[Error]} rtn.errors - 0 or more Errors.
   *  */
  // Unit tests call this in isolation, so they need to create their own logger.
  if ( logger ) {
    log = logger;
  }
  ...
  if ( generator_parsing_errors.length > 0 || !doc_AST || !doc_AST.feature ) {
    let syntax_error = log.warn({ code: `ALK0236`, context: `generating files`, before: `\n\n`, },
  ...
  if ( !new_contents ) {
    ...
    let error = log.warn({ code: `ALK0248`, context: `generating files`, }, msg );
    return { new_contents, errors: [ new Error( error )] };
  }

  let { errors: generated_parsing_errors } = validate_syntax({ file_text: new_contents });
  if ( generated_parsing_errors.length > 0 ) {
    let syntax_error = log.warn({ code: `ALK0249`, context: `generating files`, before: `\n\n`, },
...
function get_new_file_contents({ doc_AST={}, file_text=`` }) {
  ...
  for ( let scenario_obj_i = 0; scenario_obj_i < scenario_objs.length; scenario_obj_i++ ) {
    ...
    function add_errs_n_warnings({ warnings = [], errors = [] }) {
      /** Accumulates errors and warnings in in-context variables
       * 
       * @param {obj} obj - Named arguments
       * @param {[str]} obj.warnings - Warning messages
       * @param {[Error]} obj.errors - Error objects
       * 
       * @returns {undefined}
       * */
  ...
  if ( all_generated_Scenarios_strings.length === 0 ) {
    let error = log.warn({ code: `ALK0246`, context: `generating files`, }, count_msg );
...
function get_constraints_Step({ generator_Scenario }) {
  ...
  for ( let step of generator_Scenario.steps ) {
    ...
    if ( !possible_constraints_Step.dataTable ) {
      ...
      let error = log.warn({ code: `ALK0239`, context: `generating files`, }, msg );
      ...
      let error = log.warn({ code: `ALK0242`, context: `generating files`, }, msg );
      errors.push( new Error( error ));
  ...
  if ( num_Steps_found > 1 ) {
    ...
    let error = log.warn({ code: `ALK0240`, context: `generating files`, }, msg );
  ...
  if ( num_Steps_found === 0 ) {
    ...
    let error = log.warn({ code: `ALK0241`, context: `generating files`, }, msg );
    return { constraints_Step: null, errors: [ ...errors, new Error( error )] };
  }
...
const GENERATE_REGEX = /generat/i;  // generate/generates/generated/generating
  ...
  if ( number_matcher && number_matcher.length > 1 ) {
    ...
    warnings.push(
      log.warn({ code: `ALK0238`, context: `generating files`, },
        `This constrained random answers Step is ambiguous because it has more than 1 number. ALKiln will use the last number the author gave ("${ num_to_generate }"). The Step:`,
        `\n${ step_text }`
      )
    );
...
function get_final_ids({ step_text }) {
  ...
  if ( ids === null ) {
    ...
    let error = log.warn({ code: `ALK0245`, context: `generating files`, },
      missing_ids_msg
    );
    errors.push( new Error( error ));
  }
...
function pick_random_answer({ answers_str=`` }) {
  ...
  if ( choices.includes(``) ) {
    log.debug({ code: `ALK0244`, context: `generating files`, },
      `One of the constrained random answer choices in a row in this table is "". It could be on purpose or it could be a misplaced separator (";;"). The choices: "${ answers_str }"`
    );
  }
