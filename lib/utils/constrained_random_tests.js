...
function get_strs_from_1_Scenario({
  ...
  for ( let test_num = 1; test_num <= num_to_make; test_num++ ) {
    ...
    while ( !is_unique && num_attempts < max_attempts ) {
      ...
      for ( let step of scenario.steps ) {
        ...
        let is_constraints_step = is_constraints_Step({ step });
      ...
      console.log(`prev_saved_file_line_index:`, prev_saved_file_line_index)
      ...
      console.log( `rest_of_lines`, rest_of_lines );
      // for ( let line_i = prev_saved_file_line_index + 1; line_i < generator_lines.length; line_i++ ) {
      // }
      possibly_not_unique_lines.push( rest_of_lines );

      one_test_body = attempt_lines.join(`\n`);
      log.debug({ code: `ALK0000`, context: `generating files` },
        `Generated test:\n${ one_test_body }`
      );
      // If not unique, try again
      if ( all_Scenario_bodies_map[ one_test_body ] ) {
        log.debug({ code: `ALK0000`, context: `generating files` },
  ...
  // If text is valid
    if ( possibly_not_unique_lines.length > 0 ) {
      /** Get the lines after the end of the final constraints Step (and
       *     possibly before the next Scenario tag or keyword)*/
      let final_lines = [];
      // let last_index = generator_lines.length - 1;
      // for ( let tween of last_saved_file_line_index ) {
      ...
      text = [ ...definitely_unqiue_scenario_lines, ...possibly_not_unique_lines, ...final_lines ].join(`\n`);
      console.log( text )
      strs_from_this_Scenario.push( text );
    }
...
function get_tags({ scenario, generator_lines=[], test_num=1 }) { // generator_num }) {
  ...
  tags.push([ `@scenario${ test_num }_of_${ TOTAL_TESTS_PLACEHOLDER }` ]);
