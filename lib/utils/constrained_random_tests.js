...
let ARTIFACTS_FOLDER = `_alkiln_generated`;
constrained_random_tests.create = function({ generators_prefix, sources_path }) {
...
Scenario: Some other scenario ALKiln random test generator2_scenario1_of_1
  ...
  for ( let generator_path of generator_paths ) {
    ...
    let new_contents = constrained_random_tests.parse_file({ file_text});
    if ( new_contents ) {
  ...
  save_files({ tests_path, artifacts_path, file_strings: new_test_files_strings });

  return new_test_files_strings;
};  // Ends make_random_input_files()


constrained_random_tests.parse_file = function({ file_text }) {
  ...
  for ( let step of generator_Scenario.steps ) {
    ...
    if ( possible_constraints_Step !== null ) {
      ...
      if ( !possible_constraints_Step.dataTable ) {
        console.log(`🔎🤕 ALK0000 (Warning or error?) error: The constrained random input Step answers table is missing. Scenario: TODO: Scenario description and tags (the Step itself may not help much as they're all pretty similar)`);
...
function get_row_as_string({ row }) {
  ...
  let cells = row.cells;
  let values = cells.forEach.map(( cell ) => { return cell.value; });
