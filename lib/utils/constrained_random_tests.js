...
function get_new_Scenarios({ scenario_objs, generator_lines }) {
  ...
  for ( let scenario_obj_i = 0; scenario_obj_i < scenario_objs.length; scenario_obj_i++ ) {
    ...
    // TODO: Get this in the 1 Scenario generator? Because we need to know the
    // file line of the Scenario so we can get all the lines below that to
    // copy into each generated Scenario
    let scenario_top = `Scenario: placeholder`;
...
function get_fore_Steps({ scenario }) {
  ...
  let there_were_no_fore_Steps = last_valid_fore_step_index === 0;
...
function get_strs_from_1_Scenario({
  ...
  for ( num_to_make ) {
    ...
    let modified_tags = file_lines[ scenario_first_line_index ] + `sdkfjlsd`;
    ...
    for ( let tween_line of lines_between_tags_and_keyword ) {
      unqiue_scenario_lines.push( file_lines[ tween_index ] );
    }
    ...
    while ( not_unique && tries < max_attempts ) {
      ...
      for ( let step of steps ) {
        ...
        for ( let tween of lines_between_last_unsaved_line_and_this_one_including_last_unsaved_line ) {
          ...
          possibly_not_unique_lines.push( file_lines[ tween_index ] );
        }
    ...
    // If text is valid
    text = [ ...unqiue_scenario_lines, ...possibly_not_unique_lines ].join(`\n`);
    generated_lines.push( text, `\n` );
  }

  return {
    scenario_text: generated_lines.join(`\n`),
    // and other stuff
  };
