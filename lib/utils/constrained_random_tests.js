...
module.exports = class TestGenerator {
  ...
  get_strs_from_1_Scenario({
    ...
    for ( let test_num = 1; test_num <= num_to_make; test_num++ ) {
      ...
      // Example of unique text: (1 of 2) (TODO: Put that in func signature)
      let { definitely_unqiue_Scenario_lines } = misfits.add(
        get_definitely_unqiue_Scenario_lines({
          scenario, generator_lines, test_num, indexes_to_delete
        })
      );
      ...
      let {
        possibly_unique_lines,
        prev_saved_file_line_index,
        found_significant_step
      } = misfits.add(
        this.get_possibly_unique_lines_data({
          scenario,
          generator_lines,
          indexes_to_delete,
        })
      ...
      let boop = 5;
        ...
        // let possibly_unique_lines = [];
        // // State trackers
        // let prev_saved_file_line_index = keyword_index;
        // let found_significant_step = false;
        // let attempts = { count: 0, max: 10 };
        // let is_unique = false;
        // while ( !is_unique && attempts.count < attempts.max ) {
        //   // reset to build new unique test
        //   possibly_unique_lines = [];
        //   prev_saved_file_line_index = keyword_index;
        ...
        //   // TODO: Add test with no constraints Steps
        //   if ( !found_significant_step ) {
        //     errs_n_warns.add({
        //       error: log.warn({ code: `ALK1000`, context: `generating files` },
        ...
        //   if ( misfits.errors.length > 0 ) { break; }
        //   possibly_unique_lines.push( ...attempt_lines );
        ...
        //   possibly_unique_lines.push( ...rest_of_lines );

        //   // TODO: Instead, if/when we allow multiple Scenarios in one file, look
        //   // for the end of Scenario/start of next Scenario. The problem is a
        //   // docstring that may contain text that looks like tags, "Scenario:",
        //   // etc.
        //   // // Get lines before the next Scenario until all comments/blank lines
        //   // let final_lines = [];
        //   // // ...

        //   let one_test_body = possibly_unique_lines.join(`\n`);
        ...
        //     log.debug({ code: `ALK1001`, context: `generating files` },
        //       `Generated non-unique test on attempt ${ attempts.count }.`
        //     );
        //     continue;
        //   }

        //   log.debug({ code: `ALK1000`, context: `generating files` },
        //     `Unique test successful on attempt ${ attempts.count }.`
        //   );

        //   // Otherwise, add to the global map and move on
        //   is_unique = true;
...
console.log(12, misfits.warnings);
      // TODO: Test warning tags
      new_test_text = handle_generated_Scenario_warning_tag({
      ...
      // If text is valid
      if ( possibly_unique_lines.length > 0 ) {
console.log(14)
    ...
    let new_Scenarios_text = strs_from_this_Scenario.join(`\n`);

    let got_fewer_tests_than_desired = strs_from_this_Scenario.length < num_to_make;
    if ( got_fewer_tests_than_desired ) {
      let too_few_warning = log.warn({ code: `ALK0243`, context: `generating files generator`, },
        `ALKiln could only make ${ strs_from_this_Scenario.length } unique tests out of the ${ num_to_make } desired for this Scenario:`,
        scenario_description_for_logs({ scenario })
      )
      // Discuss: Should we add more structure to errors and warnings to give
      // them context levels (Feature, generator Scenario, generated Scenario)
      misfits.add({ warnings: [ too_few_warning ] });
      new_Scenarios_text = add_warning_Scenario({
        scenarios_str: new_Scenarios_text,
        warnings: [ too_few_warning ]
      });
    }
  ...
  get_possibly_unique_lines_data({
    ...
    let prev_saved_file_line_index = keyword_index;
    let found_significant_step = false;
    function reset_possibly_unique() {
      /** reset to build new unique test */
      possibly_unique_lines = [];
      prev_saved_file_line_index = keyword_index;
      // found_significant_step = false;
    }
    // State trackers important to loop management
    let attempts = { count: 0, max: 10 };
    let is_unique = false;
    while ( !is_unique && attempts.count < attempts.max ) {

      attempts.count++;
      reset_possibly_unique();
      ...
      if ( misfits.errors.length > 0 ) {
        // TODO: Idea for IDE - show only the code that will run for the
        // currently possible state
        reset_possibly_unique();
        break;
      }
      ...
      console.log( `this.all_Scenario_bodies_map[ one_test_body ]:`, this.all_Scenario_bodies_map[ one_test_body ] );
...
function get_definitely_unqiue_Scenario_lines({
   ...
   * 
   * @param {object} obj - 
   * @param {object} obj.scenario - 
   * @param {[str]} obj.generator_lines - 
   * @param {int} obj.test_num - 
   * @param {[int]} obj.indexes_to_delete - 
   * 
   * @returns {object} rtn - Named return values
   * @returns {[str]} rtn.definitely_unqiue_Scenario_lines - As it says
   * @returns {[str]} rtn.warnings - 0 or more warning messages.
   * @returns {[Error]} rtn.errors - 0 or more Errors.
   * */
  let misfits = new Misbehaviors();
  ...
  definitely_unqiue_Scenario_lines.push( get_tag_text({
  ...
  definitely_unqiue_Scenario_lines.push( ...tags_tweens, keyword_text );

  return { definitely_unqiue_Scenario_lines, ...misfits };
};
...
function handle_generated_Scenario_warning_tag({ new_text, has_warnings }) {
  /***/
  if ( has_warnings ) {
...
function add_warning_Scenario({ scenarios_str, warnings }) {
   ...
   * 
   * @param {object} obj - Named arguments
   * @params {str} obj.scenarios_str - The str of the already existing Scenarios
   * @params {[str]} obj.warnings - 0 or more warning messages
   * 
