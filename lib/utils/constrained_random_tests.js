...
module.exports = class TestGenerator {
  ...
  get_new_file_contents({ doc_AST={}, file_text=``, generator_path }) {
    ...
    if ( !new_file_contents ) {
      // TODO: Test for this log code in failure tests
...
function get_number_of_Scenarios_to_generate2({ step, prev_num_to_make }) {
  ...
  if ( num_to_make !== user_num ) {
    // TODO: Write unit test for trying to generate over max random Scenarios
    warnings.push(
...
function get_possibly_unique_lines_data({
  ...
  if ( !found_at_least_1_significant_step ) {
    ...
    let error = log.error({ code: `ALK0254`, context: `generating files`, },
      `ALKiln found 0 valid constraints Steps for the Scenario at line ${ scenario.location.line }: "${ scenario_description_for_logs({ scenario }) }". Check the Step docs at https://assemblyline.suffolklitlab.org/docs/alkiln/writing for correct language and syntax.`
    );
