...
module.exports = class constrained_random_tests {
  constructor() {
  ...
  get_new_file_contents({ doc_AST={}, file_text=``, generator_path }) {
    ...
    let { scenarios_strings, errors: scenarios_errors } = this.get_new_Scenarios({
      scenario_objs,
      generator_lines
    });
  ...
  get_new_Scenarios({ scenario_objs, generator_lines }) {
  ...
  get_strs_from_1_Scenario({
    ...
    // console.log(10)
      ...
      let { tags: modified_tags, tags_file_index } = get_tags({
        scenario, generator_lines, test_num
      });
      definitely_unqiue_scenario_lines.push( modified_tags );
      // console.log(11)
      ...
      let prev_saved_file_line_index = keyword_line;
    ...
    console.log( `all bodies map:`, this.all_Scenario_bodies_map );
        ...
        for ( let step of scenario.steps ) {
          ...
          let constr_start_index = as_index( step.location.line );
          let { tweens } = misfits.steal_misbehaviors(
            get_tweens({
              end_index_of_prev: prev_saved_file_line_index,
              start_index_of_next: constr_start_index,
              generator_lines: generator_lines,
              indexes_to_delete
            })
          );
          ...
          // // TEMP
          // console.log( misfits.errors, tweens );
          // // TEMP
    ...
    // TODO:
    // if ( misfits.errors.length > 0 ) { return; }
...
function is_constraints_Step({ step }) {
  ...
  // Use `.test()` without `g` flag. See
  // https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/RegExp/test#using_test_on_a_regex_with_the_global_flag
  let matches_constraints_Step = CONSTRAIN_REGEX.test( step.text )
                                 && (  RAND_REGEX.test( step.text )
                                       || GENERAT_REGEX.test( step.text )
                                       || MAKE_REGEX.test( step.text )
                                 );
...
function get_tags({ scenario, generator_lines=[], test_num=1 }) { // generator_num }) {
