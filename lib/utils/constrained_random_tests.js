...
Scenario: Some other scenario ALKiln random test generator2_scenario1_of_1
  ...
  for ( let generator_path of generator_paths ) {
    ...
    console.log(`generator_path:`, generator_path);
    let file_text = fs.readFileSync( generator_path, {encoding: `utf8`});
    let new_contents = constrained_random_tests.parse_file({ file_text, generator_path });
    if ( new_contents ) {
...
constrained_random_tests.parse_file = function({ file_text, generator_path }) {
  ...
  if ( generator_parsing_errors.length > 0 || !doc_AST || !doc_AST.feature ) {
    logs.warn({ code: `ALK0225`, context: `generate`, },
      `The constrained random answers generator .feature file "${ generator_path }" had a Gherkin syntax error`,
      ...get_error_stacks({ errors: generator_parsing_errors })
    );
    // TODO: return { new_contents: null, errors: generator_parsing_errors };
    return null;
  }
  ...
  if ( generated_parsing_errors.length > 0 ) {
    logs.warn({ code: `ALK0226`, context: `generate`, },
      `A file generated from "${ generator_path }" had a Gherkin syntax error`,
      ...get_error_stacks({ errors: generated_parsing_errors })
    );
  }

  // TODO: return { new_contents, errors: generated_parsing_errors };
  return new_contents;
}


function get_error_stacks({ errors=[] }) {
...
function save_files({ tests_path, artifacts_path, file_strings }) {
  ...
  //    generated
  console.log(`file_strings.length:`, file_strings.length);
  let count = 1;
