...
module.exports = class TestGenerator {
  ...
  get_strs_from_1_Scenario({
    ...
    for ( let test_num_attempt = 1; test_num_attempt <= num_to_make; test_num_attempt++ ) {
      ...
      let {
        middle_lines, final_significant_Step_index, found_significant_step
      } = misfits.add(
        this.get_new_Scenario_lines({
      ...
      new_test_bodies.push( get_new_test_text({
        starting_lines: definitely_unqiue_Scenario_lines,
        middle_lines,
        final_lines,
        misfits
      }) );
    ...
    if ( got_fewer_tests_than_desired ) {
      let warn_too_few = log.warn({ code: `ALK0243`, context: `generating files generator-scenario-level`, },
        `ALKiln could only make ${ new_test_bodies.length } unique tests out of the ${ num_to_make } desired for this Scenario:`,
      ...
      new_Scenarios_text = add_warning_Scenario({
        scenarios_str: new_Scenarios_text, warnings: [ warn_too_few ]
      });
    }
  ...
  get_new_Scenario_lines({
    ...
    while ( !is_unique && attempts.count < attempts.max ) {
      ...
      if ( misfits.errors.length > 0 ) {
        // Clear data when leaving loop after an error
        clear_inner_state();
        break;
      }
      ...
      if ( this.all_Scenarios_bodies_map[ one_test_body ] ) {
        // In case this is the last loop, delete the duplicate test
        clear_inner_state();
        log.debug({ code: `ALK1001`, context: `generating files` },
          `Test is a duplicate on attempt ${ attempts.count }.`
        );
        continue;
      }

      console.log( `this.all_Scenarios_bodies_map[ one_test_body ]:`, this.all_Scenarios_bodies_map[ one_test_body ] );
...
function get_final_lines({
  ...
  let { tweens, errors, warnings } = get_tweens({
    ...
    start_index_of_next: generator_lines.length - 1,
    generator_lines,
    indexes_to_delete
  });

  let final_index = generator_lines.length - 1;
  if ( !indexes_to_delete.includes( final_index )) {
    tweens.push( generator_lines[ final_index ]);
  }
...
function get_definitely_unqiue_Scenario_lines({
   ...
   * 
   * @returns {object} rtn - Named return values
   * @returns {[str]} rtn.definitely_unqiue_Scenario_lines - As the name says
   * @returns {[str]} rtn.warnings - 0 or more warning messages.
   * @returns {[Error]} rtn.errors - 0 or more Errors.
   * */
  let misfits = new Misbehaviors();
