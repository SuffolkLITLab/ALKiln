...
module.exports = class TestGenerator {
  ...
  get_strs_from_1_Scenario({
    ...
    new_Scenarios_text = add_total_tests({
      related_scenarios_str: new_Scenarios_text,
      num_tests: strs_from_this_Scenario.length
    });
  ...
  get_possibly_unique_lines_data({
    ...
    let prev_saved_file_line_index = keyword_index;
    let found_significant_step = false;
    function reset_for_next_attempt() {
      /** reset to build new unique test */
      attempt_lines = [];
      prev_saved_file_line_index = keyword_index;
    ...
    return {
      possibly_unique_lines: attempt_lines,
      prev_saved_file_line_index,
      found_significant_step,
      ...misfits
    }
...
function add_total_tests({ related_scenarios_str, num_tests }) {
  /** Paste in the total number of tests generated for this generator Scenario.
   * 
   * @param {object} obj - Named arguments
   * @param {str} obj.related_scenarios_str - The combined string of all the
   ...
   * */
  let with_num_total_tests = related_scenarios_str.replaceAll(
