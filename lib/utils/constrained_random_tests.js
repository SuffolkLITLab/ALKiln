...
function get_possibly_unique_lines_data({
  ...
  if ( !found_at_least_1_significant_step ) {
    ...
    // TODO: In this case, only make 1 new Scenario
    misfits.add({
      errors: [ log.error({ code: `ALK0241`, context: `generating files`,
        error: new Error(`Found 0 constraints Steps for the Scenario at line ${ scenario.location.line }: "${ scenario_description_for_logs({ scenario }) }". ALKiln will make a regular test instead.`)
      },)]
    });
  }
...
function get_constraints_Step({ generator_Scenario }) {
  ...
  for ( let step of generator_Scenario.steps ) {
    ...
    if ( !possible_constraints_Step.dataTable ) {
      ...
      let error = log.error({ code: `ALKx0239`, context: `generating files`, }, msg );
      ...
      let error = log.error({ code: `ALKx0242`, context: `generating files`, }, msg );
      errors.push( new Error( error ));
  ...
  if ( num_Steps_found > 1 ) {
    ...
    let error = log.error({ code: `ALKx0240`, context: `generating files`, }, msg );
  ...
  if ( num_Steps_found === 0 ) {
    ...
    let error = log.error({ code: `ALKx0241`, context: `generating files`, }, msg );
    return { constraints_Step: null, errors: [ ...errors, new Error( error )] };
  }
...
function get_random_Story_Table({
  ...
  // TODO: Fix the call to log.error in the rest of the file
  if ( !step.dataTable ) {
    let error = log.error({ code: `ALK0239`, context: `generating files`, 
      error: new Error(`The **table** of the constrained random answers Step at line ${ step.location.line } is missing. The Step: "${ step.text }"`)
    },);
    misfits.add({ errors: [ error ] });

  } else if ( step.dataTable.rows.length === 1 ) {
    let error = log.error({ code: `ALK0242`, context: `generating files`,
      error: new Error(`There is only a header row in the **table** of the constrained random answers Step at line ${ step.location.line }. The body of the table is missing. The Step: "${ step.text }"`)
    },);
    misfits.add({ errors: [ error ] });
...
function get_warnings_Step({ warnings = [], step_text }) {
  ...
  new_warnings_lines.push(`${ indent }  """`, `${ indent }  ━━━━━ Generator file warning or warnings ━━━━━`);
