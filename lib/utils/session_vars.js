...
/**
 * Discuss: consider returning most logs (info, error, warning) so that the
 *    calling functions can use a `Log` to log them properly instead of logging
 *    to the console. Alternatively, move everything in here that `Log` uses
 *    into its own file so that `session_vars` can have its own `Log` instance.
 * */
...
session_vars.get_runtime_config_var = function ( key ) {
  ...
  try {
    ...
    console.debug(`🖊️ ALK0233 NOTE: Skipped getting runtime config var "${ key }".`, get_var_error.stack );
    return null;
...
session_vars.delete_runtime_config_var = function ({ key }) {
  ...
  try {
    ...
    console.debug( `🖊️ ALK0232 NOTE: Skipped deleting runtime config var "${ key }".`, delete_var_error.stack );
    return null
