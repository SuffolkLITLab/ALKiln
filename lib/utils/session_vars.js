...
// Created by interview code when tests through an interview manually on the server
session_vars.get_origin = function () {
  /** Possible values: 'interview', 'github'. In future, maybe 'playground' or 'dashboard'. */
  return process.env._ORIGIN || null;
};
...
session_vars.get_tags = function () { return process.env._TAGS || null; };
...
session_vars.get_project_base_name = function () {
  let name = `testing${ session_vars.get_package_name() }${ session_vars.get_branch_name() }`;
...
// TODO next: Add use of env var in here in case not through GitHub cli_playground
