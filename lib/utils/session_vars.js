...
session_vars.get_dangerous_debug = function () {
  /** Note this will cause failure of tests that reassure the user about
  *   security precautions we took, like not taking a screenshot. */
  return process.env._ALKILN_DANGEROUS_DEBUG || null;
};
...
session_vars.save_repo_folder_name = function (repo_folder_name) {
  ...
  // Get the json, changing it, and re-write the file from scratch
  let json = JSON.parse( fs.readFileSync( runtime_config_path ) );
  json[ repo_folder_key ] = repo_folder_name;
  let file = fs.writeFileSync( runtime_config_path, JSON.stringify( json, null, 2 ) );
...
session_vars.set_sources_paths = function ( paths ) {
  ...
  */
  ...
  console.info(
    `ALKiln info: Did find "sources" folder(s) at "${ existing_paths.join('", "') }".`
  )
...
const save_runtime_config_var = function (key, value) {
  /** Save a key and its value in the runtime config file. */
  ...
  if ( session_vars.get_debug() ) {
    console.log( `ALKiln debug: Stored "${key}" locally as "${value}".` );
  }
  return file;
};  // Ends save_runtime_config_var()

const get_runtime_config_var = function (key) {
