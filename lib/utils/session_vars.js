...
session_vars.set_sources_path = function ( path ) {
  /** Returns whether the given path exists or not. If it does, store
  *   the path to the sources folder. Can be used for getting, e.g
  *   .feature files, files to upload, etc. May be annoying repetition
  ...
  * @returns bool */
  ...
  if ( sources.length > 0 ) {
    session_vars._sources_path = path;
    return true;
  } else {
    session_vars._sources_path = null;
    return false;
  }
};

session_vars.get_sources_path = function () {
  /** Get the "sources" folder path. Should never be undefined during
  *   a run of the test. run_cucumber.js should make sure it has
  *   a default value.
  * 
  * @returns str || null */
  return session_vars._sources_path;
};
