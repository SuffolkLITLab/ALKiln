...
/** WARNING: Avoid importing other project files into here. Every other
 *  file should be able to reference this one without worrying
 *  about circular references.
 * */

/** This file provides user info variable getters and setters.
 *  E.g. server address. It often uses `process.env` vars or
 *  vars that have been built from `process.env` vars. It is
 *  meant to avoid duplication, make testing easier, and leave
 *  room for future manipulation.
 *
 * No other file should need to use `process.env`.
 *
 * We use `.get_foo()` functions partly because it makes the code
 * easier to test.
 * */

/**
 * Discuss: We don't like logging directly to the console in here. Consider
 *    returning most logs (info, error, warning) so that the calling functions
 *    can use a `Log` to log them properly instead of logging to the console.
 *    Alternatively, move everything in here that `Log` uses into its own file
 *    so that `session_vars` can have its own `Log` instance. For example, put
 *    artifacts path name related functions in `Log`. That "get/set/delete"
 *    behavior would be inconsistent with other similar functions. Other places
 *    need to use artifacts path name too.
 * */
...
session_vars.get_max_tests_per_generator_scenario = function() {
  /** Returns the maximum number of tests to generate for any one generator
   *    Scenario.
   * 
   * Assumes an average of 30 secs per test with a max of 20 min. for all tests
   *    for that one generator Scenario.
   * 
   * @returns {int} - Max number of tests for one generator Scenario
   * */
  return process.env.MAX_RANDOM_TESTS_PER_SCENARIO || 40;
}
...
session_vars.save_project_name = function ( project_name ) {
  ...
  * @param project_name { string } - purely alphanumeric string */
  ...
  if ( session_vars.get_debug() ) {
    console.debug( `🐛 ALK0036 DEBUG: Stored name of Project "${ project_name }" locally.` );
  }
  return file;
...
session_vars.save_artifacts_path_name = function ( path_name ) {
  /** Save a key with the name of the current artifacts folder
...
session_vars.delete_artifacts_path_name = function () {
  /** Delete the runtime config value of the artifacts path. */
  return session_vars.delete_runtime_config_var({ key: artifacts_path_key });
};


/** We use `da_install_method` for temp server vs. author's account (Playground)
 *  on author's server. For example, for deciding how to get the url of a
 *  server. See docs on 4 environments. TODO: Change `_method` to `_location`?
 * */
const install_method_key = `da_install_method`;
session_vars.save_install_method = function ( install_method ) {
  /** Save a key with the name of the current artifacts folder in case it
   *  doesn't exist. */
...
session_vars.save_repo_folder_name = function (repo_folder_name) {
  /** Save a key with the name of the main package folder in the repository. For
   *  example, EvictionDefense in the docassemble-EvictionDefense folder. */
...
session_vars.save_runtime_config_var = function ({ key, value }) {
  ...
  * @param {str} obj.key - Key to use to store `value` in runtime config file
  * @param {any} obj.value - Value to store in runtime config object
  * 
  ...
  */
  ...
  if ( session_vars.get_debug() ) {
    console.debug( `🐛 ALK0040 DEBUG: Stored runtime config var "${key}" locally as "${value}".` );
  }
  return file;
...
session_vars.get_runtime_config_var = function ( key ) {
   ...
   * 
   * @param {str} key - Key in runtime config file that contains the desired val
   * 
  ...
  try {
    let json = JSON.parse( fs.readFileSync( runtime_config_path ));
    let value = json[ key ];
    console.log(`🐛🐛🐛🐛🐛 ALK00temp config key/value pair:`, key, value);
    if ( session_vars.get_debug() ) {
      console.debug( `🐛 ALK0041 DEBUG: Runtime config var "${ key }" is "${ value }".` );
    }
    return value;
  } catch ( get_var_error ) {

    console.debug(`🖊️ ALK0273 NOTE: Skipped getting runtime config var "${ key }".`, get_var_error.stack );
    return null;

  }
};  // Ends session_vars.get_runtime_config_var()

session_vars.delete_runtime_config_var = function ({ key }) {
  /** Delete the runtime config var of the given `key`
   * 
   * @param {object} obj - Named arguments
   * @param {str} obj.key - Key in runtime config file object to delete
   * 
   * @returns {null}
   * */
  try {
  ...
  } catch ( delete_var_error ) {

    console.debug( `🖊️ ALK0274 NOTE: Skipped deleting runtime config var "${ key }".`, delete_var_error.stack );
    return null

  }
};  // Ends session_vars.delete_runtime_config_var()


session_vars.set_sources_paths = function ( paths ) {
  /** Set the globally accessible value of the paths to the sources folder.
   *    Saves and returns the argument it was given.
   * 
   * @param paths {[string]} List of paths to the 'sources' folder.
   * 
   * @returns {null | [string]}
   * */
  session_vars._sources_paths = paths;
  return paths || null;
};  // End session_vars.set_sources_paths()

session_vars.get_sources_paths = function () {
  /** Get the previously stored "sources" folder paths.
   * 
   * @returns {null | [string]}
   * */
  return session_vars._sources_paths || null;
};
