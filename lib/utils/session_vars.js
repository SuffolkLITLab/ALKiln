...
session_vars.get_runtime_config_var = function ( key ) {
  ...
  try {
    ...
    console.debug(`🖊️ ALK0229 NOTE: Skipped getting runtime config var "${ key }".`, get_var_error.stack );
    return null;
...
session_vars.delete_runtime_config_var = function ({ key }) {
  ...
  try {
    ...
    console.debug( `🖊️ ALK0041 NOTE: Skipped deleting runtime config var "${ key }".`, delete_var_error.stack );
    return null
