...
function get_new_file_contents({ scenario_objs, file_contents }) {
  ...
  for ( let scenario_obj_i = 0; scenario_obj_i < scenario_objs.length; scenario_obj_i++ ) {
    ...
    * TODO: Add to description text (.name) */
    ...
    /** TODO: Also need the starting line of the first constraints Step found to
     *    properly get the end of the top section of the Scenario. */
    ...
    let scenario_top = get_Scenario_top({ scenario, constraints_Step, file_contents_list });
    let scenario_bottom = get_Scenario_bottom({
      next_Scenario: (scenario_objs[ scenario_obj_i + 1 ] || {}).scenario,
      constraints_Step, file_contents_list
    });
    ...
    let story_table_strs = get_Story_Tables_strings({ num_tests, ids: final_page_ids, step: constraints_Step });
    let story_table = story_table_strs[0];

    // TODO: modify Scenario descriptions
    // get_randomized_Scenarios({ scenario_top, story_tables, scenario_bottom });
    let scenario_str = scenario_top
      + story_table
      + scenario_bottom
      + `\n`;
...
function get_Feature_text({ first_scenario, file_contents_list }) {
...
function get_constraint_Steps({ scenario }) {
   ...
   *
   * TODO: Return first constraints Step we find as well. */
...
function get_Scenario_top({ scenario, constraints_Step, file_contents_list }) {
  /** Return the text that starts the Scenarios we're currently generating. */
  let start_line_num = get_Scenario_start_line_num({ scenario });
  let scenario_start_index = as_index( start_line_num );

  let step_start_index = as_index( constraints_Step.location.line );
  /** TODO: Actually, use the top of the first constraints Step, not just the
   *     constraints Step we're using. */
  let scenario_top_end_index = step_start_index - 1;

  let scenario_top_text = file_contents_list.slice(
    scenario_start_index,
    scenario_top_end_index + 1  // `slice` excludes the terminal number
  ).join(`\n`) + `\n`;
  return scenario_top_text;
};


function get_Scenario_bottom({ next_Scenario, constraints_Step, file_contents_list }) {
  /** Return the text after the constraints Step. */
  let rows = constraints_Step.dataTable.rows;
  let constraints_last_line_index = as_index( rows[ rows.length - 1].location.line );
  let bottom_start_index = constraints_last_line_index + 1;

  if ( !next_Scenario ) {
    // Get lines till the end of the file
    let scenario_bottom_text = file_contents_list.slice(
      bottom_start_index
    ).join(`\n`) + `\n`;
  ...
  let scenario_end_index = get_index_before({ line_num: next_Scenario_start_line });

  let scenario_bottom_text = file_contents_list.slice(
    bottom_start_index,
    scenario_end_index + 1  // `slice` excludes the terminal number
  ).join(`\n`) + `\n`;
...
function get_Story_Tables_strings({ num_tests, ids, step }) {
  ...
  let step_indent = get_loc_indent({ located: step });
  ...
  for ( let table_i = 0; table_i < num_tests; table_i++ ) {
    /** TODO: start table str as [str] and join with `/n`
     * let story_table_rows = [ table_start ]; */
    // Discuss: Should we always use the author's indentation or have consistent
    //    indentation?
    let row_indent = get_loc_indent({ located: step.dataTable.rows[0] });
    let story_table = table_start;
    let is_first_row = true;
    for ( let row of step.dataTable.rows ) {
      // Allow 3 columns.
      // TODO: Warn about too may columns?
      if ( is_first_row ) {
        is_first_row = false;
        story_table += table_header({ row }); // indent: row_indent, num_cols: row.cells.length });
        continue;
      }
      story_table += table_row({ row });
  ...
  console.log(`=== number created/needed`, story_tables.length, num_tests );
...
function table_header({ row }) {
   ...
   * */
  let indent = get_loc_indent({ located: row });
...
function get_loc_indent({ located }) {
