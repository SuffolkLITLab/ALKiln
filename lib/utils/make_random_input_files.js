...
module.exports = function make_random_input_files({ generators_prefix, sources_path }) {
  ...
  for ( let generator_path of generator_paths ) {
    ...
    let gherkinDocument = parser.parse( file_contents );
    let pickles = Gherkin.compile(
      gherkinDocument,
      // Metadata for the parsed objects
      `/docassemble/ALKilnTests/data/sources/alkiln_random.feature`,
      uuidFn
    );

    // console.log(`doc:`, JSON.stringify(gherkinDocument, null, 2));
    // console.log(`pickles:`, JSON.stringify(pickles, null, 2));

    console.log(`doc:`, gherkinDocument, null, 2);
    // console.log(`pickles:`, pickles, null, 2);


    build_file_str({ documentAST: gherkinDocument, uri: `/docassemble/ALKilnTests/data/sources/alkiln_random.feature` });
...
function put_lines_together_in_order({ documentAST={}, uri=`ALK_NOTHING.feature` }) {
  ...
  for ( let comment of comments ) {
    let text = get_regular_line_text({ line_obj: comment });
    file_list[ comment.location.line ] = text;
  }
