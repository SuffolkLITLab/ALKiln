...
generator.make_random_input_files = function({ generators_prefix, sources_path, new_tests_folder }) {
   ...
   *    Scenarios.
   *
   * TODO: Move `new_tests_folder` and `generators_prefix` to a config file. Or
   *    in `session_vars`?
   *
...
Scenario: Some other scenario ALKiln random test generator2_scenario1_of_1
  ...
  for ( let generator_path of generator_paths ) {
    ...
    let new_contents = generator.parse_file({ file_text});
    if ( new_contents ) {
      new_test_files_strings.push( new_contents );
    }
  }

  let sources_tests_path =  `${ sources_path }/${ new_tests_folder }`;
  let artifacts_tests_path = `${ session_vars.get_artifacts_path_name() }/_alkiln_generated`;

  // TODO: Make help_ALKiln_devs_tests_stay_reasonable()
  if ( new_test_files_strings.length > 0 ) {
    // Delete previous files if they exist (like for ALKiln devs)
    fs.rmSync(sources_tests_path, { recursive: true, force: true });
    fs.mkdirSync( sources_tests_path, { recursive: true });
    fs.mkdirSync( artifacts_tests_path, { recursive: true });
  }

  for ( let file_str of new_test_files_strings ) {
    // Add new files!
    let filename = `${ Date.now() }.feature`;
    fs.appendFileSync( `${ sources_tests_path }/${ filename }`, file_str );
    fs.appendFileSync( `${ artifacts_tests_path }/${ filename }`, file_str );
  }

  return new_test_files_strings;
};  // Ends make_random_input_files()


generator.parse_file = function({ file_text }) {
  /** Parse and return one file's contents
   *
   * @param {string} file_text - Full text of a generator feature file
   *
   * @returns {string} - Generated text for the new feature file
   *  */
