 ...
 * TODO: Severe warnings (like "will skip the random test") vs. slight warnings (like "missing number in constraints random test")
 * */
...
function get_new_file_contents({ scenario_objs, file_contents }) {
  ...
  let file_contents_list = file_contents.split(`\n`);

  /** Get text of feature to put at the top of the new file */
  let first_scenario_top = get_Scenario_start_line_num({ scenario: scenario_objs[0].scenario });
  index_of_last_feature_line = first_scenario_top - 2;  // change to 0-indexed for js

  let feature_text = file_contents_list.slice(
    0, index_of_last_feature_line + 1  // excluded from `slice`
  ).join(`\n`);
  ...
  let constrain_regex = /constrain/gi;  // constrained/constraint/constraints
  ...
  for ( let scenario_container of scenario_objs ) {
    ...
    * TODO: Add to description text (.name) */
    ...
    /** For each Scenario section, we need the 0-based indexes of where the
     *    start and end lines would be in the original generator file so that
     *    we can copy the same ones into each individual generated Scenario. */
    let scenario_top_section = {
      start_index: get_Scenario_start_line_num({ scenario }) - 1, // Gherkin is 1-indexed
    };
    let scenario_bottom_section = {};

    let constraints_Step = null;
    let num_constraint_Steps = 0;
    ...
    if ( !constraints_Step ) {
      console.log(`🔎🤕 ALK0000 (Warning or error?) warn: The constraints Step is missing. Scenario: TODO: Scenario description and tags`);
      continue;
    }
    if ( !constraints_Step.dataTable ) {
      console.log(`🔎🤕 ALK0000 (Warning or error?) warn: The constraints Step answers table is missing. Scenario: TODO: Scenario description and tags (the Step itself may not help much as they're all pretty similar)`);
      continue;
    }
    if ( num_constraint_Steps > 1 ) {
      console.log(`🔎🤕 ALK0000 (Warning or error?) warn: Only one constraint Step is allowed in a Scenario. In this Scenario there are ${ num_constraint_Steps }. ALKiln will use the last constraint Step. Scenario: TODO: Scenario description and tags (the Step itself may not help much as they're all pretty similar)`);
    }

    // TODO: Warn about table with just a header row? Or just let that play out?

    let index_of_first_constraints_line_in_file = constraints_Step.location.line - 1;  // Gherkin is 1-indexed
    scenario_top_section.end_index = index_of_first_constraints_line_in_file - 1;
    scenario_top_section.text = file_contents_list.slice(
      scenario_top_section.start_index,
      scenario_top_section.end_index + 1  // excluded from `slice`
    ).join(`\n`) + `\n`;

    let last_step = scenario.steps[ scenario.steps.length - 1 ];
    scenario_bottom_section.end_index = last_step.location.line - 1; // Gherkin is 1-indexed
    let rows = constraints_Step.dataTable.rows;
    scenario_bottom_section.text = file_contents_list.slice(
      rows[ rows.length - 1].location.line,
      // Isn't actually the end (e.g. when there's a table in the next Step). We
      // need to get the starting line of the next
      // scenario if it exists, or the end of the file otherwise.
      scenario_bottom_section.end_index + 1  // excluded from `slice`
    ).join(`\n`) + `\n`;
    ...
    let step_start = ` `.repeat( constraints_Step.location.column - 1 );
    let story_table = `${ step_start }And I get to any of the question ids [${ final_page_ids.join(`, `) }] with this data:\n`;

    // The constraints table
    let row_start = ` `.repeat( constraints_Step.dataTable.rows[0].location.column - 1 );
    let is_first_row = true;
    for ( let row of rows ) {
      // TODO: Warn about too may columns? Allow 3 columns.

      if ( is_first_row ) {
        is_first_row = false;

        story_table += `${ row_start }| var | value |`;
      ...
      // Get values of cells
      let answer = pick_random_answer({ answers_str: row.cells[1].value });
      story_table += `${ row_start }| ${ row.cells[0].value } | ${ answer } |`;
      if ( row.cells.length === 3 ) {
        story_table += ` ${ row.cells[2].value } |`;
      }
      story_table += `\n`;

    }  // ends for every row

    let scenario_str = scenario_top_section.text
      + story_table
      + scenario_bottom_section.text
      + `\n`;
...
function get_number_of_tests({ step_text }) {
  ...
  if ( num_tests_matcher.length > 1 ) {
    ...
    console.log(`🔎 ALK0000 warn: constrained random input test Step gives ${ num_tests_matcher.length } numbers. ALKiln will use the first number. The Step:\n`, step_text);
  }
...
function get_index_from_Gherkin( line_num ) {
  /**
   * Note: Gherkin is 1-indexed, but when we get text from our file we'll be
   *    using js, which is 0-indexed. We need to convert the line number to an
   *    index value.
   * */
  return line_num - 1;
}
