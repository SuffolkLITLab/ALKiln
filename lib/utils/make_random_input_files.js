...
function get_new_file_contents({ scenario_objs, file_contents }) {
  ...
  if ( !first_scenario ) { return null; }  // TODO: Warning? Error?
  let first_scenario_top = get_Scenario_start_line_num({ scenario: first_scenario });
  feature_lines.end = first_scenario_top - 2;  // change to 0-indexed for js

  let feature_text = file_contents_list.slice(
    feature_lines.start,
    feature_lines.end + 1  // excluded from `slice`
  ).join(`\n`);
  ...
  for ( let scenario_container of scenario_objs ) {
    ...
    * TODO: Add to description text (.name) */
    ...
    if ( !constraints_Step ) {
      console.log(`🔎🤕 ALK0000 (Warning or error?) The constraints Step is missing. Scenario: TODO`);
      continue;
    }
...
function get_number_of_tests({ step_text }) {
  ...
  // Too many numbers
  } else if ( num_tests_matcher.length > 1 ) {
