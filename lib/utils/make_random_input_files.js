  ...
  for ( let scenario_container of scenario_objs ) {
    ...
    * TODO: Add to description text (.name) */
    ...
    let scenario_top_section = {
      start: get_Scenario_start_line_num({ scenario }) - 1, // Gherkin is 1-indexed
    };
    ...
    for ( let step of scenario.steps ) {
      ...
      if ( is_constraints_step ) {
        let index_of_line_in_file = step.location.line - 1;  // Gherkin is 1-indexed
        let prev_line_index = index_of_line_in_file - 1;
        let next_line_index = index_of_line_in_file + 1;

        scenario_top_section.end = prev_line_index;
        scenario_bottom_section.start = next_line_index;
    ...
    scenario_top_section.text = file_contents_list.slice(
      scenario_top_section.start,
      scenario_top_section.end + 1  // excluded from `slice`
    ).join(`\n`);

    let last_step = scenario.steps[ scenario.steps.length - 1 ];
    scenario_bottom_section = {
      end: last_step.location.line - 1, // Gherkin is 1-indexed
    };
    scenario_bottom_section.text = file_contents_list.slice(
      scenario_bottom_section.start,
      scenario_bottom_section.end + 1  // excluded from `slice`
    ).join(`\n`);

    // // If it the constraints Step doesn't have any constraints table, error now
    ...
    if ( !constraints_Step ) {
      console.log(`🔎🤕 ALK0000 (Warning or error?) warn: The constraints Step is missing. Scenario: TODO`);
      continue;
    }
    ...
    let story_table_start = `And I get to any of the question ids ["", "", ""] with this data:`;

    // // The constraints table
    // for ( let row of constraints_Step.dataTable.rows ) {
    //   previous_line_num = constraints_Step.dataTable.rows[ rows.length - 1 ].location.line
    //   last_row
    // }
