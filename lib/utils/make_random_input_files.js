...
function get_new_file_contents({ scenario_objs, file_contents }) {
  ...
  for ( let scenario_container of scenario_objs ) {
    ...
    * TODO: Add to description text (.name) */
    ...
    let scenario_bottom = get_Scenario_bottom({ scenario, constraints_Step, file_contents_list });
...
function get_Scenario_bottom({ scenario, constraints_Step, file_contents_list }) {
  ...
  // TODO: Isn't actually the end (e.g. when there's a table in the next
  // Step). We need to get the starting line of the next
  // scenario if it exists, or the end of the file otherwise.
  let scenario_final_Step = scenario.steps[ scenario.steps.length - 1 ];
  let scenario_end_index = as_index( scenario_final_Step.location.line );
