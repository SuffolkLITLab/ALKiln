...
const diff_lib = require("diff");

async function importPdfLib() {
  // pdfjs only supports nodejs in its legacy build
  // https://github.com/mozilla/pdf.js/wiki/Frequently-Asked-Questions/c47467dd05966b686af96f55ce061dab2f96c306
...
let _load_pdf = async function(pdf_path) {
  ...
  let pdfjs_lib = await importPdfLib();
  return await pdfjs_lib.getDocument({
    url: pdf_path,
...
pdf_reader.compare_pdfs = async function(existing_pdf_path, new_pdf_path) {
  ...
  // Assumption: authors won't be changing their PDF templates programmatically;
  // so if the text of the PDFs are different, then it's actually the fields that are different,
  // but they aren't editable / are flattened into the PDF. 
  let change = diff_lib.diffWords(existing_content, new_content);
  ...
  let changeObjs = diff_lib.diffJson(existing_fields, new_fields);
