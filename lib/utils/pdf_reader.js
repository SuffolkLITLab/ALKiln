...
let _get_content = async function(pdf_doc) {
  ...
  for (var i = 1; i <= num_pages; i++) {
    ...
    contents += page_contents.items.map((item) => item.str).join(" ");
  }
  return contents;
};
...
pdf_reader.get_fields = async function(pdf_path) {
   ...
   *   ],
   */
  let doc = await load_pdf(pdf_path);
...
pdf_reader.compare_pdfs = async function(existing_pdf_path, new_pdf_path) {
   ...
   */
  let existing_doc = await load_pdf(existing_pdf_path);
  let new_doc = await load_pdf(new_pdf_path);
