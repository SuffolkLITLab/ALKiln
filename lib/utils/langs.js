...
let session_vars = require('./session_vars');
...
let filepaths = fg.sync( _path.resolve( dir_root, test_glob ) );
let flag = '_lang_';

let langs = session_vars.get_languages();
...
for ( let full_path of filepaths ) {  // Handle old language files before creating new ones.
  ...
  for ( let lang of langs ) {  // Create new language files
    ...
    const lang_escaped = lang.replace(/(\P{L})+/gu, '_');

    // append generated language tags to existing Feature tags if they exist
    const tags_regex = /(?:(?<tags>@.*)\n)?(?=Feature:)/;
    result = result.replace(tags_regex, (_, tags) =>  `${tags ? tags + ' ' : ''}@generated @lang @${ lang_escaped }\n`);
    ...
    let filename = `${ orig_filename }${ flag }${ lang_escaped }.feature`;
