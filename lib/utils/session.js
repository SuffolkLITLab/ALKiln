 ...
 */
 session.validateEnvironment = function() {
...
session.validateEnvironment();

session.get_dev_api_key = function () { return process.env.DOCASSEMBLE_DEVELOPER_API_KEY; };
session.get_da_server_url = function () { return process.env.BASE_URL || process.env.SERVER_URL; };
// session.get_project_name = function () { return session.getProjectName(); };
session.get_debug = function () { return process.env.DEBUG; };
session.get_repo_url = function () { return process.env.REPO_URL; };
...
session.get_branch_name = function () {
...
session.get_project_base_name = function () {
  /* Make an un-incremented project name based on the repo name and branch name. */
  let repo_url_without_trailing_slash = session.get_repo_url().replace(/\/$/, '');
  let repo_name_parts = repo_url_without_trailing_slash.split('/');
  let package_name = repo_name_parts[ repo_name_parts.length - 1 ];

  let name = `testing${ package_name }${ session.get_branch_name() }`;
  let sanitized_name = name.replace( /[^A-Za-z0-9]+/i, '' );
  return sanitized_name;
};

// // ---- Project name ----
// let no_starting_digit = ( 'testing' + session.BRANCH_NAME ).replace( /^[0-9]+/i, '' );
// session.PROJECT_BASE_NAME = no_starting_digit.replace( /[^A-Za-z0-9]+/gi, '' );

// const project_name_file_path = '_al_test_project_name.json';
// session.setProjectName = function ( value ) {
...
// session.getProjectName = function () {
...
//   if ( session.DEBUG ) { console.trace( `Getting project name` ); }
...
//     if ( session.DEBUG ) { console.log( `Project name from file is ${ actual_proj_name }.` ); }
//   } catch ( error ) {}  // Do nothing

//   // If no such file existed or value was invalid, set up the file with a basic name
//   if ( !actual_proj_name ) {
//     actual_proj_name = session.setProjectName( session.PROJECT_BASE_NAME );
//   }

//   if ( session.DEBUG ) { console.log( `Processed project name is "${ actual_proj_name }"` ); }
//   return actual_proj_name;
// };

// session.cleanUpProjectName = function () {
...
// session.getBaseInterviewURL = function () {
...
//   return `${ session.BASE_URL }${ new_session }${ session.PLAYGROUND_ID }${ session.getProjectName() }%3A`;
// };
// // session.BASE_INTERVIEW_URL = session.getBaseInterviewURL();


session.get_languages = function () {
  ...
  return langs;
};  // Ends session.get_languages()


const project_name_file_path = `_al_test_project_name.json`;
session.save_project_name = function ( project_name ) {
  ...
  * @param project_name { string } - purely alphanumeric string */
  ...
  if ( fs.existsSync( project_name_file_path ) ) {
    let old_project_name = session.get_project_name();
...
session.get_project_name = function () {
  /* Get the name of the current project  */
  let project_name =  JSON.parse( fs.readFileSync( project_name_file_path ));
  if ( session.get_debug() ) { console.log( `Project name from file is ${ project_name }.` ); }
  return project_name;
};  // Ends get_project_name()

session.delete_project_name = function () {
  /* Delete the filing storing the name of the docassemble project */
  // Avoid race coditions possibly caused by checking for existence first
  // Delete file if possible
  try {
    fs.unlinkSync( project_name_file_path );
    if ( session.get_debug() ) { console.log( `ALKiln takedown: Deleted stored name of Project.` ); }
    return true;
  } catch ( error ) {
