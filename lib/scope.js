...
module.exports = {
  getTextFieldId: async function getTextFieldId(scope, {ordinal, field_label, type}) {
    /* Returns the id for the specified text field or area.
    * 
    * Note: Arguments never undefined. At the very least, they are null.
    */
  ...
  getSpecifiedChoice: async function getSpecifiedChoice(scope, {ordinal, label_text, group_label_text, roles}) {
    ...
    */
    ...
    if ( group_label_text ) {
      ...
      expect( group_label, `Did not find a "${ group_label_text }" group on this page.` ).to.exist;
    ...
    for (let role of roles) {
      if ( roles.indexOf(role) === 0 ) { role_type += '['; }
      role_type += `@role="${ role }"`;
      if ( roles.indexOf(role) < roles.length - 1 ) { role_type += ' or '; }
      if ( roles.indexOf(role) === roles.length - 1 ) { role_type += ']'; }
    }
    ...
    expect( elem, `Did not find anything where the ${ ordinal } "${ label_text }" ${ roles } should have been.` ).to.exist;
