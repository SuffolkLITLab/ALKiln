...
module.exports = {
  ...
  // three_column_table_warning: async function ( scope, { var_data=null, raw_var_data=null }) {

  // },

  // normalize_table_base: async function ( scope, { var_data=null, raw_var_data=null }) {
  ...
  //    *
  //    * Takes care of the shared funcitonality that needs to happen to all tables.
  //    */
  ...
  //         if (row_raw.length < 2) {
  //           let bad_table_msg = `Your Story Table has less than 2 columns.`
  //               + ` Take another look at Story Table documentation at`
  ...
  getAllFields:  async function ( scope, { html }) {
    ...
    *  TODO: Pass in `$` instead? */
    ...
    if ( !encoded_trigger_var && !$proxies_node.length ) {
      // TODO: Add documentation and then link to it in this message.
      reports.addToReport(scope, {
        type: `warning`,
        value: `ALKiln may not be able to handle index variables or generic objects in this interview. See https://suffolklitlab.org/docassemble-AssemblyLine-documentation/docs/alkiln/#a-missing-trigger-variable.`
      });
      encoded_trigger_var = toBase64( `` );
    }
    ...
    for ( let node of all_nodes ) {
      ...
      if ( $( node ).hasClass( 'daquestionactionbutton' )) {
        ...
        if ( match ) {
          ...
          // This means the first guess of the name should be the encoded name
      ...
      // Defaults
      let proxies = {};
      for ( let index of `xijklmn`.split(``) ) { proxies[index] = ``; }
  ...
  getPossibleDecodedNames: async function ( scope, {
    ...
    */
    ...
    return guesses.concat( scope.decodeFully( scope, {
  ...
  with_and_without_proxies: function ( scope, { var_name, proxies={} }) {
    ...
    // We could combine these regexes, but I'd rather keep it easier to read
    let generic_obj_regex = /^(x)\b/;
    let index_var_regex = /(?<=\[)([ijklmn])(?=\])/g;
    // Replace proxies with their actual values. If the proxies
    // are absent, nothing will happen.
    let new_name = var_name.replace( generic_obj_regex, proxies.x);
    ...
    if ( new_name !== var_name ) { unique_names.push( var_name ); }

    return unique_names;

    // return [
    //   // First, keep `x`/`i` around in case dev is using the old var name version
    //   var_name,
    //   // Then get the var name that has the real values of the proxy vars
    //   // They may be the same. We're avoiding optimizing prematurely.
    //   scope.replace_proxies( scope, { proxies, var_name }),
    // ];
  },  // Ends with_and_without_proxies()

  // replace_proxies: function ( scope, { var_name, proxies }) {
  //   /** Returns new var name when there was a proxy. Otherwise returns
  //    *    an empty string.
  //    *
  //    * @returns str
  //    * */
  ...
  //   // are absent, nothing will happen.
  //   let new_name = var_name.replace( generic_obj_regex, proxies.x);
  //   new_name = var_name.replace( index_var_regex, function(match) {
  //     return proxies[ match ];
  //   });

  //   // On no new name (no proxy) return an empty string
  //   // Maybe instead return an array so that an empty array concats nothing?
  //   return new_name.replace( var_name, `` );

  //   // // If the name changed, proxies were in the names
  //   // let had_proxy = new_name === var_name;
  //   // return { had_proxy, new_name };

  //   // // Maybe just return one value. bool or empty string if didn't have proxy?
  //   // if ( new_name === var_name ) {
  //   //   return '';
  //   // } else {
  //   //   return new_name;
  //   // }
  // },  // Ends replace_proxies()
    ...
    if ( trigger_might_be_needed && uses_proxies && rows_that_match_field_names.length > 0 ) {
      ...
      if ( final_matching_rows.length === 0 ) {
        ...
        let all_trigger_names_conflict_msg = `Is this Story Table missing a row `
          + `or a trigger variable? ${ rows_that_match_field_names.length } `
          + `Story Table row(s) matched a field name, but didn't match a trigger `
          + `variable. The test will have to ignore this field. See `
          + `https://suffolklitlab.org/docassemble-AssemblyLine-documentation/docs/alkiln/#trigger.`
   ...
   */
    ...
    let disabled = await handle.evaluate(( elem )=> {
      ...
      if ( elem && !elem.disabled ) { elem.scrollIntoView(false); }
