...
module.exports = {
  ...
  getPageData:  async function ( scope, { html }) {
    ...
    /*
    Development plan:
    - [ ] Get simplest case of fields (non-showif etc.)
       - [x] Get selectors
    ...
    */
    ...
    // Deal with `option` once inside `select`
    for ( let node of all_nodes ) {
      let field_data = { selector: '', tag: '' };
      let $node = $( node );

      // If signature, all set. TODO: Fill in dummy data.
      if ( $(`.dasignature`).name ) { field_data.selector = `canvas`; break; }

      let encoded_name = '';  // may not need this in the end if we have all the other data
      ...
      // // Otherwise the var_name is contained in their own `name`
      // else {
        encoded_name = $node.attr( `name` );
      // }

      // ====================
      // Selectors
      // ====================
      // Checkbox and radio need their labels for puppeteer to interact with them
      // because puppeteer doesn't interact with hidden elements. That's a bit fiddly.
      // For those, should we just use `.click()` on inputs ourselves? Is there a downside?
      // Making sure they're not disabled first, of course.

      // Construct selectors for all except `option` and `canvas`.
      // Even gets something unique for 'none of the above' checkboxes
      let selector = '';  // Selector of element to manipulate

      field_data.tag = node.name;;
      selector += `${ field_data.tag }[name="${ encoded_name }"]`;
      if ( node.attribs.value ) { selector += `[value="${ node.attribs.value }"]` }

      // Make sure there's only one of these. I believe this
      // will be useful for continued development to ensure we're
      // catching the exact match.
      let self_nodes = $( selector );
      if ( self_nodes.length === 1 ) { field_data.selector = selector; }
      else { console.warn( `${ self_nodes.length } match the '${ selector }' selector. Check up on this with the developers.` ); }

      // ====================
      // Table rows
      // ====================

      // Ex: _field_1. Some `show if` fields have this instead of their var name
      let field_like_names = await scope.getFieldNamesDict( scope, { $: $ });

      let var_names = [], choice_names = [];
      // If danota (da none of the above checkbox or object_radio)
      if ( $node.hasClass('danota-checkbox') || $node.parents(`.da-field-container-datatype-object_radio`).name ) {
        // Must find var name info from siblings' attributes
        ...
        choice_names = [ `al_danota` ];

      // For all other input types
      } else {
        // Might just get var_names, but checkbox will give choice names as well
        ({ var_names, choice_names } = await scope.getPossibleVarNames( scope, {
          encoded: encoded_name,
          field_like_names,
        }));
      }

      // Now some things have their associated choice names.
      if ( choice_names.length === 0 ) {
        // What else needs choice names?
        // radio buttons of all kinds
        // yes/no/maybe buttons, so maybe we have to get them all? And
        //     be flexible later?
        //     TODO: Add docs about values that devs don't set and so don't know what
        //     they are. E.g. yesnomabye = True/False/None
        //     TODO: Maybe make button values into choices for consistency

        // What cares nothing for choice names?
        // Continue buttons, but we need to stay consistent for yesnomaybe buttons

        if ( node.attribs.value ) { choice_names.push( node.attribs.value ); }
      ...
      // What needs a value?
      // Buttons (non-continue) TODO: Switch this up? See above.


      // Create regex for table row


      // Shit, how are we going to make all columns match? Ex: text input field
      // with a default value won't match a row for a text input field that is
      // setting a different value. But sometimes, we do need the 'set_to' value
      // for things like buttons and checkboxes.
      // Do we need to handle all the conditions in here? Dangit.

      
      console.log( var_names, choice_names );


      // Notes
      // Selector type `${ field_data.tag }[name="${ name }"][value="${ value }"]`
      // If `button`
      // `button[name="${ name }"][value="${ set_to/choice }"]`
      // If others (e.g. `input`, `textarea`)
      // `input[name="${ name }"][value="${ set_to/choice }"]`
      // `input[name="${ name }"][value="True"]`

      // Selector type 
      // If B' or R'
      // `input[name="${ name_attr_B }"]`
      // `input[name="${ name_attr_R }"]`
      // If danota (none of the above)
      // `input#${ base64_id_of_first_choice }/..fieldset/input.danota-checkbox`

      // If `data-saveas` show if
      // `*[data-saveas="${ name }"] ${ type }`
      // `*[data-saveas="${ name }"] input[value="${ set_to/choice }"]`
      // `*[data-saveas="${ name }"] input[value="True"]`
      // `*[data-saveas="${ _field_n[B'encodedStr'] }"] input[value]`
      // `*[data-saveas="${ _field_n[R'encodedStr'] }"] input[value]`

      // If nothing else?    
      // `:not(button)[name="${ name }"]`


    }  // ends for every node


    // let inputs = await scope.getFieldData.inputs( scope, { html });

    // field_data = inputs;

    console.log( all_nodes.length );

    page_data.fields = fields_datas;

    return page_data;

    /*
    TODO:
    Docs: Can story tables only have two columns? Not if we
    want to be able to de-select items as well as select them.
    */
  },  // Ends scope.getPageData()

  getFieldNamesDict: async function ( scope, { $ }) {
    /* Some `show if` fields have `_field_n` instead of their var name.
    *  Fortunately, there's a DOM node that contains a map between such a string
    *  and the actual var name. See https://github.com/plocket/docassemble-cucumber/issues/173
    *  Example of final field-like name key/value pair:
  ...
  getPossibleVarNames: async function ( scope, { encoded, field_like_names }) {
    /* Return all the permutations we know of for an encoded var name.
    Should this just handle var_names, or should it handle choice names/values
    to set as well?
    TODO: Check if base64-encoded strings could be indicated somehow in their
    encoded string. Like ending in _base64 or _64

    `encoded` - base64 encoded string of the node
    `field_like_names` example: `_field_0: 'some_checkbox_var_name'` */
    // let names = { table_names: [], var_names: [], choice_names: [] };  // Remove this if not needed
    let names = { var_names: [], choice_names: [] };
    ...
    if ( checkbox_1_match ) {
      ...
      names.choice_names.push( box_layer_1_decoded );
      // names.var_names.push( `${ var_name }['${ box_layer_1_decoded }']` );
      ...
      try {
        ...
        names.choice_names.push( box_layer_2_decoded );
        // names.var_names.push( `${ var_name }['${ box_layer_2_decoded }']` );
      } catch ( err ) {}  // Do nothing
    ...
    // Ex: checkboxes_choices[R'Y2hlY2tib3hfb3RoZXJfb3B0XzI']
      ...
      try {
        // The part inside `[R'encoded']` could need one more decoding
        // It may have invalid chars if it's not a one-word var name, though, like '_'
        let box_layer_2_decoded = await scope.fromBase64( scope, { base64_str: box_layer_1_decoded });
        names.choice_names.push( box_layer_2_decoded );
        // names.var_names.push( `${ var_name }['${ box_layer_2_decoded }']` );
    ...
    // May only need table names. Keeping the rest until development is more... developed
    // return table_names;
    return names;
  ...
  getFieldData: {
    inputs: async function ( scope, { html }) {
      const $ = cheerio.load( html );
      let inputs_datas = [];
      ...
      for ( let input of inputs ) {
        let var_name = '';
        let saveas = input.closest( `div[data-saveas]` ).attr( `data-saveas` );
        if ( saveas ) { var_name = saveas }
        else {
          var_name = 'foo';
        }
        // $(input).attr('name');

        inputs_datas.push( input );
      }

      return inputs_datas;
    },
    buttons: async function ( scope, { html }) {
      const $ = cheerio.load( html );

    },
    dropdowns: async function ( scope, { html }) {
      const $ = cheerio.load( html );

    },
    textareas: async function ( scope, { html }) {
      const $ = cheerio.load( html );

    }, 
    canvases: async function ( scope, { html }) {
      const $ = cheerio.load( html );
  ...
  // getFieldsData?
  xgetPageData: async function ( scope, { html }) {
    /* Given HTML string of a page, returns a list of all the fields in it
    as some kind of sophisticated data? With their selectors, for example.
    TODO: Should this also return the sought variable for proxy stuff? */

    // !!!TODO: Check out a bunch of (new?) hidden input fields at the bottom
    // of the page that list various things aobut the fields on the page.
    // For example:
    // _event IS THE SOUGHT VAR!
    // _varnames to get encoded '_field_n' and the matching varname for various
    // showif variables. That seems useful, but I'm not yet sure how.
    // csrf_token? What is that?
    // _checkboxes - encoded (default?) values of encoded checkbox variable names
    // _datatypes (less likely to be useful) Datatypes of field answers. radio
    // button multi-options are 'text', though, while checkbox multi-options are 'checkboxes'

    const $ = cheerio.load( html );

    // All the different types of fields
    // buttons, inputs, textareas, inputs of all kinds, dropdowns. Is there any other possibility?

    // // The containers of each field
    // let form_groups = $(`.form-group`);
    // // Build the data
    // let fields = [];
    // for ( let group_i = 0; group_i < form_groups.length; group_i++ ) {
    //   // Field is not 
    //   let group = form_groups[ group_i ];
    //   let fields_datas = [];
    //   let selector = '';

    //   // 

    //   // Get the encoded var_name
    //   // For `show if` groups, the parents have the encoded var_name
    //   let var_name = $( group ).parent().attr( `div[data-saveas]` );
    //   if ( var_name ) {
    //     selector += `*[data-saveas="${ saveas }"] `;
    //   } else {
    //     // Otherwise the var_name is contained in their own `name`
    //     children = $( group ).find( 'input' );
    //     for ( let child of children ) {
    //       selector += $( child ).attr()
    //     }

    //     selector = var_name.length;
    //     // selector = $(var_name[0]).attr('name');
    //     // selector += `*[name="${ var_name }"] `;
    //   }
    ...
    //   fields = fields.concat( fields_datas );
    // }
  ...
  getMatchingRows: async function ( scope, { story_table, html_data }) {
