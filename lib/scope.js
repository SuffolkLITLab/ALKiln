...
module.exports = {
  ...
  load: async function ( scope, file_name ) {
    ...
    *    use scope.timeout? */
    ...
    if ( load_result.error ) {
      ...
      reports.addToReport(scope, { type: `row info`, code: `ALK0117`, value: `Successfully found the interview` });
    }
  ...
  set_interview_session_data: function ( scope, { vars = null }) {
    ...
    if ( vars && vars.i ) {
      ...
      if ( !scenario.interviews[ vars.uid ] ) {
        ...
        log.debug({ code: `ALK0185`, context: `session_id_etc` },
          `Saved new interview data. File path: ${ vars.i }. Session id: ${ vars.uid }.`
        );
      } else {
        log.debug({ code: `ALK0186`, context: `session_id_etc` },
          `Interview data already existed. File path: ${ vars.i }. Session id: ${ vars.uid }.`
        );
      }
      return vars;
    } else {
      log.debug({ code: `ALK0187`, context: `session_id_etc` },
        `Found no interview data.`
      );
      return null;
    }
  ...
  normalizeTable: async function ( scope, { var_data=null, raw_var_data=null }) {
    ...
    log.debug({ code: `ALK0122`, context: `table`, },
      `scope.normalizedTable():`, supported_table
    );
  ...
  getAllFields:  async function ( scope, { html }) {
    ...
    *  TODO: Pass in `$` instead? */
    ...
    log.debug({ code: `ALK0124`, context: `page fields`, },
      fields
    );
    return fields;
  ...
  getPossibleTableRowMatchers: async function ( scope, { var_names, values }) {
    ...
    * variable name and value. */
    log.debug({ code: `ALK0126`, context: `matching`, },
      `scope.getPossibleTableRowMatchers() var_names:`, var_names, `values:`, values
    );
  ...
  getMatchingRows: async function ( scope, { field, var_data }) {
    ...
    */
    ...
    log.debug({ code: `ALK0131`, context: `matching`, },
      `final_matching_rows:`, final_matching_rows
    );
    if ( final_matching_rows.length == 0) {
      log.debug({ code: `ALK0132`, context: `matching`, },
        `0 matching rows. Field:`, field
      );
    }
  ...
  get_handle_from_field: async function ( scope, { field }) {
    ...
    log.debug({ code: `ALK0133`, },
      `scope.get_handle_from_field() field.selector:`, field.selector
    );
  ...
  get_page_id: async function ( scope ) {
    /** Return the current page's docassemble `question` block ID or `null`
     *
     * Note: The ID is already sanitized by docassemble.
     *
     * @param {object} scope - Tests state object
     * @param {object} scope.page - puppeteerjs page object
     *
     * @returns {null | string}
     * */
    let question_id_match = await scope.page.$eval(`body`, function (elem) {
      return elem.className.match(/question-([^ ]+)/);
    });
    if ( question_id_match !== null ) {
      return question_id_match[ 1 ];
    }
    return null;
  },

  to_docassemble_sanitized_id: function ( risky_id ) {
    /** Return a version of `risky_id` that's sanitized the same way docassemble
     *    sanitizes its IDs to turn them into HTML class names in python:
     *      re.sub(r'[^A-Za-z0-9]+', '-', interview_status.question.id.lower())
     *    (reference needed)
     *
     * @param {string} risky_id - The un-sanitized id
     *
     * @returns {string} - Sanitized id
     * */
    return risky_id.toLowerCase().replace(/[^A-Za-z0-9]+/g, '-');
  },
  ...
  steps: {
    ...
    sign_in: async function ( scope, {
      ...
      */
      await scope.take_a_screenshot(scope, {path:`${scope.paths.artifacts}/Temp-1.jpg`});
      ...
      await scope.take_a_screenshot(scope, {path:`${scope.paths.artifacts}/Temp-2.jpg`});
      ...
      try {
        ...
        await scope.take_a_screenshot(scope, {path:`${scope.paths.artifacts}/Temp-3.jpg`});
        ...
        if ( error.name === `TimeoutError` ) {
          ...
          // Throw any non-timeout error
          // console.log(`🖊️ 111`);
          await scope.take_a_screenshot(scope, {path:`./_alkiln_temp/Temp-4.jpg`});
      ...
      // console.log(`🖊️ 112`);
      await scope.take_a_screenshot(scope, {path:`${scope.paths.artifacts}/Temp-4.jpg`});
      ...
      await scope.guard_against_missing_tap_element(scope, { elem });
      // console.log(`🖊️ 113`);
      await scope.take_a_screenshot(scope, {path:`${scope.paths.artifacts}/Temp-5.jpg`});

      // Submit and see what happens
      let winner = await scope.steps.race_sign_in_navigation( scope, { elem, login_url });
      // console.log(`🖊️ 113.5 ${ winner }`);
      // await scope.take_a_screenshot(scope, {path:`${scope.paths.artifacts}/Temp-6.jpg`});
      // console.log(`🖊️ 113.6 ${ winner }`);
      ...
      // console.log(`🖊️ 114`);
      // await scope.take_a_screenshot(scope, {path:`${scope.paths.artifacts}/Temp-7.jpg`});
      // if ( error_msg ) {  // undefined sometimes now
      //   return [ error_msg ];
      // } else {
      //   return [];
      // }
    ...
    race_sign_in_navigation: async function ( scope, { elem, login_url='unknown' }) {
      ...
      await scope.take_a_screenshot(scope, {path:`${scope.paths.artifacts}/Temp2-0.jpg`});
      ...
      // Redirect
      let redirect_promise = scope.page.waitForResponse(async ( response ) => {
        // console.log(`⏳ 300`);
        return response.status() === 302;
      }, { signal: controller.signal })
      .catch(async ( catch_info ) => {
        // console.log(`⏳ 201`);
        // await scope.take_a_screenshot(scope, {path:`${scope.paths.artifacts}/Temp2-1.jpg`});
        log.debug({ code: `ALK0204`, context: `nav`, },
          `302 sign-in wait catch error:`, catch_info.name, catch_info,
        );
      });

      // Invalid credentials, no navigation, no sign in
      let wrong_sign_in_promise = scope.page.waitForResponse(async ( response ) => {
        // console.log(`⏳ 301`);
      ...
      .catch(async ( catch_info ) => {
        // errors?
        // console.log(`⏳ 202`);
        // await scope.take_a_screenshot(scope, {path:`${scope.paths.artifacts}/Temp2-2.jpg`});
        log.debug({ code: `ALK0205`, context: `nav`, },
          `200s sign-in wait catch error:`, catch_info.name, catch_info,
        );
      });

      // System error
      let error_promise = scope.page.waitForResponse(async ( response ) => {
        // console.log(`⏳ 302`);
        return response.status() >= 500 && response.status() < 600;
      }, { signal: controller.signal })
      .catch(async ( catch_info ) => {
        // console.log(`⏳ 203`);
        await scope.take_a_screenshot(scope, {path:`${scope.paths.artifacts}/Temp2-3.jpg`});
        log.debug({ code: `ALK0206`, context: `nav`, },
          `500s sign-in wait catch error:`, catch_info.name, catch_info,
        );
      });

      // Duh, the other catches were triggered because those promises were aborted

      let click_promise = elem.click();  // MUST complete
      let nav_promise = scope.nav_race(scope, {}).result;
      let pic_promise = true; // scope.take_a_screenshot(scope, {path:`${scope.paths.artifacts}/Temp2-10.jpg`});
      // console.log(`⏳ 204`);
      const winner = await Promise.race([
        Promise.all([ `success`, click_promise, pic_promise, redirect_promise ]),
        Promise.all([ `failure`, click_promise, pic_promise, wrong_sign_in_promise ]),
        Promise.all([ `error`, click_promise, pic_promise, error_promise ]),
      ]).catch(async ( error ) => {
        // console.log(`⏳ 205`);
        // await scope.take_a_screenshot(scope, {path:`${scope.paths.artifacts}/Temp2-4.jpg`});
        let error_msg = reports.addToReport( scope, {
          type: `error`, code: `ALK0207`,
          value: `Unknown error waiting for results during sign in at ${ login_url }.`
        });
        // console.log(`⏳ 205.1`, error);
        throw error;
      });

      // console.log(`⏳ 206`);
      // // await waitForTimeout(500);
      // await scope.take_a_screenshot(scope, {path:`${scope.paths.artifacts}/Temp2-5.jpg`});

      // Clean up unresolved promises
      controller.abort();
      // console.log(`⏳ 207`);
      await nav_promise;
      await scope.take_a_screenshot(scope, {path:`${scope.paths.artifacts}/Temp3-1.jpg`});
  ...
  set_random_input_for: {
    ...
    buttons: async function (scope, { fields }) {
      ...
      if (field == null) {
        ...
        log.debug({ code: `ALK0164`, context: `random`, },
          `A random button field choice was null. These were the fields to choose from:`,
          fields
        );
        return;
      }
