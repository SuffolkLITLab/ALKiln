...
module.exports = {
  ...
  addToReport: async function( scope, { type, value }) {
    ...
    * keys should correspond to a list containing the relevant data. */
    ...
    if ( !scope.paths ) {
      ...
      // abstracted?
      let date = await scope.date_for_filename();
  ...
  getSafeScenarioFilename: async function( scope, options={prefix: ''} ) {
    ...
    */
    ...
    let prefix_with_possible_dash = `${ prefix }`;
    // As long as it's not an empty string, add a separator between it and the rest of the name
    if ( prefix ) {
      prefix_with_possible_dash = `${ prefix }-`;
    }
    let name = `${ prefix_with_possible_dash }${ scope.base_filename }`
    if ( scope.page && scope.page_id ) {
      name += `-${ scope.page_id }`;
    }
    let date = await scope.date_for_filename();
    name += `-${ date }`;
  ...
  date_for_filename: async function() {
  ...
  savePageJSONData: async function( scope ) {
    ...
    */
    // Get a unique json file path
    let date = await scope.date_for_filename();
    let json_path = path.join( scope.paths.scenario, `json_for-${ scope.page_id }-${ date }.json` );
    // Get the data
  ...
  detectDownloadComplete: async function detectDownloadComplete(scope, endTime) {
    /* Timeout after a bit less than the global permitted timeout
    * 
    * TODO: Wait for download to complete. See https://stackoverflow.com/a/56951024
    * and https://github.com/puppeteer/puppeteer/issues/4676 and
    * https://github.com/puppeteer/puppeteer/issues/299 and
    * https://github.com/SuffolkLITLab/ALKiln/issues/492
    */

    await scope.page.waitForTimeout(scope.timeout - 200);
  ...
  steps: {
    ...
    screenshot: async ( scope, name ) => {
      ...
      * unique as the filename will be made unique. */
      ...
      let page_id = `${ id }-`;
      if ( !name ) {
        name = `screenshot-${ page_id }${ date }`;
      } else {
        name = `screenshot-${ name }-${ page_id }${ date }`;
      }
    ...
    download: async ( scope, filename ) => {
      ...
      * 
      * TODO: Wait for download to complete. See notes in
