...
module.exports = {
  ...
  uploadFiles: async function ( scope, { handle, answer }) {
    ...
    */
    ...
    // Tests should be one place: docassemble/*/data/sources, where * is the only directory in `docassemble`.
    ...
    try {
      ...
      if (dirs.length < 1) {
        let msg = `You can store files to upload in 'docassemble/*/data/source'. These tests could not find a directory like that.`;
        await scope.addToReport( scope, { type: `warning`, value: msg});
        throw Error(msg);  // Most people will want to know the sources folder path;
      }
      if (dirs.length != 1) {
        await scope.addToReport( scope, { type: `warning`, value: `You have multiple directories in 'docassemble' folder (${ dirs }). These tests will use the first directory (${dirs[0]}).` });
      }
      pkg_path = `${docassemble_pkg_path}/${dirs[0]}`;
    } catch ( error_da ) {
      await scope.addToReport( scope, { type: `warning`, value: `You can store files to upload in 'docassemble/*/data/source'. These tests could not find a directory like that.` });
    ...
    try {
      ...
      await scope.addToReport( scope, { type: `warning`, value: `You can store files to upload in '${dir}'. These tests could not find that directory.` });
