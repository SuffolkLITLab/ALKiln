...
module.exports = {
  ...
  // processVarTable: async function ( scope, { how_many, var_data }) {
  processVar: async function ( scope, { how_many, var_data, id }) {
    ...
    * reaching the terminal page. Important to avoid recursion because of async ops. */
    ...
    // let total_vars = var_data.length

    // // TODO: Does it hang for the full timeout if there's a system or user error?
    ...
    // // Until we reach the final page or run out of vars
    // while ( !id_matches || var_data.length > 0 ) {
      ...
      // Complete all the loops so we can collect all remaining vars
      ...
      for ( let row of var_data ) {
        ...
        if ( handle ) {
          ...
          // Can't try continuing to the next page in here - if there are
          // three questions we want to answer, but only two are required,
      ...
      if ( !a_var_was_found ) {
        // if no given vars are on this page
        // we should be done with this page and able to continue

        // Should not hit continue after tapping a button? No, because then would have
        // found a var and would not be in here.
        await scope.continue( scope );
        let { error_found, error_handlers } = await scope.checkForErrors( scope );
        ...
        // Otherwise check if it was user error message
        ...
        // if ( error_found && error_handlers.system_error ) {

        //   let message = `There was a system error. See the screenshot. The question id was "${ id }". ` +
        //     `The remaining variables are ${ JSON.stringify(remaining_vars) }.`
        //     // Better to error in here closer to where the relevant code is?
        //     throw new ReferenceError( message );

        // } else if ( error_found ) {
        //   // All other errors are the ones shown to the user as feedback
        //   let message = `The user got an error. See the screenshot. The question id was "${ id }". ` +
        //     `The remaining variables are ${ JSON.stringify(remaining_vars) }.`
        //     // Better to error in here closer to where the relevant code is?
        //     throw new ReferenceError( message );
        // }
        ...
        //  else {
        //   // If successfully continue, get new page data
        //   ({ question_has_id, id, id_matches } = await scope.examinePageID( scope, scope.final_question_id ));
        // }

        // try {
        //   // TODO: Detect user and system errors? What happens if we don't?
        //   // Will this ever error? Do we need to do this a different way?
        //   await scope.continue( scope );
        //   // If successfully continue, get new page data
        //   ({ question_has_id, id, id_matches } = await scope.examinePageID( scope, scope.final_question_id ));

        // } catch (err) {
        //   // If can't continue, are not on the final page, and we don't have more vars to set :O
        //   let message = `The question id is only "${ id }", but no more variables from` +
        //     ` the table were found on this page. The remaining variables are ${ JSON.stringify(remaining_vars) }.`
        //   // Better to error in here closer to where the relevant code is?
        //   throw new ReferenceError( message );

        // }  // ends try scope.continue()
    ...
    // // ===================
    // // REACHED THE TARGET PAGE AND/OR NO VARS ARE LEFT TO SET!
    // // ===================
    // // if reached the target page and all vars were used, categorical win
    // if ( id_matches && var_data.length === 0 ) { return true; }

    // // If reached the target page, needed only some vars, and used up at least some vars, still a win
    // if ( id_matches && how_many !== 'all' && var_data.length < total_vars ) { return true; }

    // // If reached the target page and needed to use up all the vars, but didn't
    // if ( id_matches && how_many === 'all' && var_data.length > 0 ) {
    //   let message = `When the question id "${ scope.final_question_id }" was reached ` +
    //     `there were some variables left over: ${ JSON.stringify(remaining_vars) }`
    //   // Better to error in here closer to where the relevant code is?
    //   throw new ReferenceError( message );
    // }

    // // If did not use _any_ vars by the time we reached target page
    // if ( id_matches && var_data.length === total_vars ) {
    //   let message = `No variables were used, but the question id "${ scope.final_question_id }"` +
    //     ` was reached. The variables: ${ JSON.stringify(remaining_vars) }`
    //   // Better to error in here closer to where the relevant code is?
    //   throw new ReferenceError( message );
    // }

    // // if used up all the vars, but fell short of the target page
    // if ( var_data.length === 0 && !id_matches ) {
    //   let message = `We needed to get to the question id "${ scope.final_question_id }",` +
    //     ` but all the variables have been used and the question id is only "${ id }".`
    //   // Better to error in here closer to where the relevant code is?
    //   throw new ReferenceError( message );
    // }

    // console.warn( 'Used up all vars and/or reached target page, but did not match any given conditions.' );

    // return 'How did we get here?';
  // },  // Ends scope.processVarTable()
  },  // Ends scope.processVar()

  checkForErrors: async function ( scope ) {
    /* Returns whether any system or user elements were found and, if so,
    * returns those elements any elements that indicate user or system errors and whether any of
    * them appeared at all. The error elements are probably exclusive, but
    * we're not sure. */
