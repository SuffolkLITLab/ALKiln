...
module.exports = {
  ...
  getField: async function ( scope, { var_name, choice_name, set_to }) {
    ...
    let all_attempts = [
      await scope.page.$$(`#dasigcanvas`),  // signature
    ...
    * reaching the terminal page. Important to avoid recursion because of async ops. */
    ...
    // while ( !id_matches || var_data.length > 0 ) {
      ...
      for ( let row of var_data ) {
        ...
        // Wait extra long for a signature canvas to load. Use `set_to`? Or use question id?
        if ( set_to === '/sign' || set_to === '/signature' ) { await scope.page.waitFor( 500 ); }
