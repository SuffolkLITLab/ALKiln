...
let display_var_value = function(row) {
  if ( is_secret_var(row) ) {
    return '${' + row.flags.secret_name +'}';
  }
  return row.value;
};

let is_secret_var = function(row) {
  return row.flags !== undefined && row.flags.secret_name !== undefined;
}
...
module.exports = {
  ...
  convertToOriginalStoryTableRow: async function( scope, { row_data }) {
    ...
    * be pretty confusing to most devs. */
    ...
    if (is_secret_var(original)) {
      value_str = display_var_value(original);
    }
  ...
  normalizeTable: async function ( scope, { var_data, from_story_table=true }) {
    ...
    * Table must have headers. Arrays are not supported. */
    ...
    if ( session_vars.get_debug() ) { console.log( 'scope.normalizedTable():\n', JSON.stringify( supported_table )); }
    ...
    if ( ensure_all_vars_used ) {
      ...
      for ( let row of var_data ) {
        if ( row.times_used === 0 ) {
          ...
          let reported_val = display_var_value(original_row); 
          let msg = `Did not find a field on this page for the variable \`${ original_row.var }\` that could be set to \`${ reported_val }\``;
          await scope.addToReport(scope, { type: `warning`, value: msg });
        }
      }
  ...
  getThereIsAnotherValue: async function ( scope, { row, from_story_table=true }) {
    ...
    */
    let value = row.value;
    let reported_value = display_var_value(row); 
    ...
    if ( value.match( /(false|no|unchecked|uncheck|deselected|deselect)/i )) {
      ...
      if ( from_story_table ) {
        ...
        if ( row.var.match( /\.there_is_another$/ ) ) {
          ...
          await scope.addToReport( scope, { type: `warning`, value: `${ reported_value } is not a valid value for ${ row.var } here. This test will default to \`False\` to avoid problems.` });
        }
        return `False`;
      } else { return `True`; }
  ...
  steps: {
    ...
    set_secret_var: async ( scope, var_name, set_to_env_name ) => {
      ...
      let set_to = process.env[set_to_env_name];
      if ( set_to === undefined ) {
        let set_to_env_msg = `The GitHub SECRET "${ set_to_env_name }" doesn't exist. Check for a typo and check your workflow file.`
        await scope.addToReport( scope, { type: `error`, value: set_to_env_msg })
      }
