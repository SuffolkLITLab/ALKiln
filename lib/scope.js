  ...
  getMatchingRows: async function ( scope, { story_table, page_data }) {
    ...
    */
      ...
      for ( let guess of field.guesses ) {  // usually only one or two
        for ( let table_row of story_table ) {
          ...
          if ( does_match ) {
            ...
            match.table_rows.push( table_row );
            ...
            if ( page_data.uses_proxies && match.table_rows.length > 1 ) {
              // Only keep rows with matching sought var
              match.table_rows = match.table_rows.filter(function ( row ) {
    ...
    for ( let field of matches_by_field ) {
      if ( field.table_rows.length > 1 ) {
        let msg = `Warning: there are multiple matches to a field. Here is the data: ${JSON.stringify( field )}`
        await scope.addToReport(scope, { key: 'ids', value: msg });
      }
    }

    if ( env_vars.DEBUG ) { console.log( JSON.stringify( matches_by_field )); }
  ...
  processVar: async function ( scope, { var_data, id }) {
    ...
    * continuing to the next page. Important to avoid recursion because of async ops. */
    ...
    for ( let row of matches ) {
      ...
      // Maybe don't need type and tag until `setVariable`. Except button is an issue
      // We need to hit buttons last.
      // [{ selector, row, type, tag }]?
      // Also, what about the fact that there could be mutliple matches of fields/selectors.
      // What do we do with those? Those are the ones that have been 'decoded' when
      // they didn't need to be decoded. We could accidentally unset fields if we're not careful
      // with that.

      // Move bellow to set var
      ...
      // move mostly to get matches? (other than handle)
    ...
    while ( prev_length > fields_to_set.length && fields_to_set.length > 0 ) {
      ...
      for ( let field_i = 0; field_i < fields_to_set.length; field_i++ ) {
        ...
        let not_handled = await scope.setVariable( scope, { handle, row });
        if ( not_handled ) {
