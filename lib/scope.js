...
module.exports = {
  ...
  getPageData:  async function ( scope, { html }) {
    ...
    */
    ...
    for ( let node of all_nodes ) {
      // This seems like too much data to have to pass down. Surely there can be less.
      let field_data = await scope.getFieldData( scope, {
        $,
        node,
        sought_var: page_data.sought_var,
        field_like_names
      });
  ...
  getFieldData:  async function ( scope, { $, node, sought_var, field_like_names }) {
    /* Get the page data for any specific field. */
    ...
    if ( $(`.dasignature`).name ) {
      ...
      field_data.rows = [{ var_name: page_data.sought_var, value: '', checked: '' }];
      return field_data;
    }
    ...
    // For all other input types
      ...
      ({ var_names, values } = await scope.getPossibleVarNames( scope, {
