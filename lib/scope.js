...
module.exports = {
  ...
  addToReport: async function( scope, { type, value }) {
    ...
    * keys should correspond to a list containing the relevant data. */
    ...
    if ( !scope.paths ) {
      ...
      // abstracted?
      scope.paths = { debug_log: `_alkiln_test-misc_artifacts/unit_tests_logs-${ Date.now() }.txt` };
    }
  ...
  savePageJSONData: async function( scope ) {
    ...
    */
    ...
    let json_path = path.join( scope.paths.scenario, `json_for-${ scope.page_id }-${ Date.now() }.json` );
