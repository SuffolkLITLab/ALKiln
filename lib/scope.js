...
module.exports = {
  ...
  sign: async function ( scope, name ) {
    ...
    * takes a while to load. */
    //temp: cookies - signature logic
    //Test if developer wants to perform default signature or name arg signature
    //Add conditional statement: if name arg === undefined -> perform default signature
    //if name arg === string value -> perform new functionality
    console.log( 'scope.sign:', name );
    ...
    if (name === undefined) {
      ...
      // Type name arg on canvas
      await scope.page.$eval(`#dasigcanvas`, (canvas, name) => {
        const ctx = canvas.getContext("2d");
        ctx.font = "24px serif";
        ctx.fillText(name, 10, 50);
      }, name);
      scope.page.waitForTimeout(5000);
      console.log('This is the new logic functionality.');
    }
  },
  ...
  steps: {
    ...
    sign: async ( scope, name ) => {
      ...
      //temp: flour
      console.log( 'scope.steps.sign:', name );
      await scope.sign( scope, name );
      await scope.afterStep(scope);
    },  // Ends sign()
