...
module.exports = {
  ...
  waitForTab: async function waitForTab(scope, tab_to_show) {
    ...
    let tab = await scope.page.$(`div[aria-labelledby="${tab_to_show}"`);
    // TODO(brycew): consider waiting for the transitionend event: https://stackoverflow.com/a/57955251/11416267
    if ( tab ) { await scope.page.waitForTimeout( scope.showif_timeout * 2 ); }
  },

  afterStep: async function afterStep(scope, options = {waitForShowIf: false, waitForTab: '', navigated: false, waitForTimeout: 0}) {
    ...
    *    TODO: Update to latest cucumberjs, which has `AfterStep` */
    ...
    if ( scope.page ) {
      ...
      if ( options.waitForTab ) { await scope.waitForTab( scope, options.waitForTab ); }
    }  // ends if scope.page
  ...
  tapElement: async function tapElement(scope, elem, isTab=false, waitForTimeout=0) {
    ...
    if ( navigated ) {
      ...
      options = {waitForShowIf: !isTab, waitForTab: isTab, navigated: navigated};
      if (waitForTimeout > 0) {
  ...
  tapTab: async function ( scope, tab_id ) {
    let elem = await scope.page.waitForSelector(`#${tab_id}`);
    await scope.tapElement( scope, elem, isTab=true );
  },

  tapElementById: async function ( scope, selector, waitForTimeout ) {
    let elem = await scope.page.waitForSelector(`${selector}`);
    await scope.tapElement (scope, elem, false, waitForTimeout );
  },
  ...
  load: async function ( scope, file_name ) {
    ...
    *    use scope.timeout? */
    ...
    if (!scope.browser) {
      scope.browser = await scope.driver.launch({ headless: !session_vars.get_debug() });
    }
