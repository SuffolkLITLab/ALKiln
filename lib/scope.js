...
module.exports = {
  ...
  log_in: async function ( scope, { email_secret_name, password_secret_name }) {
    ...
    */
    ...
    let login_url = `${ env_vars.SERVER_URL || env_vars.BASE_URL }/user/sign-in`;
  ...
  use_the_story_table: async function ( scope, { target_id='', raw_var_data=null }) {
    ...
    * Iterate in here to keep timeout reasonable */
    ...
    let supported_table = [];
    if ( raw_var_data ) {
      supported_table = await scope.normalizeTable( scope, { var_data: raw_var_data.hashes() });
    }
    
    // Merge base story table with this story table
    if ( scope.base_story_table ) {
      supported_table = scope.base_story_table.concat( supported_table );
    }
    ...
    await scope.addToReport(scope, { type: `note`, value: `` });
  },  // Ends scope.use_the_story_table()
