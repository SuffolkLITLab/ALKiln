...
module.exports = {
  ...
  tapElementId: async function ( scope, element_id ) {
    let elem = await scope.page.waitForSelector(`#${element_id}`);
    console.log(`about to tap elem ${ element_id }`)
    await elem.evaluate( (el) => { return el.click(); });
    // A bit hacky, but there's not a better way to handle it TBH. We don't know the next step
    // (fails if "sees phrase", and what you need to do is to wait for a selector, which we can't really do unless we know what's gonna show up, but we don't get info about that.
