    ...
    */
    ...
    for ( let guess of field.guesses ) {
      for ( let var_datum of var_data ) {
        ...
        let guess_name_escaped = escape_regex( guess.var );
        let guess_name_regex = new RegExp(`^${ guess_name_escaped.replace(/['"]/g, `['"]`)}$`);
        let names_match = var_datum.var.match( guess_name_regex );

        let guess_value_escaped = escape_regex( guess.value );
        let guess_value_regex = new RegExp(`^${ guess_value_escaped.replace(/['"]/g, `['"]`)}$`);
        let names_and_values_match = names_match && var_datum.value.match( guess_value_regex );
