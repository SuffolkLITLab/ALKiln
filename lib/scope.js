...
module.exports = {
  ...
  normalizeTable: async function ( scope, { var_data }) {
    ...
    * Format 1 and 2 tables are unsupported. See see https://github.com/plocket/docassemble-cucumber/issues/264
    * Unit tests are in tests/unit_tests/normalizeTable.test.js */
    let supported_table = [];
    for ( let row of var_data ) {
      ...
      supported_table.push( converted );
    }
  ...
  getPageData:  async function ( scope, { html }) {
    ...
    */
    ...
    // Get sought var
    // This is the new sought var element that developers will have to add to their interviews.
    // AL will have them added automatically unless someone overrides the values there.
  ...
  getMatchingRows: async function ( scope, { story_table, page_data }) {
    /* Given a list of var/story table data and a list of field data,
    * returns a list of handles and their matching table rows.
    * 
    ...
    * @param page_data.fields[n].guesses[n].checked {str} - '' if the node
    ...
    */
    // An array of field objects that will have a subset of the field
    // data pluss a list of the matching rows (which we hope will only be
    // one item long)
    ...
    for ( let field of page_data.fields ) {
      ...
      let match = { selector: field.selector, type: field.type, tag: field.tag.toUpperCase(),  // upper case to match DOM
      ...
      for ( let guess of field.guesses ) {  // usually only one or two
        for ( let table_row of story_table ) {
          ...
          // a checkbox needs to have a matching value.
          ...
          switch ( field.tag ) {
            case 'canvas':
              if ( names_and_values_match ) { does_match = true; }
              // Support format 1 tables (var, choice, value)
              else if ( table_row.checked === `/sign` ) { does_match = true; }  // TODO: Add unit and feature tests for this
              break;
            ...
            case 'input':
              if ( field.type === 'checkbox' || field.type === 'radio' ) {
                ...
                if ( names_match ) { does_match = true; }
              }
              break;
  ...
  setVariable: async function ( scope, { field_data }) {
    /* Returns true if the variable was enabled (which
       assumes that it was set correctly if no error happened) */

    // These mutations feel weird to me, but not sure of alternatives
    field_data.was_handled = null;
    field_data.was_activated = null;

    let { matching_table_rows, tag, type } = field_data;
    ...
    if ( env_vars.DEBUG && handle ) {
      // Show we've detected the field, whether or not it's disabled
    ...
    let disabled = await handle.evaluate(( elem ) => { return elem.disabled; });

    // If field is disabled, don't set it yet and indicate it wasn't dealt with yet
    if ( disabled ) {
      field_data.was_handled = false;
      field_data.was_activated = false;
      return field_data.was_handled;
    }
    ...
    // If there was no matching table row found, the field will be set to an 'empty' value.
    // For checkboxes and radios, that means they will be unselected. Text-type fields
    // will be made empty. All items in drowpdowns will be unselected.
    let row = matching_table_rows[0] || { value: null };
    let set_to = row.value;

    // Detect a package's custom data type, like BirthDate, and get the element of that custom type if it exists
    let datatype = await handle.evaluate(( elem )=> {
      // If the element is in a form group then extract the form group's
      // datatype value. For now the only custom datatype we have is in a
      // form group. In future we should look into what happens with a
      // `fieldset` for `choices:`, etc.
    ...
    if ( row.value ) {
      row.was_used = true;
      field_data.was_activated = true;
    }
    field_data.was_handled = true;  // Do unclicked buttons count as set? I'd say yes - same as `false`
    return field_data.was_handled;
  ...
  setCustomDatatype: {
    "BirthDate": async function ( scope, { handle, set_to }) {
      ...
      * field values.
      * WARNING: This datatype really needs to have some ids added to it. */
      if ( set_to === null ) { set_to = '//'; }  // Empty fields of all text
      ...
      for ( let field_i = 0; field_i < fields.length; field_i++ ) {
        ...
        if ( field_i === 0 ) {  // Set the month
          // We avoid `setField.SELECT` to avoid waiting for showif for each
          // field individually. That optimization may not be necessary.
          if ( !date_parts[ 0 ] ) {
            await field.evaluate( elem => { elem.selectedIndex = 0; });
          } else {
            await field.select( date_parts[ 0 ] );
          }
        } else {  // Set the day and year
  ...
  setField: {
    BUTTON: async function ( scope, { handle, set_to }) {
      if ( set_to === null ) { return; }
      await scope.tapElement( scope, handle );
    },
    A: async function ( scope, { handle, set_to }) {
      await scope.setField[`BUTTON`]( scope, { handle, set_to });
    },
    TEXTAREA: async function ( scope, { handle, set_to }) {
      if ( set_to === null ) { set_to = ''; }
    ...
    SELECT: async function ( scope, { handle, set_to }) {
      if ( set_to === null ) {
        // Clear selection of dropdown
        
      } else {
        // Ran into a problem where dev added a space to the end of the option,
        // but the story table got rid of that trailing space, so there was no match.
        // Handle possible extra spaces on either end. Not sure how to condense this.
        let found_with_original = await handle.$(`option[value="${ set_to }"]`);
        let found_with_trailing = await handle.$(`option[value="${ set_to } "]`);
        let found_with_preceeding = await handle.$(`option[value=" ${ set_to }"]`);
        let found_with_both = await handle.$(`option[value=" ${ set_to } "]`);
        // Make sure to use whichever was the correct value
        if ( found_with_trailing )    { set_to = set_to + ' '; }
        if ( found_with_preceeding )  { set_to = ' ' + set_to; }
        if ( found_with_both )        { set_to = ' ' + set_to + ' '; }

        try {
          await handle.select( set_to );
        } catch ( error ) {
          await scope.addToReport({ key: `id`, value: `Could not set dropdown to "${ set_to }".` });
          throw error;
        }
      }
    ...
    CANVAS: async function ( scope, { handle, set_to }) {
      if ( set_to === null ) { return; }  // When would this happen?
      // Wait for `<canvas>` if needed (takes a while to load)
      await scope.page.waitForSelector('#dasigcanvas');
    ...
    INPUT: async function ( scope, { handle, set_to }) {
      ...
      if ( type === `radio` || type === `checkbox` ) {
        ...
        if ( env_vars.DEBUG ) {
          if ( !set_to ) { await label.evaluate( elem => { elem.style.background = 'tomato'; }); }
        ...
        if ( type === `radio` ) { await label[ scope.activate ](); }
          ...
          if ( set_to === null ) { set_to = `false`; }
          await scope.setCheckbox( scope, { label, set_to });
        }
  ...
  processVar: async function ( scope, { var_data, id }) {
    ...
    * continuing to the next page. Important to avoid recursion because of async ops. */
    ...
    // TODO: Discuss: Might be able to use `field_data.was_handled` - as long as the number
    // of those stay the same, we've reached statis
    let fields_to_set = [...matches];
    ...
    while ( prev_length > fields_to_set.length && fields_to_set.length > 0 ) {
      ...
      for ( let field_data of fields_to_set ) {
        let { tag, type } = field_data;
        // We will only continue after setting all other possible fields (see next loop)
        if (( tag === 'BUTTON' && type === 'submit' ) || tag == `A` ) {
          remaining_fields.push( field_data );
          continue;
        }

        // Try to set other types of fields
        let field_was_handled = await scope.setVariable( scope, { field_data });
        if ( field_was_handled ) {
    ...
    for ( let data_i = 0; data_i < fields_to_set.length; data_i++ ) {
      let field_data = fields_to_set[ data_i ];
      let { tag, type } = field_data;
      if ( tag === 'BUTTON' && type === 'submit' ) {
        let page_url = await scope.page.url();
        await scope.setVariable( scope, { field_data });
        // Only wait for continuance if button was actually pressed
        if ( !field_data.was_activated ) { continue; }
        ...
        // Mutating array while looping is ok because we break right after.
    ...
    // If didn't already press a continue button, press one now that all fields have been set.
