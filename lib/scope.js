...
module.exports = {
  ...
  getMatchingRow: async function ( scope, { story_table, page_data }) {
    ...
    */
    // An object containing a key for each field. Its value will be
    // an array of table rows matching that field. See longer comments after
    // loop for detailed explanation.
    ...
    for ( let field of page_data.fields ) {
      for ( let guess of field.rows ) {  // usually only one or two
        for ( let table_row of story_table ) {
          ...
          if ( rows_match ) {
            // Collect all the matches for each individual field since
            // we may need to de-duplicate them.
            if ( matches_by_field[ field.selector ] ) {  // unique value per field
