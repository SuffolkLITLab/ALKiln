...
module.exports = {
  ...
  getField: async function ( scope, { var_name, choice_name, set_to }) {
    ...
    let all_attempts = [
      await scope.page.$$(`#dasigcanvas`),  // signature canvas
  ...
  setField: {
    ...
    CANVAS: async function ( scope, { handle }) {
    ...
    // while ( !id_matches || var_data.length > 0 ) {
      ...
      for ( let row of var_data ) {
        ...
        if ( handle ) {
          ...
          // three questions we want to answer, but only one is required,
          // we'll continue too soon.
        } else {
