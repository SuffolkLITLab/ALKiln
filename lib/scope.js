...
module.exports = {
  ...
  getAllFields:  async function ( scope, { html }) {
    ...
    *  TODO: Pass in `$` instead? */
    ...
    if ( !encoded_trigger_var ) {
      ...
      reports.addToReport(scope, {
        type: `warning`,
        value: `You are missing an element containing the data for the page's trigger variable. Some fields, like loops with index variables, may not work correctly in these tests. See https://suffolklitlab.org/docassemble-AssemblyLine-documentation/docs/automated_integrated_testing/#a-missing-trigger-variable.`
      });
    ...
    for ( let node of all_nodes ) {
      ...
      if ( $(`#dasigpage`).length > 0 && $(`#dasigpage`)[0].name ) {
        ...
        encoded_name = $( `input[name="_save_as"]` ).attr( `value` );
        field_data.guesses = [{
          var: trigger,
          value: '/sign',
        }];
      ...
      // Table rows
      // ====================
      let has_none_of_the_above = false;
      if ( $node.hasClass('danota-checkbox') || $node.parents(`.da-field-container-datatype-object_radio`).name ) {
        ...
        encoded_name = first_sib.attribs.name;
      ...
      let var_names = await scope.getPossibleVarNames( scope, {
        ...
        field_like_names,
        has_none_of_the_above: has_none_of_the_above,
      });
  ...
  getPossibleVarNames: async function ( scope, { encoded, field_like_names, has_none_of_the_above }) {
    ...
    *  @param has_none_of_the_above {bool} - is the field a 'none of the above' field? */
    ...
    return scope.decodeFully(scope, { expr_str: layer_1_decoded, field_like_names, has_none_of_the_above });
  },  // Ends scope.getPossibleVarNames()

  decodeFully: function ( scope, { expr_str, field_like_names = {}, has_none_of_the_above} ) {
    /*
