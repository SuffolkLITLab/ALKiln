...
module.exports = {
  ...
  uploadFiles: async function ( scope, { handle, answer }) {
    ...
    */
    ...
    if (chosen_dir === null && err_paths.length > 0) {
    ...
    if (dirs.length > 1) {
      await scope.addToReport( scope, { type: `warning`, value: `You have multiple folders in the 'docassemble' folder (${ dirs }). These tests will use the first folder with a "data/sources" subfolder (${chosen_dir}).` });
    }
