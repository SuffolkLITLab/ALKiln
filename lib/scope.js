...
module.exports = {
   ...
   */
    ...
    // TODO: right now this is wrong because it's the original `row.value`. `.there_is_another`
    // sometimes needs to be an adjusted value. Maybe we could just recalculate that value in here.
  ...
  setCheckbox: async function ( scope, { label, set_to }) {
    ...
    // `.there_is_another` won't work for this yet (because `set_to` should not be the original `row.value` sometimes)
  ...
  getThereIsAnotherValue: async function ( scope, { row, from_table=true }) {
    ...
    */
    ...
    // Invalid value defaults to 'False' with a line added to the report
      ...
      // console.warn?
      await scope.addToReport( scope, { key: `id`, value: `You must use a number or \`False\` in a \`.there_is_another\` row. The value was instead ${ value }. This test will default to \`False\` to avoid an infinite loop.` });
      return `False`;
