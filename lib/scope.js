...
module.exports = {
  ...
  start: async function ( scope, file_name, attempt_num ) {
  ...
  getField: async function ( scope, { var_name, choice_name, set_to }) {
    ...
    if ( choice_name ) {
      throw new ReferenceError( `${ err_message } and the value "${ choice_name }. Is it hidden?"` );
    } else {
      throw new ReferenceError( `${ err_message }. Is it hidden?` );
    }
