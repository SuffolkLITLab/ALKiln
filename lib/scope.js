...
module.exports = {
  ...
  addToReport: async function( scope, { type, value }) {
    ...
    * keys should correspond to a list containing the relevant data. */
    ...
    // Ensure filepath for unit tests.
    if ( !scope.paths ) {
      // TODO: put in existing artifacts folder if possible. Can this be usefully
      // abstracted?
      scope.paths = { debug_log: `_alkiln_test-unit_tests_logs-${ Date.now() }.txt` };
    }

    fs.appendFileSync(scope.paths.debug_log, `Report:\ntype: ${ type }, value: ${ value }`);
    return scenario;
