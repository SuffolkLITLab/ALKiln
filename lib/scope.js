...
module.exports = {
  ...
  getFieldSelector: async function ( scope, { $, node }) {
    ...
    */
    ...
    // TODO: Use `expect` instead? Maybe add link to issues page. [which issue?]
    // Submit buttons are a dime a dozen on da pages and aren't used to match variables.
    // Everything else should be unique.
    else if ( selector !== '#daquestion button' ) {
      throw `${ matching_nodes.length } nodes match the '${ selector }' selector. Talk to the developers.`;
    }
