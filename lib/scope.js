...
module.exports = {
  ...
  uploadFiles: async function ( scope, { handle, answer }) {
    ...
    */
    ...
    // TODO: change this to just get the one directory in the docassemble dir, or change the tests/features hardcoded path
    let dir_source = `${ process.cwd() }/docassemble/${ session_vars.get_repo_url().match(/docassemble-(.*)/)[1] }/data/sources`;
    let dir_features = `${ process.cwd() }/docassemble/ALKilnTests/data/sources`;
    let dir = null;
    ...
    try {
      ...
      try {
        ...
        await scope.addToReport( scope, { type: `warning`, value: `You have both a docassemble package 'sources' folder and a 'docassemble/ALKilnTests' folder. These tests will use the package's 'sources' folder.` });
