...
module.exports = {
  ...
  normalizeTable: async function ( scope, { var_data, from_story_table=true }) {
    ...
    for ( let row of var_data ) {
      ...
      if (is_secret_var(row)) {
        let actual_var_value = process.env[row.flags.secret_name];
        if ( actual_var_value === undefined ) {
          ...
          throw ReferenceError(`Could not find "${not_in_env_msg}".`)
        }
      }
    ...
    if ( ensure_all_vars_used ) {
      ...
      for ( let row of var_data ) {
        if ( row.times_used === 0 ) {
          ...
          let reported_val = printable_var_value(original_row); 
  ...
  getThereIsAnotherValue: async function ( scope, { row, from_story_table=true }) {
    ...
    */
    let value = row.value;
    let reported_value = printable_var_value(row); 
