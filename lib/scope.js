...
module.exports = {
  ...
  ensureSpecialRows: async function ( scope, { var_data, from_story_table=true }) {
    ...
    */
    ...
    for ( let var_datum of var_data ) {
      ...
      // Add a `.there_is_another` or `there_are_any` vars if they might be needed.
      // If they're not used in the end, that's fine.
      ...
      if ( from_story_table && var_datum.var.match( /\.target_number$/ )) {
        ...
        // already a `.there_are_any`, that will come first. Will it be used instead?
        // Check if later matches override earlier matches.
        if ( parseInt(var_datum.value) === 0 ) {
          let there_are_any = {
            artificial: true,  // not currently used but is future-proofing to mark non-dev generated rows
            source: var_datum,
            var: var_datum.var.replace( /\.target_number$/, `.there_are_any` ),
            value: `False`,
        ...
        // help allow using only `.target_number` for loop answers.
        // Could be in an `else`, but that seems like unnecessary nesting.
  ...
  getThereIsAnotherValue: async function ( scope, { row, from_story_table=true }) {
    ...
    */
    ...
    // TODO: Consider setting a default value of `'0'` for `.times_used`
    } else if ( row.times_used >= int_val ) {  // Story table loop will end
        return `False`;

    } else if ( row.source && row.source.times_used >= int_val ) {  // Just in case use changes a tad
        return `False`;

    } else if ( isNaN( parseInt( value )) ) {  // Story table loop will end
      ...
      // It's a number that is below the the maximum.
      // Story table loop will continue.
      return `True`;
    }
