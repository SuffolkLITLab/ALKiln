...
module.exports = {
  ...
  normalizeTable: async function ( scope, { var_data }) {
    ...
    * Turn arrays into objects. */
    ...
    let var_objs = var_data;
    if ( Array.isArray( sample )) {
      let rows = raw_var_data.raw();
      var_objs = rows.map(( row ) => {
        let result = { var: row[0], value: row[1], checked: row[2] };
        if ( row[3] ) { result.sought = row[3]; }
        return result;
      });
    }
  ...
  getPageData:  async function ( scope, { html }) {
    ...
    */
    ...
    if ( !page_data.sought_var ) {
      ...
      await scope.addToReport(scope, { key: 'ids', value: `Warning: You are missing an element containing the data for the page's sought/trigger variable. Some fields, like loops with index variables, may not work correctly in these tests. Talk to the folks at the AssemblyLine project to learn more.` });
    ...
    for ( let node of all_nodes ) {
      ...
      // To allow more felxibility for devs of simpler interviews,
      // will not require sought var unless it's necessary.
      ...
      //   // Get the data that will help us construct the intrinsic name later and
      //   // look for the sought_var match now.
      //   let pattern = await scope.getSoughtVarPatterns( scope, { field_name: name, });
      //   field_data.name_patterns.push( pattern );
      //   // Try to pull out the intrinsic name parts
      //   let maybe_map = await scope.getProxiesToNamesMap( scope, {
      //     sought_var: page_data.sought_var, pattern_data: pattern,
      //   });
      //   // If that works, it's the right map for the proxy values
      //   if ( Object.keys( maybe_map ).length ) {
      //     proxies_to_names_map = maybe_map;
      //   }
      //   // TODO: Turn proxies_to_names_map into a list if there turn out to be mutliple
      //   // layers of name encodings. For now, let's keep it simple.
      // }

      // temp_fields.push( field_data );

      field_data.rows = await scope.getPossibleTableRowMatchers( scope, { node, var_names, values });
      page_data.fields.push( field_data );
    }  // ends for every node

    // // Build the actual field data with real var names (intrinsic names of fields)
    // page_data.fields = [];
    // for ( let temp_field of temp_fields ) {
    //   // The final field data
    //   let field = {
    //     selector: temp_field.selector,
    //     tag: temp_field.tag,
    //     type: temp_field.type,
    //   };

    //   // Get the correct intrinsic names for each field
    //   let var_names = [];
    //   for ( let pattern of temp_field.name_patterns ) {
    //     // Use the sought var and the data of this field to ensure any
    //     // proxy names are converted into intrinsic names
    //     let intrinsic_name = await scope.getIntrinsicName( scope, {
    //       name_parts: pattern.name_parts,
    //       proxies_to_names_map: proxies_to_names_map,
    //     });
    //     var_names.push( intrinsic_name );
    //   }
      
    //   // TODO: rename `rows` to `guesses`?
    //   field.rows = await scope.getPossibleTableRowMatchers( scope, {
    //     node: temp_field.node, var_names, values: temp_field.values,
    //   });

    //   page_data.fields.push( field );
    // }  // ends for every field's data
  ...
  getFieldSelector: async function ( scope, { $, node }) {
    ...
    */
    ...
    // Submit buttons are a dime a dozen on da pages and aren't used to match variables.
    // Everything else should be unique.
    if ( matching_nodes.length > 1 && selector !== '#daquestion button' ) {
  ...
  getIntrinsicName: async function ( scope, { name_parts, proxies_to_names_map }) {
    /* Given a field's name that may contain proxy vars like x, i,
    *  j... and the page's sought variable intrinsic name, will
    *  return the matching intrinsic name. */
    let intrinsic_name = '';

    for ( let part of name_parts ) {
      if ( proxies_to_names_map[ part ] ) {
        intrinsic_name += proxies_to_names_map[ part ];
      } else {
        intrinsic_name += part;
      }
    }  // ends for building instance name

    if ( intrinsic_name === '' ) { intrinsic_name = name_parts.join(''); }
    return intrinsic_name;
  },  // Ends scope.getIntrinsicName()
  ...
  getSoughtVarPatterns: async function ( scope, { field_name }) {
    /** Given a var returns data about its use of proxies. If it uses
    *  proxies (x, i, j, etc.), get all the necessary parts.
    *  Note: 'proxies' is my own name for this behavior.
    * 
    * Example:
    * x[i].name.first ->
    * {
    *   uses_proxies: true,
    *   proxies: [ 'x', 'i' ],
    *   names_regex: /^(.+)\[(.+)\]\.name\.first$/,
    *   name_parts: [ '', 'x', '[', 'i', '].name.first' ]
    * }
    * 
    * TODO: How do we detect false matches? e.g. if both foo['bar'] and
    *   fiz['buz'] are asked on the page and only one is x[i]. Or foo['bar']
    *   and fiz['bar'] with x['bar']. Would that come up?
    */
    let pattern_data = {
      uses_proxies: false,  // x, i, j, etc.
      proxies: [],
      // TODO: Discuss: Valid regex that won't find a match?
      names_regex: new RegExp( `this regex should not match anything` ),
      name_parts: [ field_name ],  // Entire name by default
    };

    if ( proxies_regex.test( field_name )) {
      pattern_data.uses_proxies = true;
      /* substitute, placeholder, proxy, envoy, mediary, wildcard, gap var, catcher, ~catch all~, reserve, generic, space */
      pattern_data.proxies = field_name.match( proxies_regex );
      let betweens = field_name.split( proxies_regex )

      // The regex that will discover and capture the actual var names
      let escaped = [];
      for ( let tween of betweens ) { escaped.push(
        tween.replace(/[\-\[\]{}()*+?.,\\\^$|#\s]/g, "\\$&")
      );}
      let full_regex = '^' + escaped.join( '(.+)' ) + '$';
      pattern_data.names_regex = new RegExp( full_regex );

      // A list where every item of the proxy string is included in order
      // There must be a regex way to do this, but I haven't found it
      let total_len = pattern_data.proxies.length + betweens.length;
      let rebuilt = [];
      for ( let indx = 0; indx < total_len; indx++ ) {
        if ( indx % 2 === 0 ) {
          rebuilt.push( betweens[ indx/2 ] );
        } else {
          rebuilt.push( pattern_data.proxies[ Math.floor( indx/2 ) ]);
        }
      }  // end for rebuilding
      pattern_data.name_parts = rebuilt;
    }  // ends if has proxy parts (x, i, j, etc.)

    return pattern_data;
  },  // Ends scope.getSoughtVarPatterns()

  getProxiesToNamesMap: async function ( scope, { sought_var, pattern_data }) {
    /** Identify the parts of the name that will match up with parts in proxies.
    * Find the pattern in the data that matches the sought variable name.
    * Capture and return which proxy variables match which variable name parts. */
    let proxies_to_names_map = {};

    let name_finder_regex = pattern_data.names_regex;
    // Avoid matching empty strings
    if ( name_finder_regex ) {
      let matches = sought_var.match( name_finder_regex );
      if ( matches ) {
        proxies_to_names_map = {};
        let proxies = pattern_data.proxies;
        for ( let proxy_i = 0; proxy_i < proxies.length; proxy_i++ ) {
          let proxy_char = proxies[ proxy_i ];
          // The first item in js `.match` is just the full match
          proxies_to_names_map[ proxy_char ] = matches[ proxy_i + 1 ];
        }
      }  // ends if matches are found
    }  // ends if the var uses proxies

    return proxies_to_names_map;
  },  // Ends scope.getProxiesToNamesMap()
  ...
  getMatchingRow: async function ( scope, { story_table, page_data }) {
    ...
    */
        ...
        for ( let table_row of story_table ) {
          ...
          switch ( field.tag ) {
            case 'canvas':
              ...
              // Support v1 tables
              else if ( table_row.checked === `/sign` ) { rows_match = true; }  // TODO: Add unit and feature tests for this
          ...
          if ( rows_match ) {
            // Collect all the matches for each individual field - we
            // may need to de-duplicate them.
            if ( matches_by_field[ field.selector ] ) {  // selector should be unique per field
              matches_by_field[ field.selector ].push({ field: field, table: table_row });
            } else {
              matches_by_field[ field.selector ] = [{ field: field, table: table_row }];
            }
    ...
    // We think this should only happen if the row is using a proxy vars.
    ...
    // know which row to use when, we need the one unique var that
    // will appear on the page - the sought var. When a row with `users[i]`
    // has a sought var described that matches the page sought var, the
    // program knows to use that row to set the value.

    // For now, we're being flexible about it - if we don't think we
    // need that sought var because the situation is not as described
    // above, we don't prevent data getting through.
    ...
    de_duped_matches = [];
    for ( let field_key in matches_by_field  ) {
      let field_matches = matches_by_field[ field_key ];
      if ( page_data.uses_proxies && field_matches.length > 1 ) {
        // If multiple story table rows match one field, only
        // keep the one(s) that match this particular page (sought var)
        for ( let match of field_matches ) {
          if ( match.table.sought === page_data.sought_var ) {
            de_duped_matches.push( match );
          }
        }
      } else {
        // If proxy vars aren't relevant here or if only
        // one table row matched this field, use that match obj
        de_duped_matches.push( field_matches[0] );
      }  // ends if page uses proxies
    }  // ends for each field's matches

    return de_duped_matches;
  ...
  setVariable: async function ( scope, { handle, row }) {
    ...
    if ( type === `checkbox` ) { set_to = row.checked; }
  ...
  processVar: async function ( scope, { var_data, id }) {
    ...
    * continuing to the next page. Important to avoid recursion because of async ops. */
    ...
    for ( let row of matches ) {
      ...
      // Only wait for `<canvas>`. It takes a while to load, but will never be
      // hidden. Other elements could be hidden, though, and will never show up.
      if ( field.tag === 'canvas' ) {
        await scope.page.waitForSelector('#dasigcanvas');
      }
