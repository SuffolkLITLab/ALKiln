...
module.exports = {
  ...
  uploadFiles: async function ( scope, { handle, answer }) {
    ...
    */
    ...
    // Tests should be one place: docassemble/*/data/sources, where * is the only folder in `docassemble`.
    // Makes an absolute path from the folder in which `npm run` was used plus our internal folders
    let docassemble_pkg_path = `${ process.cwd() }/docassemble`;
    let subpackage_name = session_vars.get_package_name().substring(12);
    let pkg_paths = [];
    try {
      fs.accessSync(docassemble_pkg_path);
      // https://stackoverflow.com/a/24594123/11416267
      let dirs = fs.readdirSync(docassemble_pkg_path, {withFileTypes: true}).filter(dirent => dirent.isDirectory()).map(dirent => dirent.name);
      if (dirs.length < 1) {
        let msg = `You can store files to upload in 'docassemble/${ subpackage_name }/data/source'. These tests could not find a folder like that starting at ${ process.cwd() }.`;
        await scope.addToReport( scope, { type: `warning`, value: msg});
        throw Error(msg);  // Most people will want to know the sources folder path;
      }
      for (let dir of dirs) {
        pkg_paths.push(`${docassemble_pkg_path}/${dir}`);
      }
    } catch ( error_da ) {
      let msg = `You can store files to upload in 'docassemble/${ subpackage_name }/data/source'. ` +
          `These tests could not find a folder like that. The current directory is ` +
          ` ${ process.cwd() }; is that in the top level folder of your docassemble package? (${ session_vars.get_package_name() })?`;
      await scope.addToReport( scope, { type: `warning`, value: msg});
      throw error_da;  // Most people will want to know the sources folder path;
    }

    let chosen_dir = null;
    let err_paths = [];
    for (let pkg_path of pkg_paths) {
      let dir = `${ pkg_path }/data/sources`;
      try {
        fs.accessSync(dir);
        chosen_dir = dir;
        break;
    ...
    if (chosen_dir === null) {
      let msg = `You can store files to upload in ${err_paths.join(", or ")}. These tests could not find any of those folders.`;
      await scope.addToReport( scope, { type: `warning`, value: msg });
      throw Error(msg);  // Most people will want to know the sources folder path;
    }

    if (pkg_paths.length > 1 && pkg_paths.length - err_paths.length > 1) {
      await scope.addToReport( scope, { type: `warning`, value: `You have multiple folders in the 'docassemble' folder (${ pkg_paths }). These tests will use the first folder with a "data/sources" subfolder (${chosen_dir}).` });
    }
    ...
    for ( let name of file_names ) {
      ...
      name = name.replace(/^\s/, '');
      let file_path = path.join( chosen_dir, name );
      ...
      try {
        ...
        await scope.addToReport( scope, { type: `warning`, value: `Could not find "${ name }" in "${ chosen_dir }"` });
      }
    }
  ...
  draw_signature: async function ( scope ) {
    ...
    for ( let field of fields ) {
      ...
      if ( field.tag === 'button' && field.type === 'submit' ) {
        ...
        if ( row_used ) {
          ...
          let { id } = scope.examinePageID( scope );
          await scope.page.screenshot({
            path: `./${ scope.paths.scenario }/pic-${Date.now()}-${ id }.jpg`,
            type: 'jpeg',
            fullPage: true
          });
