...
const fs = require('fs');
const _internal_tests = require('../tests/features/test.js');
...
module.exports = {
  ...
  normalizeTable: async function ( scope, { var_data, from_story_table=true }) {
    ...
    * Table must have headers. Arrays are not supported. */
    ...
    if ( env_vars.DEBUG ) { console.log( 'scope.normalizedTable():', JSON.stringify( supported_table )); }
  ...
  getFieldSelector: async function ( scope, { $, node }) {
    ...
    */
    ...
    if ( matching_nodes.length > 1 && !selector.includes(`[class="file-caption-name"]`) ) {
      let msg = `WARNING: ${ matching_nodes.length } items on this page matched the base64 encoded name '${ selector }'. You may be setting the same variable in multiple places on this page.`;
  ...
  uploadFiles: async function ( scope, { handle, set_to }) {
    ...
    try {
      ...
      try {
        ...
        await scope.addToReport( scope, { type: `warning`, value: `WARNING: You have both a docassemble package 'sources' folder and a 'test/features' folder. These tests will use the package's 'sources' folder.` });
      ...
      try {
        ...
        await scope.addToReport( scope, { type: `warning`, value: `WARNING: There are two places where you can store files to upload: "${ dir_source }" and "${ dir_features }". These tests could not find either of those.` });
    ...
    for ( let name of file_names ) {
      ...
      try {
        ...
        await scope.addToReport( scope, { type: `warning`, value: `WARNING: Could not find "${ name }"" in "${ dir }"` });
      }
    }
    ...
    if ( ensure_all_vars_used ) {
      ...
      for ( let row of var_data ) {
        if ( row.times_used === 0 ) {
          ...
          let origina_row = source_row.original;
          let msg = `Did not find a field on this page for the variable \`${ origina_row.var }\` that could be set to \`${ origina_row.value }\``;
          await scope.addToReport(scope, { type: `error`, value: msg });
        }
      }
      expect( num_unused_rows, `Missing variables caused test failure.` ).to.equal( 0 );
    }
  ...
  //#####################################
  //#####################################
  testReport: async function ( scope, { stored_report_key }) {
