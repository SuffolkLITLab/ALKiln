...
module.exports = {
  ...
  getLoadData: async function ( scope, { timeout=null }) {
    ...
    * which won't match either of those and this will timeout and, I assume, error.*/
    ...
    let options = {};
    if ( timeout ) { options.timeout = timeout; }  // No 0 timeout allowed
    let winner = await Promise.race([
      scope.page.waitForSelector( `#daMainQuestion`, options ),
      scope.page.waitForSelector( `#da-retry`, options ),
    ]);
