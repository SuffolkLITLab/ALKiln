...
module.exports = {
  ...
  increment_row_use: async function ( scope, { row }) {
    ...
    *   Mutates `row` and its props. */
    ...
    if ( row.source ) {
      // Increment the number times the source row's artificial row trackers
      ...
      // Increment the number of times the source row has been used in total.
  ...
  getThereIsAnotherValue: async function ( scope, { row, from_story_table=true }) {
    ...
    */
    ...
    if ( value.match( /(false|no|unchecked|uncheck|deselected|deselect)/i )) {
      ...
      if ( from_story_table && !row.is_artificial ) {
    ...
    // Why `desired_num - 1`? The first `there_is_another` is actually the second loop
    // (after they've already filled in the first list item info). If the user always had
    // to set `there_are_any`, this wouldn't be a problem, but sometimes the author sets
    // `there_are_any` beforehand instead so we can't depend on this, so we only count the
    // use of `there_is_another` and make sure to count the 1st appearance as the second
    // loop. (or the 0'th appearance as the first loop)
    } else if ( row.source && row.source.used_for[ row.var ] >= ( desired_num - 1 )) {  // Story table loop will end
      return `False`;
    } else {
      return `True`;
