...
module.exports = {
  ...
  getPageData:  async function ( scope, { html }) {
    ...
    */
    ...
    // TODO: Change to `sought_name`?
    let sought_var_str = await scope.fromBase64( scope, { base64_str: encoded_sought_var });
    page_data.sought_var = JSON.parse( sought_var_str )[0];  // Why is this a list? Can multiple vars be sought?
    ...
    for ( let node of all_nodes ) {
      ...
      if ( $(`#dasigpage`).length > 0 && $(`#dasigpage`)[0].name ) {
        ...
        field_data.values = [ `/sign` ];  // Our custom value for signatures. TODO: discuss
        field_data.var_names = [ page_data.sought_var ];
        let canvas_pattern = await scope.getSoughtVarPatterns( scope, {
          field_name: page_data.sought_var,
        });
        field_data.name_patterns = [ canvas_pattern ];
      ...
      // TODO: add default of empty string for values?

      field_data.var_names = var_names;
      field_data.values = values || [];

      // We need to try the full names of each field. If there are proxy vars (x, i, etc)
      // this will let us find which one matches the sought_var and we can use it to
      // create a map from proxies to intrinsic names. Ex: {x: "foo"}.
      // We will then need to use the data gathered here to convert each field to
      // have its intrinsic name.
      field_data.name_patterns = [];
  ...
  getSoughtVarPatterns: async function ( scope, { field_name }) {
    ...
    */
    ...
    // This uses a word break to make sure we get an isolated x, i, j, ...
    if ( proxies_regex.test( field_name )) {
      ...
      let escaped = [];
      for ( let tween of betweens ) {
        // Save things like brackets, but replace every other group with .*
        // Example need:
        // Sought var is `people[1].name.first`
        // Because the question is using `code:`, the sought var is there as that exact match: `people[1].name.first`
        // A second field is using `i`, though: `people[i].birthdate`
        // Matching only to 'people[', 'i', '.name.first' will not work as there's no such text on the page
        // Here we have to be able to discover 'i' using any field that may
        // contain 'i', no matter what the other text surrounds it. Will this be too permissive?
        let sans_periods = tween.replace(/\./g, "");
        let safe = sans_periods.replace(/[\-\[\]{}()*+?.,\\\^$|#\s]/g, "\\$&")
        let alpha_replaced = safe.replace(/[A-Za-z0-9_]+/g, ".*");
        escaped.push( alpha_replaced );
      }
