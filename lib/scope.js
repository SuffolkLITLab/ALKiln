...
module.exports = {
  ...
  normalizeTable: async function ( scope, { var_data }) {
    ...
    * Table must have headers. Arrays are not supported. */
    ...
    // Support tables with no 'sought' column
    let supported_table = [];
  ...
  sign: async function ( scope ) {
    /* Draw in a #dasigcanvas element. `<canvas>` takes a while to load. */
  ...
  processVar: async function ( scope, { var_data, id, non_table=false }) {
    ...
    * If it's not data in a table, the code can be more permissive about some input. */
    ...
    if ( !non_table && !already_continued ) {
