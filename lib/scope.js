...
module.exports = {
  ...
  getPageData:  async function ( scope, { html }) {
    ...
    */
    ...
    for ( let node of all_nodes ) {
      ...
      // Example of field-like name key/value: _field_2: 'some_checkbox_field'
      ...
      let var_names = await scope.getPossibleVarNames( scope, { encoded: encoded_name, field_like_names });

      // Construct selectors for all except `option` and `canvas`.
      let selector = '';  // Selector of element to manipulate
      let tag = node.name;
      field_data.tag = tag;
      selector += `${ tag }[name="${ encoded_name }"]`;
      ...
      // because puppeteer doesn't interact with hidden elements.
      // For those, should we just use `.click()` ourselves? Is there a downside?
      ...
      // Possible alternative: `closest()` will match self too.



    // Selector type `${ tag }[name="${ name }"][value="${ value }"]`
    // If `button`
  ...
  getPossibleVarNames: async function ( scope, { encoded, field_like_names }) {
    ...
    if ( checkbox_1_match ) {
      ...
      let layer_1_decoded = await scope.fromBase64( scope, { base64_str: checkbox_1_match[2] });
      names.choice_names.push( layer_1_decoded );
      table_names.push( `${ var_name }['${ layer_1_decoded }']` );
      ...
      try {
        ...
        let layer_2_decoded = await scope.fromBase64( scope, { base64_str: layer_1_decoded });
        names.choice_names.push( layer_2_decoded );
        table_names.push( `${ var_name }['${ layer_2_decoded }']` );
      } catch ( err ) {}  // Do nothing
    ...
    // Ex: checkboxes_choices[R'Y2hlY2tib3hfb3RoZXJfb3B0XzI']
      ...
      // Get the choice name. E.g. option_1, option_2
      let layer_1_decoded = await scope.fromBase64( scope, { base64_str: checkbox_2_match[2] });
      names.choice_names.push( layer_1_decoded );
      table_names.push( `${ var_name }['${ layer_1_decoded }']` );
    ...
    // This may end up needing multiple options. Just preparing for that.
