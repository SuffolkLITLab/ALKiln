...
module.exports = {
  ...
  uploadFiles: async function ( scope, { handle, answer }) {
    ...
    */
    ...
    // First make sure the list of files is of the correct format
    ...
    // `/usr/share/docassemble/files/playgroundsources/${session_vars.get_user_id()}/${session_vars.get_user_project_name()}/*.feature`);
    // `/tmp/playgroundsources/${session_vars.get_user_id()}/${session_vars.get_user_project_name()}/*.feature`);
    // MUST have proj name for right upload files

    let dirs_playground = fg.sync(
      [ `/usr/share/docassemble/files/playgroundsources/**/data/sources` ],
      { onlyFiles: false, suppressErrors: true }
    );
    console.log(`\n=== dirs_playground ===`);
    console.log(dirs_playground);

    let dirs_playground_all = fg.sync(
      [ `/usr/share/docassemble/**/data/sources` ],
      { onlyFiles: false, suppressErrors: true }
    );
    console.log(`\n=== dirs_playground_all ===`);
    console.log(dirs_playground_all);

    let dirs_s3 = fg.sync(
      [ `/tmp/playgroundsources/**/data/sources` ],
      { onlyFiles: false, suppressErrors: true }
    );
    console.log(`\n=== dirs_s3 ===`);
    console.log(dirs_s3);

    let dirs_repo = fg.sync(
      [ `${ process.cwd() }/**/docassemble/**/data/sources` ],
      { onlyFiles: false, suppressErrors: true }
    );
    console.log(`\n=== dirs_repo ===`);
    console.log(dirs_repo);

    // Make sure sources directory exists
    let dirs = fg.sync([ `${ process.cwd() }/**/docassemble/**/data/sources` ], { onlyFiles: false });
    if ( dirs.length === 0 ) {
      await scope.addToReport( scope, {
        type: `warning`,
        value: `The test files need to be in the "sources" directory anywhere within "${ process.cwd() }", but ALKiln couldn't find it.`
      });
      return false;
    }
    let dir = null;
    ...
    for ( let name of uploads_names ) {
      ...
      if ( dir === null ) {
        ...
        matching_paths = fg.sync([ `${ dir }/${ trimmed_name }` ]);
      }

      if ( matching_paths.length === 0 ) {
        missing_files.push( trimmed_name );
      } else {
        paths.push( matching_paths[0] );
      }
    }  // ends for each file
    ...
    if ( missing_files.length > 0 ) {
      await scope.addToReport( scope, {
        type: `warning`,
        value: `ALKiln could not find "${ missing_files.join('", "') }" in "${ dir }".`
      });
    }
