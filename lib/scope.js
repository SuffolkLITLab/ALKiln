...
module.exports = {
    ...
    */
    ...
    for ( let node of all_nodes ) {
      ...
      if ( $(`.dasignature`).name ) {
        ...
        field_data.rows = [{ var_names: [ page_data.sought_var ], choice_names: [], values: [] }];
        break;
      }
      ...
      // Selectors
      // ====================
      // For checkboxes and radios, puppeteer needs to interact with their labels
      ...
      else { throw `${ self_nodes.length } match the '${ selector }' selector. Talk to the developers.`; }
      ...
      // Not everything has its associated choice names yet.
      if ( choice_names.length === 0 ) {
        ...
        // TODO: Text inputs and textareas must ignore the choice_name, etc. How?

        if ( node.name === `select` ) {
          choice_names = await scope.getOptionValues( scope, { $select: $node });
        } else if ( node.attribs.value ) {
          choice_names.push( node.attribs.value );
        }
      }

      // Get data for table rows (as table rows come in as data)
      let possible_table_rows = [];
      for ( let var_name of var_names ) {
        // Inputs like textareas don't have choice_names
        if ( choice_names.length > 0 ) {
          for ( let choice_name of choice_names ){
            // Get the status of elements that can be checked or unchecked
            let row = { var_name, choice_name, checked: false };
            if ( node.attribs.checked !== undefined ) { row.checked = true; }
            possible_table_rows.push( row );
          }
        } else {
          possible_table_rows.push({ var_name, choice_name: '', checked: '' });
        }
        
      }  // end iterate through row possibilities

      field_data.rows = possible_table_rows;
      fields_datas.push( field_data );
      // console.log( field_data.rows );
    ...
    want to be able to de-select items as well as select checkboxes.
    */
  ...
  getOptionValues: async function ( scope, { $select }) {
    /* Gets all the option values of the select node */
    let options = [];
