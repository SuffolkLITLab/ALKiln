...
module.exports = {
    ...
    for ( let guess of field.guesses ) {
      for ( let var_datum of var_data ) {
        ...
        if ( var_datum.var.includes( `.there_is_another` )) {
          table_value = await scope.getThereIsAnotherValue( scope, { row: var_datum, from_table: from_table });
        }
   ...
   */
    ...
    if ( row.var.includes( `.there_is_another` )) {  // Seemed cleaner to just repeat
      set_to = await scope.getThereIsAnotherValue( scope, { row: var_datum, from_table: from_table });
    }
    ...
    * If it's not data in a table, the code can be more permissive about some input. */
    ...
    while ( prev_length > fields_to_set.length && fields_to_set.length > 0 ) {
      ...
      for ( let field_i = 0; field_i < fields_to_set.length; field_i++ ) {
        ...
        let row_used = await scope.setVariable( scope, { field, var_data, from_table });
        if ( row_used ) {
          row_used.times_used += 1;  // Esp important for `.there_is_another`
    ...
    for ( let data_i = 0; data_i < fields_to_set.length; data_i++ ) {
      ...
      if ( tag === 'button' && type === 'submit' ) {
        ...
        // TODO: discuss: calculate the `.there_is_another` value here? Would need to pass in fake
        // var_data. Then what data to we want to give to the developer in the reports?
        // The fake data? It would remove the need to pass down `from_table`, though.
        // Could just send a whole complex object with a 'special' `set_to`.
        // TODO: Is it possible for the same `.there_is_another` to show up again after it's already been set to False?
        let row_used = await scope.setVariable( scope, { field: fields_to_set[ data_i ], var_data, from_table });
        if ( row_used ) {
          row_used.times_used += 1;  // Esp important for `.there_is_another`
  ...
  getThereIsAnotherValue: async function ( scope, { row, from_table=true }) {
    ...
    */
    ...
    if ( row.times_used >= int_val ) {  // Loop will end
      return `False`;

    } else if ( value === `False` || value === `false` ) {  // Loop will end
      return `False`;
    ...
    } else if ( isNaN( int_val )) {  // Loop will end
      // console.warn?
      await scope.addToReport( scope, { key: `id`, value: `You must use a number or \`False\` in a \`.there_is_another\` row. The value you gave was ${ value }. This test will default to \`False\` to avoid an infinite loop.` });
      return `False`;
