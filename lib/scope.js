...
module.exports = {
  ...
  uploadFiles: async function ( scope, { handle, answer }) {
    ...
    */
    ...
    let dirs = globSync( `**/data/sources` );
    if ( dirs.length === 0 ) {
      await scope.addToReport( scope, {
        type: `warning`,
        value: `ALKiln couldn't find a "sources" directory in "${ process.cwd() }" where it could get files to upload.`
      });
      return false;
    }
    let dir = dirs[0];
    ...
    for ( let name of uploads_names ) {
      ...
      let matching_paths = globSync( `${ dir }/${ trimmed_name }` );
