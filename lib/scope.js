...
module.exports = {
  ...
  setFields: async function ( scope, {
    ...
    */
    ...
    if ( session_vars.get_debug() ) { await scope.page.waitForTimeout(2000); }
    ...
    sign_in: async function ( scope, {
      ...
      */
      ...
      if ( api_key_secret_name ) {
        ...
        let api_key_msg = `The API key called "${ api_key_secret_name }" is missing. Check for a typo in your test, workflow file, GitHub secret, or server config.`
        if ( api_key === undefined ) {
          ...
          reports.addToReport( scope, { type: `warning`, code: `ALK0200`,
            value: `ALKiln found the API key called "${ api_key_secret_name }", but it was empty. It had no text in it. Edit the value of the GitHub secret or server config variable called "${ api_key_secret_name }".` });
        } else {
          ...
          reports.addToReport( scope, { type: `warning`, code: `ALK0201`,
            value: `The server rejected the API key called "${ api_key_secret_name }". Does it exist on this server? Was it deleted somehow? You can always make a new one.` });
          }
  ...
  // Test that all interviews have been deleted
