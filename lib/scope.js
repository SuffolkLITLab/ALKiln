...
module.exports = {
  ...
  findFiles: async function( scope, {uploads_names}) {
  ...
  uploadFiles: async function ( scope, { handle, answer }) {
    ...
    */
    ...
    let paths = await scope.findFiles(scope, {uploads_names});
  ...
  steps: {
    ...
    compare_pdfs: async function (scope, {existing_pdf_path, new_pdf_path}) {
      let full_existing_path = await scope.findFiles(scope, {uploads_names: [existing_pdf_path]});
      let full_new_pdf_path = `${scope.paths.scenario}/${new_pdf_path}`;
      let diffs = await pdf_reader.compare_pdfs(full_existing_path[0], full_new_pdf_path);
      if (diffs) {
        let diff_str = diffs.reduce((err_str, part) => {
          if (part.added) {
            return err_str + `added "${ part.value }", `
          } else if (part.removed) {
            return err_str + `removed "${ part.value }", `
          } else {
            return err_str;
          }
        }, '');
        let msg = `The PDFs were not the same: The new PDF ${ diff_str }`;
        await reports.addToReport(scope, { type: `error`, value: msg });
        expect( diffs, msg ).to.be.empty;
      }
    },
