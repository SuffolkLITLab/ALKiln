  ...
  uploadFiles: async function ( scope, { handle, answer }) {
    ...
    */
    ...
    try {
      ...
      if (dirs.length > 1) {
        await scope.addToReport( scope, { type: `warning`, value: `You have multiple folders in the 'docassemble' folder (${ dirs }). These tests will use the first folder (${dirs[0]}).` });
      }
      pkg_path = `${docassemble_pkg_path}/${dirs[0]}`;
    } catch ( error_da ) {
    ...
    try {
      ...
      await scope.addToReport( scope, { type: `warning`, value: `You can store files to upload in '${dir}'. These tests could not find that folder.` });
      throw error_full;  // Most people will want to know the sources folder path;
    }
