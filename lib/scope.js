...
module.exports = {
    ...
    */
    ...
    for ( let guess of field.guesses ) {
      for ( let var_datum of var_data ) {
        ...
        let possible_row_names = [ var_datum.var ];
    ...
    for ( let var_datum of var_data ) {
      ...
      if ( from_table && var_datum.var.match( /\.target_number$/ )) {
        let special_row = {
          ...
          value: await scope.getThereIsAnotherValue( scope, { row: var_datum }),
      ...
      // Neutralize a `.there_is_another` row in a story table
        ...
        // Warn the developer not to use `.there_is_another` if needed.
        await scope.addToReport( scope, { key: `id`, value: `WARNING: The attribute \`.there_is_another\` is invalid in story table tests. Replace it with \`.target_number\` in your \`var\` and \`trigger\` columns. Set the \`value\` to the number of items in that list. See https://suffolklitlab.github.io/docassemble-AssemblyLine-documentation/docs/automated_integrated_testing#there_is_another. The row data is\n${ JSON.stringify( var_dataum.original )}` });
      }
    }
  ...
  getThereIsAnotherValue: async function ( scope, { row }) {
    ...
    * 
    * Sometimes this will be from `.there_is_another` from Steps
    * Sometimes this will be from `.target_number` from story tables
    * From story tables should always be a number, others should always be True or False
    * I think. This feels precarious, but I can't see another permuation.
    * 
    ...
    */
    ...
    } else if ( value.match( /(true|yes)/i )) {
      return `True`;
