...
module.exports = {
  ...
  getMatchingRows: async function ( scope, { field, var_data }) {
    ...
    `PLEASE READ THE COMMENTS IN THIS FUNCTION. It's complicated.`;
    ...
    */
      ...
      for ( let table_row of var_data ) {
        ...
        if ( this_row_matches_this_field ) {
          ...
          if ( !uses_proxies ) {
            ...
            // AVOID `.test()` - check the MDN. You can have unexpected results.
            uses_proxies = guess.var.match( proxies_regex );
          }

          let table_row_trigger_name = table_row.trigger;
          if ( table_row_trigger_name === `ALKiln: no trigger variable needed` ) {
    ...
    if ( trigger_might_be_needed && uses_proxies && rows_that_match_field_names.length > 0 ) {
      ...
      for ( let a_row_that_matches_this_field of rows_that_match_field_names ) {

        let table_row_trigger_name = a_row_that_matches_this_field.trigger;
        ...
        if ( !table_row_trigger_name ) {
          ...
          let row_as_story_table = await scope.convertToOriginalStoryTableRow( scope, { row_data: a_row_that_matches_this_field });
          ...
          rows_without_a_trigger_name.push( a_row_that_matches_this_field );
        } else {
          let triggers_match = await scope.matchNeutralizedQuotes( scope, { value1: page_trigger_name, value2: table_row_trigger_name });
          if ( triggers_match ) {
            rows_that_also_match_trigger_name.push( a_row_that_matches_this_field );
          } else {
            rows_with_conflicting_trigger_names.push( a_row_that_matches_this_field );
          }
        }
      }  // ends for each a_row_that_matches_this_field
