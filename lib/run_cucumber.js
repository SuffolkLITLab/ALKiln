...
function build_config( command_args ) {
  ...
  // Joining and splitting handles tags surrounded with `""` or those without `""`
  let tags = command_args[`_`].join(` `).split(` `);
  // If there is no tag expression, `tags` is ['']. Ignore those cases.
  // Length test may be unnecessary.
  ...
  *   manipluate the output. */
  ...
  fs.readFile(summary_file_name, 'utf8', (err, text) => {
    ...
    try {
      ...
      log.error({ type: `internal run`, code: `ALK0181`, pre: `This is sometimes fine. ALKiln was unable to read ${ log.unexpected_filename }, which might just mean there was an unrelated failure above.`, data: error });
    }
  });
})();
