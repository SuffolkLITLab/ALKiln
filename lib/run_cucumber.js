...
function build_config( command_line_args ) {
  ...
  let config = {};
  let command_line_args = process_command_line_args({ log });
  if ( command_line_args.tags ) { config.tags = command_line_args.tags; }
  */
  ...
  for ( let sources_path of sources_paths ) {
    const generators_prefix = `alkiln_random`;
    constrained_random_tests.create({ generators_prefix, sources_path });
    // Ignore feature files that we parsed ourselves
    let glob = fast_glob.sync(
      `${ sources_path }/**/*.feature`,
      { ignore: [`${ sources_path }/${ generators_prefix }*.feature`] }
    );
  ...
  let tags = command_line_args[`_`].join(` `).split(` `);
