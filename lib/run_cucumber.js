...
(async function main() {
  ...
  *   manipluate the output. */
  ...
  // TODO: Check what happens with a wrongly formatted table
  const exitCode = success ? 0 : `ALK1`;
  ...
  fs.readFile(summary_file_name, 'utf8', (err, text) => {
    ...
    try {
      ...
      log.info({ type: `internal run`, code: `ALK0182`, pre: `Something probably went wrong earlier. Look above for more details, but here's more info just in case: ALKiln was unable to write to ${ artifacts_path }/${ log.debug_log_file }.`, data: error });
    }
    try {
      ...
      log.info({ type: `internal run`, code: `ALK0181`, pre: `Something probably went wrong earlier. Look above for more details, but here's more info just in case: ALKiln was unable to write to ${ artifacts_path }/${ log.unexpected_filename }.`, data: error });
    }
  });
})();
