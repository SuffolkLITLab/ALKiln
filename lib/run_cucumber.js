...
function build_config() {
  ...
  for ( let sources_path of sources_paths ) {
    let glob = fast_glob.sync(
      ...
      // Ignore "generator" feature files that we used to generate actual tests
...
async function main() {
  ...
  *   manipulate the output. */
  ...
  /** Write to files.
   *
   * Note: Avoid adding to the purely report-focused files. Those are files
   *    where authors can focus on the info we give them, which they often miss
   *    in the GitHub action.
   */

  if ( success ) {
    log.info({ code: `ALK0054`, context: `post_run`, },
      `Test process exited without error.`
    );
  } else {
  ...
  fs.readFile(summary_file_name, 'utf8', (read_error, text) => {
    if ( read_error ) {
      ...
      log.warn({ code: `ALK0056`, context: `post_run error`, },
        `Failed to read the report of the 3rd party library, cucumberjs. There may be more information above or below.`, read_error
      );
      return;
    }
    ...
    try {
      ...
      if ( temp_debug_unexpected_content !== `` ) {
        ...
        try {
          ...
          /* Missing "unexpected results" file is ok */
        }

        // TODO: Is this meant to write to the debug file or is this debug info
        // really supposed to be in the "unexpected results" file? Or are these
        // comments old?
...
// 3. Prep the sources paths to find .feature files
