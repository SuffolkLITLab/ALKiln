...
// TODO: Remove after merging Log class
const files = require('./utils/files');

const SUMMARY_FILENAME = `cucumber-report.txt`;


function build_config( cli_args ) {
  /** Use the command line arguments to build the cucumberjs config object.
   * 
   * @param {string?} - Arguments passed to the command line.
   * 
   * @returns {object} - cucumberjs config object
   * */
  // ========== TODO: Abstract after merging Log class PR
  ...
  for ( let sources_path of sources_paths ) {
    // constrained_random_tests.create({ sources_path, logger: log });
    // Ignore feature files that we parsed ourselves
    let glob = fast_glob.sync(
      `${ sources_path }/**/*.feature`,
      { ignore: [`${ sources_path }/${ globals.generators_prefix }*.feature`] }
    );
  ...
  let tags = cli_args[`_`].join(` `).split(` `);
  ...
  *   manipulate the output. */
  ...
  if ( !one_dir_exists ) {
    ...
    log.throw({ code: `ALK0050`, context: `pre_run`,
      error: new ReferenceError(
        `ALKiln can't find your "sources" folder(s). See warnings and errors above.`
      ),
    });
  }
  ...
  if ( success ) {
    ...
    log.warn({ code: `ALK0055`, context: `post_run`, },
      `The tests ran into something unexpected. See warnings and errors above.`
    );
  }
   ...
   * 1. Generate files into the sources folder
   * 2. copy the randomly generated files into the artifacts folder
   * 3. MVP: delete the randomly generated files from the sources folder
   * 4. post MVP: let the author keep the randomly generated tests in the
   *    sources folder and make a PR with them
   * */

  /**
   * Loop through sources folders
   *   If there are generated files in the generated files folder(s)
   *     delete that folder from the sources folders where we found them
   * */
  ...
  fs.readFile(summary_file_name, 'utf8', (read_error, text) => {
    ...
    try {
      ...
      if ( exitCode === 50 ) {
        ...
        log.debug({ icon: `🔎`, code: `ALK0216`, context: `post_run`, level: `warn` },
          `\n\n`, text
        );
      } else {
...
≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡\n\n`
      ...
      let msg = `Something probably went wrong earlier. See warnings and errors above. Skipped writing to ${ log.path }/${ log.debug_log_filename }.`;
      try {
