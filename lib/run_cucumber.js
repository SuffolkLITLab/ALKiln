...
function generate_random_input_file_str( generator_file_contents ) {
   ...
   * for the second, do we end up with 4 random tests?
   *
   ...
   * 1. Turn each Scenario into a set of Scenarios. And those strings to the one
   *    file.
   * */

  // let num_tests = generator_file_contents.match(/number: (.*)/)[1];
  // let url = generator_file_contents.match(/url: (.*)/)[1];
  // let ids = generator_file_contents.match(/ids: (.*)/)[1];
  // let opts = generator_file_contents.match(/options:\n((.|\n)*)/)[1];
  ...
  for ( let scenario of scenarios ) {
    ...
    // let { top, constraints, bottom } = get_scenario_parts({ scenario });
  }

  return `Feature: 1 random test(s)

# This file was generated from the sources file "<name of file>.feature"
# TODO: See docs at <link>

@random_generated @alk_1
Scenario: Random test #1
  Given I start the interview at "test_kickout.yml"
  And I get to the question id "make choice" with this data:
    | var | value |
...
function get_feature_file_parts({ generator_file_contents }) {
  ...
  if ( !found_a_scenario ) {
    // TODO: Send message to the user that there were no Scenarios
