...
(async function main() {
  ...
  if (tags.length > 0) {
    ...
    if (tags.every((val) => val.startsWith("@"))) {
      tags_start = tags.join(' or ');
    }
    // If we get a list of seemingly no tags, assume they meant to use tags
    else if (tags.every((val) => !val.startsWith("@"))) {
      tags_start = tags.map((val) => "@" + val).join(' or ');
    }
    // If it's a mix of tags and no tags, it's probably a boolean expression!
    else {
      tags_start = tags.join(' ');
    }

    if ( tags.includes( `@al_language` )) {
      // User's messing with languages. Leave it alone.
      provided_config.tags = tags_start;
    } else {
      // Avoid testing any non-default languages
      provided_config.tags = `(${ tags_start }) and not @al_language`
    }

  } else {
    // Avoid testing any non-default languages
    provided_config.tags = `not @al_language`;
  }
