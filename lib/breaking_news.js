...
function break_the_news({
  ...
  let articles_data = [
    {
     ...
     // * node lib/breaking_news.js --alkiln_desired_version=5.15.0 --alkip_version=1.3.1
      ...
      body: [`Install ALKiln <span class="version">5.15.0</span> or above on your server to create the new <a target="_blank" href="https://assemblyline.suffolklitlab.org/docs/components/ALKiln/writing/#constrained_random">constrained randomized test generator table</a>. An author can make a generator ".feature" file. In that file, an author can write a table that contains many possible interview answers. ALKiln can generates many tests by picking random answers from that table. ALKiln then saves and runs those tests. Follow the instructions in the linked documentation to create a generator ".feature" file. You probably shouldn't be seeing this notification, though. This notification is just for test purposes. Find your nearest developer and report this to them at once! And have a nice day.`],
      relevant: (
        ...
        && (
          (
            ...
            semver_a_less_recent_than_b( alkip_semver, [1, 4, 0] )  // ex: 1.3.0
          )
        )
      ),
      log_code: `ALK0280`,
    ...
    },
    {
      ...
      body: [`Install ALKilnInThePlayground <span class="version">1.3.1</span> or above on your server to get updates about ALKiln and ALKilnInThePlayground. Then you can get notifications here about any mismatches between your ALKiln version and your ALKilnInThePlayground version, new features, bug fixes, and more. You probably shouldn't be seeing this notification, though. This notification is just for test purposes. Find your nearest developer and report this to them at once! And have a nice day.`],
  ...
  // Front page
  let banner_text = `<div class="banner"><span class="smaller_right">BREA</span><span>K</span><span class='bigger_left bigger_right'>I</span><span class="bigger_left">N</span><span class="smaller_left">G</span> <span class="bigger_right">N</span>E<span>W</span><span class="smallest_left">S!!!</span></div>`;
  if ( alkiln_desired_version ) {
    // Insert (is that the name of a last-minute page?)
    banner_text = `<div class="banner">EXTRA! EXTRA!</div>`;
  }
  ...
  let {
    ...
    version,
    date
  } = get_relevant_articles_parts({ articles_data });
  ...
  let news = [
    `<div class="outer_limits">`, [
      `<div class="news layout">`, [
        ...
        ...get_flag_masthead_parts({ publication_volume, version, date }),
...
function get_relevant_articles_parts({ articles_data }) {
  ...
  let articles_oldest_first = articles_data.sort(( article1, article2 ) => {
    if ( article1.is_no_news_notification ) { return 1; }  // put this last
    return (new Date( article1.date_written ) - new Date( article2.date_written ));
  });
  for ( let article of articles_oldest_first ) {
  ...
  let most_recent_article = articles_used.at(-1);
  let publication_volume = most_recent_article.publication_volume;
  let version = most_recent_article.version_at_publication;
  let date = most_recent_article.date_written;
  ...
  return {
    ...
    publication_volume, version, date
  };
};
...
function get_flag_masthead_parts({ publication_volume, version, date }) {
...
function get_css_and_svg() {
  ...
  return [ `<style>
    ...
    :root {
      ...
      note: "Vars";
      --ink-default: dimgray;
      --ink-light-roc: color-mix(in srgb, var(--ink-default), white 5%);
      --ink-paragraph: rgb(80, 80, 80)
    ...
    .subhead {
      note: "Font stuff";
      font-family: "RockDown", var(--default-font-family);
      font-weight: bold;
    }
    ...
    .flag_masthead {
      ...
      justify-content: space-between;
      padding-top: .1em;
      border-top: 1px solid var(--ink-default);
      border-bottom: 1px solid var(--ink-default);
    }
    ...
    a:not(:visited) {
      color: color-mix(in srgb, var(--ink-default), blue 40%);
    }
    ...
    .article .version {
      ...
      padding-left: .15em;
      padding-right: .15em;
      color: color-mix(in srgb, var(--ink-default), black 20%);
    }
