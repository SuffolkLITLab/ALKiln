...
When(/I set the name of ?(?:the var(?:iable)?)? "([^"]+)" to "([^"]+)"/, async (var_base, name_str) => {
 ...
 // expect(name_parts.length, 'input less than 4 names').to.be.lessThan(4);
  if(name_parts.length > 4) {
    await scope.addToReport(scope, { type: `warning`, value: `There were too many inputs in the name field` });
    name_parts = name_parts.slice(0, 4);
  }

  name_parts = name_parts.slice(' ');
