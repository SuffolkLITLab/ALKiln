...
When(/I set the name of ?(?:the var(?:iable)?)? "([^"]+)" to "([^"]+)"/, async (var_base, name_str) => {
  ...
  */
  ...
  if(name_parts.length > 4) {
    await scope.addToReport(scope, { type: `warning`, value: `The name "${ name_str }" has more than 4 parts, but 4 is the maximum allowed. The test is using the first 4 parts.` });
