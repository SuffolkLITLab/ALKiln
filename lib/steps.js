...
Then(/of two interviews, I delete one __internal__/i, async () => {
  ...
  let internal_keys = Object.keys( interviews );
  expect(
    internal_keys.length,
    `Of 2 interview sessions, ALKiln only saved ${ internal_keys.length }`
  ).to.equal( 2 );

  // Check that both interviews exist on the server. 1 √ 2 √
  let existing_interviews = await da_api.get_interviews({ interviews });
  let existing_keys = Object.keys( existing_interviews );
  expect(
    existing_keys.length,
    `2 interviews should be on the server. Instead: ${ existing_keys.join(', ') }`
  ).to.equal( 2 );

  // Delete one. 1 x 2 √
  let to_delete = interviews[ internal_keys[ 0 ]];
  await da_api.quietly_try_to_delete_interviews({
    interviews: { [internal_keys[ 0 ]]: to_delete }
  });
  ...
  // 1 x 2 √
  let after_deletion = await da_api.get_interviews({ interviews });
...
// Then(/there are no interview_vars saved __internal__/i, async () => {
//   /** Test that no interviews were saved internally */
//   // Check the number of interviews
//   let interviews = scope.scenarios.get( scope.scenario_id ).interviews;
//   let keys = Object.keys( interviews );
//   expect(
//     keys.length,
//     `0 interviews should exist, but at least one does: ${ keys.join(', ') }`
//   ).to.equal( 0 );
// });
...
After(async function(scenario) {
  ...
  try {
    let interviews = scope.scenarios.get( scope.scenario_id ).interviews;
    await da_api.quietly_try_to_delete_interviews({ interviews });
    let existing_interviews = await da_api.get_interviews({ interviews });
    let existing_keys = Object.keys( existing_interviews );
    ...
    if ( existing_keys.length > 0 ) {
      log.debug({ type: `internal warning`, code: `ALK0197`,
        pre: `${ existing_keys.length } interviews were not deleted: ${ existing_keys.join( ', ' ) }`
      });
    }
  } catch ( error ) {
    // Deleting interviews is nice, but not non-critical. Don't bother the
    // user with it. Don't want to cover up a more meaningful failure (like
    // going to the wrong file). If it becomes a problem we can track it down later.
    // Consider add "internal" to docs: "If you see this, make an issue in ALKiln with your debug/verbose log"
    log.debug({ type: `internal warning`, code: `ALK0198`, pre: `Error while cleaning up interviews.`, data: error });
  }
