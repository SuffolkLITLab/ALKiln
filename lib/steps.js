...
BeforeStep(async function(step) {
	// Discuss: In the report, add a way for the reports to add "✔" or "✖" to the
	//    start of a Step after it's been completed, as cucumber already does. Add
	//    a 'step' type and additional success/failure information, though that
	//    may be challenging.
	reports.addToReport(scope, { type: `row`, value: step.pickleStep.text });
	reports.indent();
});
...
Given(/I start the interview at "([^"]+)"/i, { timeout: -1 }, async file_name => {
	// √
	...
	let custom_download_behavior_timeout = new Promise(async (resolve, reject) => {
		let page_timeout = setTimeout(function() {
			reject(`🤕 ALK0057 ERROR: Took too long to set download behavior.`);
		}, scope.timeout);
	...
	let custom_afterStep_timeout = new Promise(async (resolve, reject) => {
		let page_timeout = setTimeout(function() {
			reject(`\`afterStep()\` took too long.`);
		}, scope.timeout);
		// self-invoke an anonymous async function so we don't have to
		// abstract the details of resolving and rejecting.
		(async function() {
...
Given(/I (?:sign|log) ?(?:in)?(?:on)?(?:to the server)? with(?: the email)? "([^"]+)",?(?: and)?(?: the password)? "([^"]+)"(?: SECRETs)?(?:,?(?: and)?(?: the API key)? "([^"]+)")?/i, { timeout: -1 }, async (email, password, api_key) => {
...
When(/I am using an? "([^"]+)"/i, async device => {
...
Given(/the max(?:imum)? sec(?:ond)?s for each step (?:in this scenario )?is (\d+)/i, async seconds => {
	...
	if (scope.page) {
		await scope.page.setDefaultTimeout(scope.timeout);
	}
});
...
//#####################################
//#####################################
const INCLUDING_QUOTES_REGEX = /"([^"]*)"/g;
When(/I? ?gets? to ?.*?(".*").*?with.*?(?:this data:)?/i, { timeout: -1 }, async (target_ids, raw_var_data) => {
	...
	// have non-button fields often don't have a var assigned to the 'continue' button.
	// It is optional for the table to use all the vars and some vars may be used
	// multiple times.
	...
	let ids_as_list = Array.from(target_ids.matchAll(INCLUDING_QUOTES_REGEX), match => {
		return match[1];
	});
	let sanitized_ids = ids_as_list.map(risky_id => {
		return scope.to_docassemble_sanitized_id(risky_id);
	});

	let supported_table = await scope.normalizeTable(scope, { raw_var_data });

	let id = await scope.get_page_id(scope);
	let id_matches = id && sanitized_ids.includes(id);
	...
	while (!id_matches) {
		let custom_timeout = new Promise(function(resolve, reject) {
			...
			let on_page_timeout = async function() {
				let non_reload_report_msg = `The target id or ids were ${target_ids}, but it took ` + `too long to try to reach a valid id (over ${scope.timeout / 1000 / 60} min). The ` + `test got stuck on "${id}".`;
				let error = non_reload_report_msg;
				await scope.handle_page_timeout_error(scope, {
					non_reload_report_data: {
						code: `ALK0060`,
			...
			(async function() {
				try {
					...
					log.debug({ code: `ALK0061`, context: `table` }, `Rows, including special rows:`, ensured_var_data);
					...
					// Discuss: use Log class to throw this error?
					if (error.was_found) {
						await scope.throwPageError(scope, { id: id });
					}
					resolve();
				} catch (err) {
					let err_msg = `ALKiln ran into an error when it tried to answer fields on the page with the question id "${id}". The text above might have more information.`;
		...
		// Prep for next loop
		id = await scope.get_page_id(scope);
		id_matches = id && sanitized_ids.includes(id);
	...
	let sorted_rows = supported_table.sort(function(row1, row2) {
		if (row1.report_str < row2.report_str) {
			return -1;
		}
		if (row1.report_str > row2.report_str) {
			return 1;
	...
	reports.addToReport(scope, { type: `step info`, code: `ALK0064`, value: `${' '.repeat(4)}And I get to any of the question ids ${target_ids} with this data:` });
...
When(/I wait ?(?:for)? (\d*\.?\d+) seconds?/i, { timeout: -1 }, async seconds => {
	 ...
	 */
	return wrapPromiseWithTimeout(scope.afterStep(scope, { waitForTimeout: parseFloat(seconds) * 1000 }), scope.timeout);
});

Then(/I take a (?:screenshot|pic) ?(?:named "([^"]+)")?/i, { timeout: -1 }, async name => {
	/* Download and save a screenshot. `name` does not have to be
	 * unique as the filename will be made unique. */
	return wrapPromiseWithTimeout(scope.steps.screenshot(scope, name), scope.timeout);
});

/* Here to make writing tests more comfortable. */
When(/I do nothing/i, async () => {
...
Then(/the (?:question|page|screen) id should be "([^"]+)"/i, { timeout: -1 }, async question_id => {
	...
	// Wrong question id
	let no_match_msg = `The question id was supposed to be "${question_id}"` + `, but it is actually "${id}".`;
	if (!id_matches) {
...
Then(/I ?(?:should)? see the phrase "([^"]+)"/i, async phrase => {
	...
	if (!bodyText.includes(phrase)) {
		reports.addToReport(scope, { type: `error`, code: `ALK0072`, value: msg });
	}
...
Then(/I should not see the phrase "([^"]+)"/i, async phrase => {
	...
	if (bodyText.includes(phrase)) {
		reports.addToReport(scope, { type: `error`, code: `ALK0073`, value: msg });
	}
...
Then(/an element should have the id "([^"]+)"/i, async id => {
	// √
	const element = await scope.page.$('#' + id);

	let msg = `No element on this page has the ID "${id}".`;
	if (!element) {
		reports.addToReport(scope, { type: `error`, code: `ALK0074`, value: msg });
	}
...
Then(/I (don't|can't|don’t|can’t|cannot|do not) continue/i, async unused => {
	// √
	...
	if (scope.navigated) {
		reports.addToReport(scope, { type: `error`, code: `ALK0075`, value: msg });
	}
...
Then(/I will be told an answer is invalid/i, async () => {
	// √
	...
	if (!was_found) {
		reports.addToReport(scope, { type: `error`, code: `ALK0076`, value: no_error_msg });
	}
	...
	if (!was_user_error) {
		reports.addToReport(scope, { type: `error`, code: `ALK0077`, value: not_user_error_msg });
	}
...
Then(/I arrive at the next page/i, async () => {
	// √
	/* Tests for detection of url change from button or link tap.
	 *    Can other things trigger navigation? Re: other inputs things
	 *    like 'Enter' acting like a click is a test for docassemble */
	...
	if (!scope.navigated) {
		reports.addToReport(scope, { type: `error`, code: `ALK0078`, value: msg });
	}
...
Then(/I should see the link "([^"]+)"/i, async linkText => {
	// √
	...
	if (!link) {
		reports.addToReport(scope, { type: `error`, code: `ALK0079`, value: msg });
	}
...
Then(/I should see the link to "([^"]+)"/i, async url => {
	// √
	...
	if (!link) {
		reports.addToReport(scope, { type: `error`, code: `ALK0080`, value: msg });
	}
	expect(link, msg).to.exist;
...
Then(/the "([^"]+)" link leads to "([^"]+)"/i, async (linkText, expected_url) => {
	// √
	...
	if (actual_url !== expected_url) {
		reports.addToReport(scope, { type: `error`, code: `ALK0081`, value: msg });
	}
...
Then(/the "([^"]+)" link opens in (a new window|the same window)/i, async (linkText, which_window) => {
	// √
	let [link] = await scope.page.$$(`xpath/.//a[contains(text(), "${linkText}")]`);
	...
	let hasCorrectWindowTarget = (should_open_a_new_window && opens_a_new_window) || (!should_open_a_new_window && !opens_a_new_window);

	let msg = `The link "${linkText}" does NOT open in ${which_window}.`;
	if (!hasCorrectWindowTarget) {
		reports.addToReport(scope, { type: `error`, code: `ALK0082`, value: msg });
	}
...
Then(/(?:the )?text(?: in)?(?: for)?(?: the)?(?: JSON)?(?: var)?(?:iable)?(?: )?"([^"]+)" should be/i, async (var_name, expected_value) => {
	...
	if (actual_value !== expected_value) {
		reports.addToReport(scope, {
			type: `error`,
			code: `ALK0083`,
			value: `The variable "${var_name}" was different from the expected ` + `value on the "${scope.page_id}" screen.` + `The expected values was\n` + expected_value + `\nwhile the actual value was\n` + actual_value,
		});
	}
...
Then(/I get the(?: page's)?(?: JSON)? var(?:iable)?s and val(?:ue)?s/i, { timeout: -1 }, async () => {
	 ...
	 */
	return await wrapPromiseWithTimeout(scope.steps.add_json_vars_to_report(scope), scope.timeout);
});
...
Then(/I expect the baseline PDF "([^"]+)" and the new PDF "([^"]+)" to be the same/i, async (existing_pdf_path, new_pdf_path) => {
	...
	return await wrapPromiseWithTimeout(scope.steps.compare_pdfs(scope, { existing_pdf_path, new_pdf_path }), scope.timeout);
});
...
Then(/the "([^"]+)" link opens a working page/i, { timeout: -1 }, async linkText => {
	return wrapPromiseWithTimeout(scope.steps.link_works(scope, linkText), scope.timeout);
});
...
When(/I check the page for (?:accessibility|a11y) issues/i, { timeout: -1 }, async () => {
	let { passed, axe_filepath } = await wrapPromiseWithTimeout(scope.checkForA11y(scope), scope.timeout);
...
// -------------------------------------
// --- RANDOM INPUT ---
When(/I answer randomly for at most ([0-9]+) (?:page(?:s)?|screen(?:s)?)/i, { timeout: -1 }, async max_pages_str => {
	...
	while (buttons_exist && curr_page_num < max_pages) {
		...
		let no_content_err = `The interview seemed to run into an error page` + ` after the page with the question id "${id}".` + ` See the picture of the page in "${random_input_path}".`;
		...
		let custom_timeout = new Promise(async function(resolve, reject) {
			...
			let page_timeout_id = setTimeout(async function() {
				...
				if (!has_content) {
					...
					let timeout_message = `The page with an id of "${id}" took too long` + ` to fill with random answers (over ${scope.timeout / 1000 / 60} min).`;
			...
			(async function() {
				try {
					...
					if (!has_content) {
						...
						let err_msg = `The page with the question id "${id}" errored when trying to fill` + ` it with random answers: ${error.name}`;
...
When(/I set the var(?:iable)? "([^"]+)" to "([^"]+)"/i, { timeout: -1 }, async (var_name, answer) => {
...
When(/I set the var(?:iable)? "([^"]+)" to\s*$/i, { timeout: -1 }, async (var_name, answer) => {
...
Then(/I upload "([^"]+)" to "([^"]+)"/i, { timeout: -1 }, async (filenames, var_name) => {
	...
	return wrapPromiseWithTimeout(scope.steps.set_regular_var(scope, var_name, filenames), scope.timeout);
});

// -------------------------------------
// --- OTHER UI STEPS ---

When(/I tap the defined text link "([^"]+)"/i, { timeout: -1 }, async phrase => {
	/** Tap a link that doesn't navigate. Depends on the language of the text. */
	// `page` timeout will deal with custom timeout
	await scope.steps.tap_term(scope, phrase);
});

Then(/^I sign$/i, { timeout: -1 }, async () => {
	return wrapPromiseWithTimeout(scope.steps.sign(scope), scope.timeout);
});

Then(/I sign with the name "([^"]+)"/i, { timeout: -1 }, async name => {
	/* Signs with the string argument */
	return wrapPromiseWithTimeout(scope.steps.sign(scope, name), scope.timeout);
});

When(/I download "([^"]+)"$/i, { timeout: -1 }, async filename => {
	...
	return wrapPromiseWithTimeout(scope.steps.download(scope, filename), scope.timeout);
});
...
When(/I set the name of ?(?:the var(?:iable)?)? "([^"]+)" to "([^"]+)"/i, { timeout: -1 }, async (var_base, name_str) => {
	 ...
	 */
	return wrapPromiseWithTimeout(scope.steps.set_al_name(scope, var_base, name_str), scope.timeout);
});

When(/I set the address of ?(?:the var(?:iable)?)? "([^"]+)" to "([^"]+)"/i, { timeout: -1 }, async (var_base, address_str) => {
	 ...
	 */
	return wrapPromiseWithTimeout(scope.steps.set_al_address(scope, var_base, address_str), scope.timeout);
});
...
//#####################################
//#####################################
// Messages from ALKiln to author
//#####################################
//#####################################

const LOG_CODE_REGEX = /ALKiln\d\d\d\d/i;
Given(/ALKiln warns the author/i, async warnings_text => {
	/** Create a warning message for the author files they wrote to generate
	 *    constrained random answers tests.
	 *
	 * @param {str} warnings_text - Warning text generated by ALKiln
	 * */
	reports.addToReport(scope, { type: `as_is`, value: warnings_text });
});
...
Given(/the final Scenario status should be "([^"]+)"/i, async expected_status => {
	/** WARNING: If a Step will fail, this Step must come first.
	 *
	 * TODO: Status "A" ambiguous (internal)
	 *
...
Then(/of two interviews, I delete one __internal__/i, async () => {
	...
	expect(interview_keys.length, `Of 2 interview sessions, ALKiln only saved ${interview_keys.length}`).to.equal(2);
	...
	expect(visible_keys.length, `2 interviews should be on the server. Instead: "${visible_keys.join(', ')}"`).to.equal(2);
	...
	expect(keys_after_deletion.length, `1 interviews should be on the server: ${keys_after_deletion.join(', ')}`).to.equal(1);
...
Then(/I fail to delete (\d) detected interview(?:s)? and get no error __internal__/i, async num => {
	...
	let interviews = scope.scenarios.get(scope.scenario_id).interviews;
	expect(Object.keys(interviews).length, `The test should have saved ${num} interview session(s)`).to.equal(num);

	// There should be no error about an unauthorized user
	let api_keys = scope.scenarios.get(scope.scenario_id).api_keys;
	let visible_interviews = await da_api.get_interviews({ interviews, api_keys });
	let visible_keys = Object.keys(visible_interviews);
	expect(visible_keys.length, `User should detect 0 interviews on the server. Instead: "${visible_keys.join(', ')}"`).to.equal(0);
...
AfterStep(async function({ result }) {
	...
	if (result.status === `PASSED`) {
		...
		log.stdout({ records_only: true }, `F`);
	} else if (result.status === `UNDEFINED`) {
		log.stdout({ records_only: true }, `U`);
	} else if (result.status === `SKIPPED`) {
		log.stdout({ records_only: true }, `-`);
	} else if (result.status === `PENDING`) {
		log.stdout({ records_only: true }, `P`);
	}
...
After(async function(scenario) {
	...
	if (scenario.result.message) {
		log.debug({ code: `ALK0091`, context: `scenario` }, scenario.result.message);
	}
	...
	if (changeable_test_status !== `PASSED`) {
		log.debug({ code: `ALK0094`, level: `warn` }, `Non-passed status occurred while running test: ${changeable_test_status}`);
	...
	if (scope.expected_in_report && scope.expected_in_report.length > 0) {
		let report_includes_all_expected_strings = await scope.reportIncludesAllExpected(scope, { expected: scope.expected_in_report });
	...
	if (scope.expected_not_in_report && scope.expected_not_in_report.length > 0) {
		let all_prohibited_strings_were_absent = await scope.reportDoesNotInclude(scope, { not_expected: scope.expected_not_in_report });
		if (!all_prohibited_strings_were_absent) {
			changeable_test_status = `FAILED`;
		}
		// Reset report values no matter what so they don't mess up future scenarios
		scope.expected_not_in_report = null;
	...
	if (changeable_test_status === `PASSED` || changeable_test_status === `FAILED`) {
		if (changeable_test_status === scope.expected_status) {
			...
			let msg = `Test ${changeable_test_status}. ` + `Expected status: ${scope.expected_status}. ` + `See the report for details.`;
			let code = `ALK0099`;
			expected_status_err_msg = log.debug({ code: code, level: `error` }, msg);
		}
	}
	...
	if (changeable_test_status !== `FAILED`) {
		...
		log.debug({ code: force_failure_code, level: `error` }, force_failure_msg);
	...
	if (session_vars.get_origin() !== `playground`) {
		try {
			...
			if (visible_keys.length > 0) {
				...
				log.debug({ code: `ALK0197`, context: `internal`, level: `warn` }`The API key can see ${visible_keys.length} interviews that were not deleted:`, visible_keys);
			}
		} catch (error) {
			// Deleting interviews is nice, but not non-critical. Don't bother the
			// user with it. Don't want to cover up a more meaningful failure (like
			...
			log.debug({ code: `ALK0198`, context: `internal`, level: `warn`, error }, `Cleaning up interviews incomplete, but unimportant.`);
		}
	}

	// Collect all the relevant special errors
	let internal_errors = [];
	if (!expected_status_was_met) {
		internal_errors.push(expected_status_err_msg);
	}
	if (!signout_succeeded) {
		internal_errors.push(signout_error_msg);
	}
	...
	if (need_to_force_failed_status) {
		...
		log.debug({ code: `ALK0203`, context: `internal`, level: `warn` }, `There was no need to force failed status and yet there are error messages.`, internal_errors);
	}

	// Find the race condition.
	log.debug({ code: `ALK0100`, context: `scenario` }, `Scenario After() message:`, scenario.result.message);
});
