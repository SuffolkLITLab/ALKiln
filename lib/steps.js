...
BeforeStep(async function ( step ) {
  // TODO: Add a way for the reports to add "✔" or "✖" to the start of a Step
  //    after it's been completed, as cucumber already does. Add a 'step' type
  //    and additional success/failure information, though that may be
  //    challenging
...
// with(?:.*):?
// ^(?:the user|I)? gets? to ?.*?(".*") ?with .*?
// .*gets? to ?.*?(".*") ?with.*?
// More readable?:
// I? ?gets? to ?.*?(".*") ?with.*?(?:this data:)?
// get to + stop at (?:gets? to|stops? at)
// `before "a", "b", "c" and after` -> `"a", "b", "c"`
// Impossible with cucumber `before "a", "b", "c" and after` -> `["a", "b", "c"]`
const INCLUDING_QUOTES_REGEX = /"([^"]*)"/g;
When(/I? ?gets? to ?.*?(".*").*?with.*?(?:this data:)?/, {timeout: -1}, async (target_ids, raw_var_data) => {
  ...
  * Iterate in here to keep timeout reasonable */
  ...
  let ids_as_list = Array.from(
    target_ids.matchAll( INCLUDING_QUOTES_REGEX ),
    ( match ) => { return match[ 1 ]; }
  );
  ...
  while ( !id_matches ) {
    let custom_timeout = new Promise(function( resolve, reject ) {
      ...
      let on_page_timeout = async function() {
        ...
        let non_reload_report_msg = `The target id or ids were ${ target_ids }, but it took `
        + `too long to try to reach a valid id (over ${ scope.timeout/1000/60 } min). The `
      ...
      (async function() {
        try {
          ...
          // TODO: use Log class to throw this error?
          if ( error.was_found ) { await scope.throwPageError( scope, { id: id }); }
          resolve();
        } catch ( err ) {
          let err_msg = `An error occurred when ALKiln tried to answer fields on question with the id "${ id }". See above for warnings and errors.`
 ...
 *
 * @param raw_var_data { [{ var, possible_values }] } List of table row objects
 * @param raw_var_data[i].var {str} Name of the variable to set
 * @param raw_var_data[i].possible_values {str} Format is ";;" separated
 *    possible values for the variable (lawyers use ";" grammatically)
 * */

/** Need a sentence for:
 * And I will go to the interview at the address ("")
 * And I will run (100) constrained random tests
 * And those tests should get to one of these page ids - [("lsdjf", "sldkjf")]
 * And I will get there with random choices from this data:
 *   | var | possible_values |
 *   | favorite_color | blue;; green;; red |
 *
 * Write files
 * 1. Make an object from the given constraints table (split by single _and_
 *    double quotes? Just double quotes? Document author _must_ use double
 *    quotes?)
 * 1. Make sure that it's not the same as previous objects
 * 1. Far in the future: Make sure it's not the same as a test the author
 *    already wrote (need to wrap cucumber for this to run all random tests
 *    last)
 * 1. Make a Scenario description using the iteration number
 * 1. Add to one big file (name includes datetime) for all these Scenarios
 * 1. Increment iteration number
 * 1. Leave them there to get run later
 *
 * Supposedly more performant to write one big file [^citation needed]. Not
 *    sure it matters that much yet. If we do want to optimize, maybe using
 *    a write stream directly is the way to go.
 *
 * Command:
 * I run (\d*) random tests for/at "interview.yml" that will reach one of these question/page ids (\[".*"]) with these constraints: (table)
 * (\d*) random at "interview.yml" will reach one/any of (\[.*]) with: (table)
 *
 * Given I generate 100 random tests
 * And I test the interview at ""
 * And I reach any of these question/page ids: ""
 * And I answer with/give these possible answers: (table)
 * I generate 100 random tests for the interview at "" to reach any of the question ids [] with these possible answers
 *
 */
...
const LOG_CODE_REGEX = /ALKiln\d\d\d\d/;
Given(/ALKiln warns the author about a generator problem with:/i, async ( warnings_text ) => {
