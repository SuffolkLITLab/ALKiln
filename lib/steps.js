...
When('I tap to continue', async () => {
  let [elem] = await scope.page.$x(`//*[@aria-labelledby="daMainQuestion"]//button[@type="submit"]`);
  await scope.tapElement( scope, elem );
});

// base64: https://github.com/nodejs/node/issues/6107#issuecomment-207177131
When('I tap the button that sets the variable {string} to {string}', async (var_name, value) => {
  let base64_name = Buffer(var_name, 'utf8').toString('base64').replace(/[^A-Za-z0-9]/g, '');
  let [elem] = await scope.page.$x(`//button[@name="${base64_name}" and @value="${value}"]`);
  await scope.tapElement( scope, elem );
});
...
When(/I set the var(?:iable)? "([^"]+)" to "([^"]+)"/, async (var_name, value) => {
  let base64_name = Buffer(var_name, 'utf8').toString('base64').replace(/[^A-Za-z0-9]/g, '');
  // console.log('base64_name:', base64_name);

  // Should catch inputs like radio and checkbox too, which will then be rejected
  let elem = await Promise.race([
    scope.page.waitForSelector(`button[name="${base64_name}"][value="${value}"]`),  // button
    scope.page.waitForSelector(`:not(button)[name="${base64_name}"]`),  // select
    scope.page.waitForSelector(`*[data-saveas="${base64_name}"] input`),  // text input or textarea
  ]);

  let type = await elem.evaluate( elem => { return elem.getAttribute('type') });

  if ( type === `checkbox` || type === `radio` ) {
    throw new TypeError(`You need the ${type} step, e.g. 'I tap the ${type} variable "some_dict" with the value "choice_value"'`);
  }

  let tagName = await elem.evaluate( elem => { return elem.tagName });
  // console.log( 'tagName:', tagName, 'type:', type );
...
When(/I tap the ?(?:checkbox|radio(?: button)?)? var(?:iable)? "([^"]+)"(?: with the value "([^"]+)")?/, async (var_name, value) => {
  /* Tap a checkbox or radio button. We need to be clear that we toggle them, not
  *    set them to a value. (radio can be treated like a checkbox, but not the other way around)
  * Other sentences came to mind, but this is the one that kept the
  *    variable name and value in an order consistent with the other
  *    variable step.
  */
  let base64_name = Buffer(var_name, 'utf8').toString('base64').replace(/[^A-Za-z0-9]/g, '');
  let tagName = await scope.page.$eval(`*[name="${base64_name}"`, elem => {return elem.tagName});
  let type = await scope.page.$eval(`*[name="${base64_name}"`, elem => {return elem.getAttribute('type')});
  // console.log( 'tagName:', tagName, 'type:', type );
  console.log( 'base64_name:', base64_name );
  ...
  if ( type === `checkbox` || type === `radio` ) {
    let elem;
    if ( value ) {
      [elem] = await scope.page.$x(`//input[@name="${base64_name}" and @value="${value}"]/../label`);
    } else {
      [elem] = await scope.page.$x(`//input[@name="${base64_name}"]/../label`);
    }
    ...
    console.warn( 'WARNING: This step is only for checkboxes or (carefully) radio buttons.' );
  }
});
