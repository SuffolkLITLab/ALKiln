...
Given(/I log ?(?:in)?(?:on)?(?:to the server)? with(?: the email)? "([^"]+)" and(?: the password)? "([^"]+)"(?: SECRETs)?/i, async ( email, password ) => {
  ...
  * 
  * Variations:
  * I log into the server with the email "USER1_EMAIL" and the password "USER1_PASSWORD" SECRETs
  * I log on with the email "USER1_EMAIL" and the password "USER1_PASSWORD" SECRETs
  * I log in with "USER1_EMAIL" and "USER1_PASSWORD"
  */
  // TODO: Implement cucumber v8 custom timeout once that's merged
