...
Given(/I start the interview at "([^"]+)"/, {timeout: -1}, async (file_name) => {  // √
  ...
  const browser = await scope.driver.launch({ headless: !session_vars.get_debug(), devtools: session_vars.get_debug() });
  const page = await browser.newPage();
  await page.goto(
    `https://apps-dev.suffolklitlab.org/interview?i=docassemble.playground204ALKilnCombobox:temp_test_combobox.yml`,
    { waitUntil: 'domcontentloaded', timeout: scope.timeout }
  );

  // dates
  const date1 = await page.$(`#c3RhcnRfZGF0ZQ`);
  // let html = await date1.evaluate( el => el.outerHTML );
  // console.log( `html:`, html );
  await date1.evaluate(( elem )=> {
    if ( !elem.disabled ) { elem.scrollIntoView(false); }
  });
  await date1.scrollIntoView(false);
  // await date1.focus();
  // await date1.type(`08/20/2023`);
  await scope.setText(scope, { handle: date1, answer: `08/20/2023` });
  await scope.afterStep(scope, { waitForShowIf: true });
  const date2 = await page.$(`#ZW5kX2RhdGU`);
  // let html2 = await date2.evaluate( el => el.outerHTML );
  // console.log( `html:`, html2 );
  await date2.evaluate(( elem )=> {
    if ( !elem.disabled ) { elem.scrollIntoView(false); }
  });
  await date2.scrollIntoView(false);
  // await date2.focus();
  // await date2.type(`08/21/2023`);
  await scope.setText(scope, { handle: date2, answer: `08/21/2023` });
  await scope.afterStep(scope, { waitForShowIf: true });

  // combobox
  const handle = await page.$(`#amlyYV91c2Vy`);
  // let html3 = await handle.evaluate( el => el.outerHTML );
  ...
  await scope.setText(scope, { handle, answer: `Something` });
  await scope.afterStep(scope, { waitForShowIf: true });
  //await scope.steps.screenshot( scope, 'foo' );
  // await handle.focus();
  // // On `keyup`, a hidden field gets updated (at least when it's typed manually)
  // await handle.type( `Something` );

  // Blurring will also update the hidden field
  // await scope.page.evaluate(el => {el.blur()}, handle);
  let val = await handle.evaluate( el => {
    // This is the field that gets updated on `keyup` and gets
    // its value submitted by the form
    let input = document.querySelector(`input[name="amlyYV91c2Vy"]`);
    return input.value;
  });
  console.log(`Hidden field value:`, val);
  if ( !val ) {
