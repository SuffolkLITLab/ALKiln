...
When(/I set the var(?:iable)? "([^"]+)" to "([^"]+)"/, async (var_name, value) => {
  /* Set any field using its variable name and (maybe?) its value */
  let base64_name = Buffer(var_name, 'utf8').toString('base64').replace(/[^A-Za-z0-9]/g, '');
  ...
  if (( tagName === `INPUT` && type === `text` ) || tagName === `TEXTAREA` ) {
    await scope.page.$eval(`*[name="${ base64_name }"]`, (el) => { el.value = ''});
    await scope.page.type( `*[name="${ base64_name }"]`, value );
  ...
  // Maybe only radio right now
  if ( tagName === `INPUT` && ( type === `radio` || type === `checkbox` )) {
    let [elem] = await scope.page.$x(`//input[@name="${base64_name}" and @value="${value}"]/../label`);
    await elem[ scope.activate ]();
    await scope.afterStep(scope, {waitForShowIf: true});
  }

  if ( tagName === `SELECT` ) {
    let [elem] = await scope.page.$x(`//select[@name="${base64_name}"]`);
