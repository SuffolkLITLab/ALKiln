...
And(/of two interviews, one is deleted __internal__/i, async () => {
  /** Test that an interview does get successfully deleted. This tests whether
   *    the API code is working correctly, not whether our `After()`
   *    code uses the API code correctly.
   */
  // test_interview_deletion

  // API test
  // Two interviews have been created. Check that they both exist.
  // Make sure we have two interviews in `scope.scenario.interviews` (two session ids)

  // Check that both session ids exist on the server
  // 1 √ 2 √
  ...
  // In After(), the other session should get deleted and there should
  // be no errors.
});
...
After(async function(scenario) {
  ...
  if ( changeable_test_status === `PASSED`
    ...
    if ( changeable_test_status === scope.expected_status ) {
      ...
      let msg = `Test failed. Expected status: ${scope.expected_status}. `
          + `ALKiln's assessed status: ${ changeable_test_status }. `
  ...
  try {
    ...
    if ( scenario.test_interview_deletion ) {

      // Assertions:
      // Does da_api.delete_interview() work? Did I delete one interview on the server?
      // Does the cleanup work?
      // clean_this_up()
      // Assert that cleanup worked. Did I delete the rest of the interviews on the server?

      // scenario.test_interview_deletion = false;
    } else {

      // clean_this_up()

      // // In a function somewhere else that we need to make a mock test for
      // // Mocks are useless!
      // // await quietly_try_to_delete_loaded_interviews();  // See below
      // for ( let id in scenario.interviews ) {
      //   let interview_vars = scenario.interviews[ id ];
      //   await da_api.delete_interview({ interview_vars });
      // }
      
      // for ( let id in scenario.interviews ) {
      //   let interview_vars = scenario.interviews[ id ];
      //   let fetched_interview = await server.get_interview({ interview_vars });
      //   if ( fetched_interview ) {
      //     throw;
      //   }
      // }

      // Always assert? If so, warn/info.

    }
  } catch ( error ) {
    // If this fails, I don't want the user to know about it. It's not critical.
    // "If you see this, make an issue in ALKiln with your debug/verbose log"

    // Reveal interview deletion errors? It feels better than mocking
    // x error = { type: `info`, code: `ALK0192`, pre: `Error while deleting the interview.`, data: error };
    // x errors.push(error);
    // Maybe put in report and check report at the end, just before throwing errors
    // Maybe this whole thing should be an error accumulator
    // Don't want to cover up a more meaningful failure (like going to the wrong file)
    log.debug({ type: `info`, code: `ALK0192`, pre: `Error while deleting the interview.`, data: error });
  }

  // Handle all the accumulated errors
  let errors = []
  if ( !expected_status_was_met ) { errors.push( expected_status_err_msg ); }
  if ( !signout_succeeded ) { errors.push( signout_error_msg ); }
  if ( errors.length > 0 ) {
