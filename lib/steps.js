...
When(/(some|all)(?: of)?(?: the(?:se)? var(?:iable)?s are used)?/, {timeout: -1}, async (how_many, raw_var_data) => {
  ...
  // TODO: Collect question ids in the order that they appear and log them at the end.
  ...
  while ( !id_matches ) {  // && remaining_vars.length > 0 ) {
    ...
    let promise = new Promise(function( resolve, reject ) {
        ...
        // self-invoke an anonymouse async function so we don't have to
        ...
        (async function( resolve, reject ) {
          ...
          try {
            ...
            clearTimeout( page_timeout );  // prevent hang at end of tests
            resolve( result );
          } catch (err) {
            clearTimeout( page_timeout );  // prevent hang at end of tests
            reject( err );
          }
          // return scope.processVar(scope, { how_many, var_data: remaining_vars, id });
        })( resolve, reject );
