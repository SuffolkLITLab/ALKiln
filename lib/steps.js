...
When(/(some|all)(?: of)?(?: the(?:se)? var(?:iable)?s are used)?/, {timeout: -1}, async (how_many, raw_var_data) => {
  ...
  * Iterate in here to keep timeout reasonable */
  ...
  // if reached the target page and all vars were used, categorical win. Needed?
  if ( remaining_vars.length === 0 ) { return true; }

  // If reached the target page, needed only some vars, and used up at least some vars, still a win. Needed?
