...
async function beforeScenario( scenario ) {
  // console.log('👁️ 111 start beforeScenario');
  ...
  if (!scope.browser) {
    if ( session_vars.get_origin() !== `local` ) {
      ...
      // console.log('👁️ 112 no browser, await new local browser');
      scope.browser = await scope.driver.launch({ args: ['--no-sandbox'] });
    } else {
      // console.log('👁️ 113 no browser, await new remote browser');
      scope.browser = await scope.driver.launch({ headless: !session_vars.get_debug(), devtools: session_vars.get_debug() });
    }
  }
  // console.log('👁️ 114 we have ensured browser exists, awaiting pages');
  // Clean up all previously existing pages
  for (const page of await scope.browser.pages()) {
    // console.log('👁️ 115 awaiting closing page');
    await page.close();
  }
  // console.log('👁️ 116 awaiting opening a new page');
  // Make a new page
  ...
  // console.log('👁️ 117 awaiting getting safe scenario name');
  ...
  // console.log('👁️ 118 got safe scenario name');
...
Given(
  ...
  */
  ...
  foo = scope.page
  // console.log(`⭐️ ${ JSON.stringify(scope.page, null, 2) }`)
  let result = await scope.steps.sign_in( scope, {
  ...
  // console.log(`🖊️ 115`);
  // await scope.take_a_screenshot(scope, {path:`${scope.paths.artifacts}/Temp-8.jpg`});
...
AfterStep(async function({ result }) {
  // TODO: Abstract this
  // await scope.take_a_screenshot(scope, {path:`${scope.paths.artifacts}/Temp-13.jpg`});
  // console.log(`🛑 100`);
...
async function afterScenario(scenario) {
  // console.log(`👀 1 ${ foo == scope.page } ${ JSON.stringify(scope.page, null, 2) }`);
  await scope.take_a_screenshot(scope, {path:`${scope.paths.artifacts}/Temp-30.jpg`});
  // Log errors
  ...
  // console.log(`👀 2`);
  await scope.take_a_screenshot(scope, {path:`${scope.paths.artifacts}/Temp-9.jpg`});
  ...
  if (changeable_test_status !== `PASSED`) {
    ...
    if ( scope.page ) {
      // console.log(`👀 3`);
      await scope.take_a_screenshot(scope, {path:`${scope.paths.artifacts}/Temp-10.jpg`});
      ...
      if ( !!scope.disable_error_screenshot ) {
        reports.addToReport(scope, {
          ...
          // information about a screen from getting out? For example, they have
          // proprietary info in the HTML itself.
          value: `For security, ALKiln will avoid creating a picture of the page for this error. It's possible a secret is being used on this screen. ALKiln will save the HTML, which omits the values in the fields.`
        });
      }
      // console.log(`👀 4 ${ foo == scope.page } ${ JSON.stringify(scope.page, null, 2) }`);
      await scope.take_a_screenshot(scope, {path:`${scope.paths.artifacts}/Temp-11.jpg`});
      ...
      // console.log(`👀 5 ${ foo == scope.page } ${ JSON.stringify(scope.page, null, 2) }`);
      await scope.take_a_screenshot(scope, {path:`${scope.paths.artifacts}/Temp-12.jpg`});
