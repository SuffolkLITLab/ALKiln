...
After(async function(scenario) {
  ...
  if ( changeable_test_status === `PASSED`
    ...
    if ( changeable_test_status === scope.expected_status ) {
      ...
      let code = `ALK0099`;
      log.debug({ type: `error`, code: code, pre: msg, })
      expected_status_err_msg = `🤕 ${ code } ERROR: ${ msg }`
    }
  }
  ...
  // If assessed status is FAILED, but result.status is something else
    ...
    log.debug({
      type: `error`,
      code: force_failure_code,
      pre: force_failure_msg
    });
  ...
  if ( session_vars.get_origin() !== `playground` ) {
    try {
      ...
      if ( visible_keys.length > 0 ) {
        ...
        log.info({ type: `internal warning`, code: `ALK0197`,
          pre: `The API key can see ${ visible_keys.length } interviews that were not deleted: ${ visible_keys.join( ', ' ) }`
        });
      }
    } catch ( error ) {
      // Deleting interviews is nice, but not non-critical. Don't bother the
      ...
      log.info({ type: `internal warning`, code: `ALK0198`, pre: `Error while cleaning up interviews.`, data: error });
    }
  }
