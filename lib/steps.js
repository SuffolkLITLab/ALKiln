...
Before(async (scenario) => {
  ...
  scope.safe_scenario_name = await scope.getSafeScenarioBaseFilename(scope, {scenario});
  // Add a date for uniqueness in case dev has accidentally copied a Scenario description
  let date = Date.now();
  scope.paths.scenario = `${ scope.paths.artifacts }/${ scope.safe_scenario_name }-${ date }`;
