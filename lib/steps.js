...
Given(/I start the interview at "([^"]+)"(?: in lang "([^"]+)")?/, {timeout: -1}, async (file_name, language) => {  // √
  ...
  while ( num_attempts < max_load_attempts ) {
    ...
    num_attempts++;
    let url = `${ await get_base_interview_url() }${ file_name }`;
    let timeout_message = `Failed to load "${ file_name }" at URL ${ url } after ${ num_attempts } tries. `
...
When(/I set the var(?:iable)? "([^"]+)" to secret "([^"]+)"/, { timeout: -1 }, async ( var_name, set_to_env_name ) => {
