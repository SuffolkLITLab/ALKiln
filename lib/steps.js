...
Before(async (scenario) => {
  ...
  scope.scenarios.get( scope.scenario_id ).api_keys = [ session_vars.get_dev_api_key() ];
...
Given(
  ...
  * I sign in with the email "USER1_EMAIL", the password "USER1_PASSWORD", and the API key "USER1_API_KEY"
  * I sign in with "USER1_EMAIL", "USER1_PASSWORD", "USER1_API_KEY"
  */
...
Then(/of two interviews, I delete one __internal__/i, async () => {
  ...
  let existing_interviews = await da_api.get_interviews({ interviews, api_keys });
...
Then(/as an anonymous user, I fail to delete an interview and get no error __internal__/i, async () => {
  /** Test that an interview passes even when unable to delete an interview.
   *    An API test
   */
  ...
  expect(
    ...
    `The test should have saved one interview session`
  ).to.equal( 1 );
  ...
  expect(
    existing_keys.length,
    `Anonymous user should detect 0 interviews on the server. Instead: "${ existing_keys.join(', ') }"`
  ).to.equal( 0 );
