...
After(async function(scenario) {
  ...
  if (scenario.result.status !== `PASSED`) {
    ...
    if ( scope.page ) {
      ...
      // error-scenario 1 description-endingPgID-3pm.png
      let scenario_filename = await scope.getSafeScenarioFilename( scope, { prefix: `error` });
      ...
      // error-pgID-3pm.png
      let screenshot_name = `error`;
      if ( scope.page_id ) {
        screenshot_name += `-${ scope.page_id }`;
      }
      screenshot_name += `-${ Date.now() }`;
      await scope.page.screenshot({
  ...
  // Add report to scenario folder after everything has been added to the report
