...
Given(/I start the interview at "([^"]+)"[ on ]?(.*)/, async (file_name, optional_device) => {  // √
  ...
  if (!scope.page) {
    ...
    await scope.page._client.send('Page.setDownloadBehavior', {
      behavior: 'allow',
      // downloadPath: path.resolve(__dirname, `downloads_${ scope.scenario_name }`),
      downloadPath: path.resolve( scope.docs_dir ),
    });
  }
...
When(/I tap the (button|link) "([^"]+)"/, async (elemType, phrase) => {  // √
  ...
  *    navigation, validation error, page error, document download, or just a click.
  * WARNING: Can only handle one download per scenario currently. */
  ...
  if (elem) {
    winner = await Promise.race([
      ...
      scope.page.waitForSelector('.alert-danger'),
      scope.page.waitForSelector('.da-has-error'),
      // The below is from https://stackoverflow.com/a/51423688
      // Wait for possible download
      // TODO: remove event listener
      scope.page.on('response', async response => {
        // If response has a file on it (TODO: Find way to store filename)
        // if (response._headers['content-disposition'] === `attachment;filename=${filename}`) {
        if ( response._headers['content-disposition'] && (response._headers['content-disposition']).includes(`attachment;filename=`) ) {
          // Remove the event listener since we might not be closing the browser
          scope.page.removeEventListener('response', arguments.callee);  // Not sure this will work...
          // Log the size (for the sake of our sanity)
          console.log('Size del header: ', response._headers['content-length']);
          // TODO: Get current size of directory (in case we'll be downloading multiple documents)
          // Watch event on download folder or file
          // fs.watchFile(scope.docs_dir, function (curr, prev) {
          await fs.watchFile(scope.docs_dir, function (curr, prev) {
            ...
            if (parseInt(curr.size) === parseInt(response._headers['content-length'])) {
              // browser.close();
              this.close();  // correct if not closing browser? closing fs?
              return true;
            }
          });  // ends fs.watchFile
