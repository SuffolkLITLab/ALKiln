...
Given(/^(?:I always have this data|the below is always true|this is always true):/i, async (raw_var_data) => {
  /* To be used in `Background`. Allow pre-creating a story table for Scenarios. */
  scope.base_story_table = await scope.normalizeTable( scope, { var_data: raw_var_data.hashes() });
});
...
When(/^(?:the user gets)?(?:I get)? to ?(?:the)?(?: ?question)?(?: id)? "([^"]+)" with this data:?/, {timeout: -1}, async (target_id, raw_var_data) => {
  ...
  * Iterate in here to keep timeout reasonable */
  ...
  await scope.use_the_story_table( scope, { target_id, raw_var_data });
});

When(/I get to ?(?:the)?(?: ?question)?(?: id)? "([^"]+)" with the (?:given|base|background) data/i, {timeout: -1}, async (target_id) => {
  /* Same as the story table (above), except it doesn't need a new story
  * table. It just uses `scope.base_story_table`. */
  await scope.use_the_story_table( scope, { target_id });
});
...
Given(/I log in with(?: the email)? "([^"]+)" and(?: the password)? "([^"]+)"(?: SECRETs)?/i, async ( email, password ) => {
  ...
  * 
  * Variations:
  * I log in with the email "USER1_EMAIL" and the password "USER1_PASSWORD" SECRETs
  * I log in with "USER1_EMAIL" and "USER1_PASSWORD"
  */
  await scope.log_in( scope, {
...
When(/I set the name of ?(?:the var(?:iable)?)? "([^"]+)" to "([^"]+)"/, async (var_base, name_str) => {
  ...
  */
  ...
  if (name_parts.length > 4) {
    ...
    await scope.addToReport(scope, {
      type: `warning`,
      value: `The name "${name_str}" has more than 4 parts, but 4 is maximum allowed. The test will set the name to "${name_parts.join(" ")}".`
    });
