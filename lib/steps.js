...
Then("I take a screenshot", async () => {
  await scope.page.screenshot({ path: `./screenshot_${ Date.now() }.jpg`, type: 'jpeg', fullPage: true });  // With timestamp name
...
Then('the question id should be {string}', async (question_id) => {  // √
  ...
  *     re.sub(r'[^A-Za-z0-9]+', '-', interview_status.question.id.lower()) */
  ...
  // Add error message saying what the screen id was
...
let tap_button_or_link = new RegExp(`I tap the ${ ordinal_regex } ?(?:"([^"]+)")? (button|link)`);
Then(tap_button_or_link, async (ordinal, phrase, elemType) => {
  /* Taps a button or link and stores what happens:
  *    navigation, validation error, page (alert?) error, just a click
  *    The result can be detected with the next step. */
  let start_url = await scope.page.url();

  if (!ordinal && !phrase) { throw new Error( `You need to describe the button in some way.` )}

  phrase = phrase || '';
  // Making this a race would probably be more simple
  let elems = [];
  // TODO: let elems = await Promise.race([]);
  // button elemType:
  if (elemType === 'button') {
    // TODO: replace with $x(`//button/descendant-or-self::*[contains(text(), "${ phrase }")]`)
    elems = await scope.page.$x(`//button/span[contains(text(), "${ phrase }")]`);
    if ( !elems ) {
      elems = await scope.page.$x(`//button[contains(text(), "${ phrase }")]`);
      if ( !elems ) {
        // This how we'll handle it for now, but oh boy
        // Possibly look into https://stackoverflow.com/a/56044998/14144258
        elems = await scope.page.$x(`//div[contains(@class, "form-actions")]//a[contains(text(), "${ phrase }")]`);
      }
    }
  // link elemType:
  } else {
    elems = await scope.page.$x(`//a[contains(text(), "${ phrase }")]`);
  }
  expect( elems, `No "${ phrase }" button was found` ).to.not.be.empty;

  // Get the correct element of the list of lements
  ordinal = ordinal || 'first';  // apparently `null` is the function's default value
  let index = scope.ordinal_to_index[ ordinal ];
  let elem = elems[ index ];
  expect( elem, `No ${ ordinal } "${ phrase }" button was not found` ).to.exist;

  // Either go to a new page or cause the user to see an error.
  let winner;
  if (elem) {
    // Click with no navigation will end immediately
    winner = await Promise.race([
      Promise.all([  // Error loads page, so no need to detect <- meaning?
        elem[ scope.activate ](),
...
Then(/I set the ?(?:"([^"]+)")? text field to "([^"]*)"/, async (field_label, to_type) => {
  ...
  */
  ...
  let id = await scope.getTextFieldId(scope, { field_label, type: 'input' });
  await scope.page.$eval(`#${id}`, (el) => { el.value = ''});
  if ( to_type ) { await scope.page.type( `#${id}`, to_type ); }  // Only type if needed
  await scope.afterStep(scope, { waitForShowIf: true });
});

let text_field = new RegExp(`I fill the ${ ordinal_regex } ?(?:"([^"]+)")? text (field|input|box) with "([^"]*)"`);
Then(text_field, async ( ordinal, field_label, desired_type, to_type ) => {
  /* Clears the matching text field or box in the question area and then types in it 
  * 
  * 'text box' means a multi-line text area
  * 'text field' or 'text input' means a one-line text field
  * 
  * Examples:
  * 1. I fill the first text field with "blah"
  * 1. I fill the "label" text field with "blah"
  * 1. I fill the second "label" text field with "blah"
  * 1. I fill the first text box with "blah"
  * 1. I fill the "label" text box with "blah"
  * 1. I fill the second "label" text box with "blah"
  * 1. I fill the second "label" text box with ""
  */
  let type;
  if ( desired_type === 'box' ) { type = 'textarea'; }
  else { type = 'input'; }
  let id = await scope.getTextFieldId(scope, { field_label, type });
...
When('I go to the help screen', async () => {
  try {
    await scope.page.click( `#dahelptoggle` );
  } catch (error) {
    console.warn('No help page was found');
    throw( error );
  }
  
  await scope.afterStep(scope, { waitForTimeout: 500 });
});

When('I go back to the question screen', async () => {
  try {
    await scope.page.click( `#dabackToQuestion` );
  } catch (error) {
    console.warn(`This might not be a help screen`);
    throw( error );
  }
