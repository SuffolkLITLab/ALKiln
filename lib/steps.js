...
Before(async (scenario) => {
  ...
  scope.safe_id = `_${ scenario_name }${ tags }`
  ...
  // Report
  scope.report[ scope.safe_id ] = { ids: [] };
...
Given(/I start the interview at "([^"]+)"(?: in lang "([^"]+)")?/, async (file_name, language) => {  // √
  ...
  // This won't be needed after tags get added in
  if ( language ) { scope.safe_id = `${scope.safe_id}_${ language }`; }
...
When(/^(some|all)(?: of)?(?: the(?:se)? var(?:iable)?s are used)?/, {timeout: -1}, async (how_many, raw_var_data) => {
  ...
  * Iterate in here to keep timeout reasonable */
  ...
  if ( sample.var === undefined || sample.choice === undefined || sample.value === undefined ) {
...
After(async function(scenario) {
  if (scenario.result.status !== 'passed') {
    await scope.addToReport(scope, { key: 'ids', value: '~ something went wrong here ~' });
