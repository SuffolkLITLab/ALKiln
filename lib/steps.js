...
BeforeAll(async () => {
  ...
  // This is used by at least run_cucumber.js.
...
Before(async (scenario) => {
  ...
  if (!scope.browser) {
    scope.browser = await scope.driver.launch({ args: ['--no-sandbox'], headless: !session_vars.get_debug(), devtools: session_vars.get_debug() });
  }
...
After(async function(scenario) {
  ...
  if ( scope.page ) {
    ...
    try {
      ...
      console.log(`${ session_vars.get_da_server_url() }/user/sign-out`);
      await scope.page.goto(`http://${ session_vars.get_da_server_url() }/user/sign-out`, {waitUntil: `domcontentloaded`});  
