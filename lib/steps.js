...
const make_artifacts_folder = require('./utils/make_artifacts_folder' );
...
BeforeAll(async () => {
  ...
  if ( !session_vars.is_dev_env() ) {
    ...
    scope.paths.artifacts = make_artifacts_folder();
...
Before(async (scenario) => {
  ...
  scope.paths.scenario = `${ scope.paths.artifacts }/${ scope.base_filename }`;
...
Given(/I start the interview at "([^"]+)"(?: in lang "([^"]+)")?/, {timeout: -1}, async (file_name, language) => {  // √
    ...
    (async function() {
      ...
      try {
        ...
        await scope.page._client.send('Page.setDownloadBehavior', {
          behavior: 'allow',
          downloadPath: path.resolve( scope.paths.scenario ),  // Save in root of this project
        });
        resolve();
  ...
  scope.page.on('response', async response => {
    ...
    if ( response._headers['content-disposition'] && (response._headers['content-disposition']).includes(`attachment;filename=${scope.toDownload}`) ) {
      ...
      // Renaming isn't working yet.

      let listener = function (curr, prev) {
        /* Listen for the file to finish downloading, then rename it */
        // If current size eq to expected size (size from response) then close
        if (parseInt(curr.size) === parseInt(response._headers['content-length'])) {
          let extension = path.extname( scope.toDownload );
          let new_filename = `${ path.basename( scope.toDownload, extension )}-${ Date.now() }${ extension }`
          let old_path = path.join( scope.paths.scenario, scope.toDownload );
          let new_path = path.join( scope.paths.scenario, new_filename );
      ...
      };  // ends fs.watchFile listener

      // Watch event on download folder or file, and be sure to stop watching
      // https://stackoverflow.com/a/12551777/14144258
      // https://nodejs.org/api/fs.html#fsunwatchfilefilename-listener
      await fs.watchFile( scope.paths.scenario, listener);
      await fs.unwatchFile( scope.paths.scenario, listener);
...
When(/I wait (\d*\.?\d+) seconds?/, { timeout: -1 }, async ( seconds ) => {
...
After(async function(scenario) {
  ...
  if (scenario.result.status !== `PASSED`) {
    ...
    if ( scope.page ) {
      ...
      // Save it in the outer-most artifact folder
      let safe_filename = await scope.getSafeScenarioFilename( scope, { prefix: `error_screenshot` });
      await scope.page.screenshot({
        path: `${ scope.paths.artifacts }/${ safe_filename }.jpg`,
        type: `jpeg`,
        fullPage: true
      });
      // Save it in the artifact's scenario folder
      await scope.page.screenshot({
        path: `${ scope.paths.scenario }/error_screenshot-${ Date.now() }.jpg`,
        type: `jpeg`,
        fullPage: true
      });
    }  // ends if scope.page
  ...
  fs.writeFileSync( `${ scope.paths.scenario }/report.txt`, report );
...
AfterAll(async function() {
  ...
  fs.writeFileSync( `${ scope.paths.artifacts }/report.txt`, report );
