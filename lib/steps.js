...
When(/I? ?gets? to ?.*?(".*") ?with.*?(?:this data:)?/, {timeout: -1}, async (target_ids, raw_var_data) => {
  ...
  * Iterate in here to keep timeout reasonable */
  ...
  let id_matches = id && ids_list.includes( id );
  ...
  while ( !id_matches ) {
    ...
    // Prep for next loop
    // ({ question_has_id, id, id_matches } = await scope.examinePageID( scope, target_ids ));
    id = await scope.get_page_id( scope );
    id_matches = id && ids_list.includes( id );
