...
When(/the final question id is "([^"]+)"/, async ( final_id_name ) => {
...
When(/(some|all)(?: of)?(?: the(?:se)? var(?:iable)?s are used)?/, {timeout: -1}, async (how_many, raw_var_data) => {
  ...
  let sample_row = var_data[0];
  let table_has_correct_headings = sample_row.var && sample_row.choice && sample_row.value
  let header_msg = `The table of variables needs the headings "var", "choice", and "value". ` +
    `The current headings: "${ Object.keys( var_data[0] ) }"`;
  expect( sample_row.var, header_msg ).to.exist;
  expect( sample_row.choice, header_msg ).to.exist;
  expect( sample_row.value, header_msg ).to.exist;


  // let document_wait = (120 * 1000) * 2;  // How to know how many documents might be loaded and downloaded
  // let var_wait = scope.showif_timeout + 50
  // scope.page.setDefaultTimeout( ( var_data.length() * var_wait ) + document_wait );  // timeout for each var (showif )


  // await scope.processVarTable( scope, { how_many, var_data });
  ...
  while ( !id_matches ) {  // && remaining_vars.length > 0 ) {

    // let timeout_message = `Variable table attempt did not resolve within ${ scope.timeout } ms.`;
    // let promise = scope.processVar(scope, { how_many, var_data: remaining_vars })
    // console.log( promise instanceof Promise );
    // remaining_vars = await Promise(scope.processVar(scope, { how_many, var_data: remaining_vars })).timeout(scope.timeout, message);
    ...
    let promise = new Promise(function(resolve, reject) {
        ...
        (async function() {
          // Some errors happen in there to be closer to the relevant code
          try {
            let result = await scope.processVar(scope, { how_many, var_data: remaining_vars, id });
            resolve( result );
          } catch (err) {
            reject( err );
          }
        ...
        // Set up the timeout
        setTimeout(function() {
            reject(`Variable table attempt did not resolve within ${ scope.timeout } ms. Check the error screenshot`);
        }, scope.timeout);

        // return scope.processVar(scope, { how_many, var_data: remaining_vars });
    });

    remaining_vars = await promise;
    expect( Array.isArray(remaining_vars) ).to.be.true;

    // remaining_vars = await Promise.race([
    //   scope.processVar(scope, { how_many, var_data: remaining_vars }),
    //   setTimeout(function() {
    //         throw `Variable table attempt did not resolve within ${ scope.timeout } ms.`;
    //   }, scope.timeout),
    // ]);
  ...
  if ( how_many === 'all' ) {
    ...
    let no_vars_used_msg = `No variables were used, but the question id "${ target_id }"` +
      ` was reached. The variables: ${ remaining_json }`;;
    expect( remaining_vars.length < total_vars, no_vars_used_msg ).to.be.true;
  }

  // // If reached the target page and needed to use up all the vars, but didn't
  // if ( id_matches && how_many === 'all' && remaining_vars.length > 0 ) {
  //   let message = `When the question id "${ target_id }" was reached ` +
  //     `there were some variables left over: ${ remaining_json }`
  ...
  // if ( id_matches && remaining_vars.length === total_vars ) {
  //   let message = `No variables were used, but the question id "${ target_id }"` +
  //     ` was reached. The variables: ${ remaining_json }`;
  //   // Better to error in here closer to where the relevant code is?
  //   throw new ReferenceError( message );
  // }
...
When(/I set the ?(?:"([^"]+)" choice of)? var(?:iable)? "([^"]+)" to "([^"]+)"/, async ( choice_name, var_name, set_to ) => {
  /* Set any variable (with or without a choice associated with it) to a value.
...
Then('I sign', async () => {
  let canvas = await scope.page.$('canvas');
